    ...
    class employees extends CI_Controller
    {
        ...
        function index()
        {
            $data["Modules"] =  array(
                ...
                array("title" => "Approval Route",                    "icon" => "check-to-slot-duotone.svg",          "url" => "employees/approval_routes",         "info"=>"Appoint approvers and set a sequential workflow, where request moves from one approver to the next in a predefined order.",             "access" => "Employee", "id" => "approval_route")
            );
            ...
            $data['settings']                   = "employees/employee_types";
        ...
        //     $data["module"]           = [base_url() . $module, "Employees", "Employee Types"];         // Main Menu Path, Module, Page Title
        //     $data["id_prefix"]        = "EMT";
        ...
        //     $this->load->view('templates/header');
        //     $this->load->view('templates/employee_settings_views', $data);
        // }
        function employee_types(){
        ...
        //     $data["module"]           = [base_url() . $module, "Employees", "Positions"];         
        //     $data["id_prefix"]        = "POS";
        ...
        //     $data["module"]           = [base_url() . $module, "Employees", "Branches"];        
        //     $data["id_prefix"]        = "BRC";
        ...
        function directories()
        {
            ...
            // if($this->input->get('all') == null){
            if (!$search) {
                $data['DISP_EMP_LIST']          = $this->employees_model->MOD_DISP_ALL_EMPLOYEES_LIMIT_DIRECTORIES($offset, $row, $dept, $sec, $group, $line, $branch, $division, $team, $status, $company, $employment_type);
                $data['C_DATA_COUNT']           = count($this->employees_model->FILTER_EMPLOYEE_COUNT_DIRECTORIES($dept, $sec, $group, $line, $branch, $division, $team, $status, $company, $employment_type));
                // $data['C_DATA_COUNT']           = count($this->employees_model->FILTER_EMPLOYEE_COUNT($dept,$sec,$group,$line,$branch,$division,$team,$status,$company));
            } else {
        ...
        function approval_routes()
        {
            ...
            $data['DISP_APPROVERS']             = $this->employees_model->GET_EMPL_APPROVALS_NEW($search, $limit, $offset, $filter_arr);
        ...
        //     // return;
        //     $data['DISP_EMPLOYEES_NONFILTERED'] = $this->employees_model->MOD_DISP_ALL_EMPLOYEES();
        //     $data['DISP_APPROVERS']             = $this->employees_model->GET_EMPL_APPROVALS($search,$limit,$offset,$filter_arr);
        //     $total_count = $this->employees_model->GET_EMPL_APPROVALS_COUNT($search,$filter_arr);
        //     $excess      = $total_count%$limit;
        //     $data['C_DATA_COUNT']   = $total_count;
        //     $data['PAGES_COUNT']    = $excess>0 ? intval($total_count/$limit)+1: intval($total_count/$limit);
        ...
        // 	$data['DISP_VIEW_LINE']           = $this->employees_model->GET_SYSTEM_SETTING("com_line");
        //     $this->load->view('templates/header');
        //     $this->load->view('modules/employees/approval_route_views_old',$data);
        // }
        ...
        function salary_history()
        {
            ...
            $personal                            = $this->employees_model->GET_EMPLOYEE_SPECIFIC($id);
            $data['FIRSTNAME']                   = $personal[0]->col_frst_name;
        ...
        function salary_details()
        {
            ...
            if (!$search) {
                ...
                $data['DISP_EMP_LIST_TABLE']            = $empl_list = $this->employees_model->GET_FILTERED_EMPLOYEELIST_TABLE($offset, $row, $param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line, $param_company);
            } else {
                // $data['DISP_EMP_LIST']                  = $this->employees_model->GET_SEARCHED($search);
        ...
        function payroll_assignment()
        {
            ...
            //   if($this->input->get('all') == null){
            if (!$search) {
                $data['DISP_EMP_LIST']                    = $empl_list = $this->employees_model->GET_FILTERED_EMPLOYEELIST_PAYROLL_ASSIGNMENT($offset, $row, $param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line);
                $data['C_DATA_COUNT']                     = $this->employees_model->GET_COUNT_FILTERED_EMPLOYEE_PAYROLL_ASSIGNMENT($param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line);
            } else {
                $data['DISP_EMP_LIST']                    = $this->employees_model->GET_SEARCHED_PAYROLL_ASSIGNMENT($search);
                $data['C_DATA_COUNT']                     = count($this->employees_model->GET_SEARCHED_PAYROLL_ASSIGNMENT($search));
            }
        ...
        function update_employment_detail()
        {
            ...
            if (empty($new)) {
                ...
                $this->employees_model->UPDATE_EMPLOYMENT_DET($hired_date, $reg_date, $resign_Date, $end_date, $emp_type, $position, $branch, $dept, $division, $sec, $group, $line, $team, $com_num, $com_email, $hmo_prov, $hmo_num, $termination_reason, $resignation_reason, $termination_date, $user_id);
                $edit_id = $this->session->userdata('SESS_USER_ID');
                $empl_id = $user_id;
                try {
                    foreach ($new as $new_row) {
        ...
        function setup_organization()
        {
            ...
            if (!$search || true) {
                // $data['C_DATA_COUNT']                   = $this->employees_model->GET_FILTERED_EMPLOYEELIST_COUNT($param_branch,$param_dept,$param_division,$param_section,$param_group,$param_team,$param_line,$param_company);
                $data['C_DATA_COUNT']                   = $this->employees_model->GET_FILTERED_SETUP_ORGANIZATION_TABLE_COUNT($param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line, $param_company, $search);

                $data['DISP_EMP_LIST_TABLE']            = $empl_list = $this->employees_model->GET_FILTERED_SETUP_ORGANIZATION_TABLE($offset, $row, $param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line, $param_company, $search);
            } else {
                $data['DISP_EMP_LIST_TABLE']            = $this->employees_model->GET_SEARCHED_SALARY_DETAILS($search);
                $data['C_DATA_COUNT']                   = count($this->employees_model->GET_SEARCHED_SALARY_DETAILS($search));
            }
