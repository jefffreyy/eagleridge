    ...
    <nav aria-label="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Payslip Generator
            ...
            <div class="row">
                <!-- Title Text -->
                <div class="col-md-6">
                    <h1 class="page-title"><a href="<?= base_url().'payrolls';?>"><i class="fa-duotone fa-circle-left"></i></a>&nbsp;Payslip Generator<h1>
                </div>
            ...
            <form action="<?php echo base_url('payrolls/generator'); ?>" id="payslip_period" method="post" accept-charset="utf-8" autocomplete='off'>
                <div class="row">
                    ...
                    <div class="col-md-9 mt-4">
                         ...
                         <a href=<?= base_url().'payrolls/downloadExcel'?> class="btn btn-primary float-right ml-2 mb-2"><i class="fas fa-file-export"></i>&nbsp;Custom Payslip</a>
             ...
             <div class="row">
                <div class="col-md-12">
                    <div class="remote_att">
                        ...
                        <a href=<?=base_url().'payrolls/with_payslip/'.$date ?> class="btn btn-primary mb-2" id="btn_quick_view" hidden><i class="fa-kit fa-solid-bolt-pen"></i>&nbsp;Quick View</a>
                                    ...
                                    <tbody>
                                        <?php if (count($PAYSLIPS) > 0) { ?>
                                            <?php foreach($PAYSLIPS as $payslip_row){ 
                                                $name = $payslip_row['col_last_name'];
                                                if(!empty($payslip_row['col_suffix']))$name =$name.' '.$payslip_row['col_suffix'];
                                                if(!empty($payslip_row['col_frst_name']))$name =$name.', '.$payslip_row['col_frst_name'];
                                                if(!empty($payslip_row['col_midl_name']))$name =$name.' '.$payslip_row['col_midl_name'][0].'.';
                                                ?>
                                                 <tr class="payslip_row">
                                                    ...
                                                    <td><?= $name
                                                    // $payslip_row['col_frst_name'].' '. $payslip_row['col_last_name']
                                                     ?> 
                                                    </td>
                            ...
                            <?php if(count($PAY_SLIPS_READY)>0) { ?>
                                 <?php foreach($PAY_SLIPS_READY as $employee) { 
                                    ...
                                    <tr>
                                        ...
                                        <td><?=$name 
                                        // $employee['col_last_name'].' '.$employee['col_frst_name']
                                        ?></td>
    ...
    <script>
        function changeTab(e) {
            if (e) {
                ...
                $("#btn_quick_view").removeAttr("hidden");
                
            } else {
                $("#delete_all_payslips").attr("hidden", true);
                $("#btn_export").attr("hidden", true);
                $("#btn_quick_view").attr("hidden", true);
            }
        }
        $(document).ready(function() {
            ...
            $('.btn-generate_payslip_pdf').on('click',function(){
                ...
                if(record.col_midl_name)name = `${name} ${record.col_midl_name[0]}.`;
                doc.text(xcoor, ycoor+=increment_by,record.col_empl_cmid);
                doc.text(xcoor, ycoor+=increment_by,name);
                ...
                let tot_2=[  format(record.TOT_ABSENT), format(record.TOT_TARDINESS), format(record.TOT_UNDERTIME), format(record.WTAX), format(record.SSS_EE_CURRENT), format(record.PAGIBIG_EE_CURRENT), format(record.PHILHEALTH_EE_CURRENT)];
                ...
                let tot_3=[ format(record.SSS_ER_CURRENT), format(record.PAGIBIG_ER_CURRENT), format(record.PHILHEALTH_ER_CURRENT)];
                ...
                doc.text(x_coor,y_coor, format(record.EARNINGS), {align: 'right'});

                x_coor=159;
                doc.text(x_coor,y_coor, format(record.DEDUCTIONS), {align: 'right'});
   
                x_coor=201;
                doc.text(x_coor,y_coor, format(record.NET_INCOME) , {align: 'right'});
                    ...
                    data.empl_not_ready_for_payslip.forEach(function(id) {
                        getEmployeeData(url3, id).then(data1 => {
                            data1.employee_data.forEach(function(x) {
                                ...
                                let fullName = x.col_last_name;
                                if(x.col_suffix)fullName = `${fullName} ${x.col_suffix}`;
                                if(x.col_frst_name)fullName = `${fullName}, ${x.col_frst_name}`;
                                if(x.col_midl_name)fullName = `${fullName} ${x.col_midl_name[0]}.`;
                                $('#container_employees_not_ready_for_payslip').append(`
                                    <tr class="payslip_row">
                                        <td>` + x.col_empl_cmid + `</td>
                                        <td><a>
