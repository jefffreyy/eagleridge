...
<div class="content-wrapper">
  <div class="container-fluid p-4">
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title d-flex align-items-center" id="page_title"><a href="<?= base_url() . 'payrolls/'.$TAB.'?period='.$PERIOD; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" /></a>&nbsp;Payslip Published Details<h1>
      </div>
    ...
    <div class="row">
      ...
      <div class=" col-md-2 py-2">
        <div class="d-flex">
          <input type="checkbox" class="form-control mx-2" id="input_ta" name="input_ta" style="width: 20px; height:20px; font-size: 13px;" <?= ($SPECIFIC_PAYROLL_SCHEDULES->chk_taxable == "0") ? 'checked' : '' ?> disabled>
          <label for="cb_ti" class="mr-4"> Taxable Allowance</label><br>
        </div>
      </div>
      <div class=" col-md-2 py-2">
        <div class="d-flex">
          <input type="checkbox" class="form-control mx-2" id="input_nta" name="input_nta" style="width: 20px; height:20px; font-size: 13px;" <?= ($SPECIFIC_PAYROLL_SCHEDULES->chk_nontaxable == "0") ? 'checked' : '' ?> disabled>
...
<script>
  function exportToExcel() {
    ...
    .map((_, index) => index)
    .filter(index => !hiddenColumns.columns.includes(index));
    const visibleHeaders = visibleColumnIndices.map(index => allHeaders[index]);
    const visibleData = hot.getData().map(row =>
      row.filter((_, colIndex) => !hiddenColumns.columns.includes(colIndex))
    );
    ...
    XLSX.writeFile(wb, 'payslip_published_detail.xlsx');
  }
</script>
</body>
</html>