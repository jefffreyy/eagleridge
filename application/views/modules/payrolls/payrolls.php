...
class payrolls extends CI_Controller
{
  ...
  function index()
  {
    $data["Modules"] =  array(
      ...
      array("title" => "Withholding Tax Table",           "value" => "Payroll Tax",                   "info" => "Generate employees withholding tax.     ",       "icon" => "wtax.svg",                         "url" => "payrolls/payroll_tax",                "access" => "Payroll", "id" => "payroll_tax"),
  ...
  function thitteenthmonthpay()
  {
    ...
    if ($this->input->get('search') == null) {
      $data['DISP_EMP_LIST']                  = $this->payrolls_model->GET_FILTERED_EMPLOYEELIST($offset, $row, $param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line);
  ...
  function payroll_summary()
  {
    ...
    if (!empty($specific_payroll_sched)) {
      ...
      ($specific_payroll_sched->chk_earnings == null    || $specific_payroll_sched->chk_earnings == 1)    ? $en_ti = 0 : $en_ti = 1;
      ($specific_payroll_sched->chk_deductions == null  || $specific_payroll_sched->chk_deductions == 1)  ? $en_nti = 0 : $en_nti = 1;
    ...
    preg_match('/\b\d{4}\b/', $specific_payroll_sched->name, $matches);
    $year                             = $matches[0] ?? null;
    ...
    foreach ($attendanceRecordLockFiltered as $employee) {
      ...
      $gross_income        = $basic_pay + $ot_pay + $tax_allowance_total + $nontax_allowance_total;
      ...
      $sss_ee_current         = isset(($this->payrolls_model->GET_SSS_VALUE_PAYSLIP($gross_income_total, $period_year))[0]->ee) ? ($this->payrolls_model->GET_SSS_VALUE_PAYSLIP($gross_income_total, $period_year))[0]->ee : 0;
      $sss_er_current         = isset(($this->payrolls_model->GET_SSS_VALUE_PAYSLIP($gross_income_total, $period_year))[0]->er) ? ($this->payrolls_model->GET_SSS_VALUE_PAYSLIP($gross_income_total, $period_year))[0]->er : 0;
      $sss_ec_er_current      = isset(($this->payrolls_model->GET_SSS_VALUE_PAYSLIP($gross_income_total, $period_year))[0]->ec_er) ? ($this->payrolls_model->GET_SSS_VALUE_PAYSLIP($gross_income_total, $period_year))[0]->ec_er : 0;
      ...
      if ($regular_pay <= $philhealth_min_basic) {
        $philhealth_ee_current = $philhealth_min_premium / 2;
        $philhealth_er_current = $philhealth_min_premium / 2;
      } else if ($regular_pay >= $philhealth_max_basic) {
        $philhealth_ee_current = $philhealth_max_premium / 2;
        $philhealth_er_current = $philhealth_max_premium / 2;
      } else {
        $philhealth_ee_current = ($regular_pay * ($philhealth_rate / 100)) / 2;
        $philhealth_er_current = ($regular_pay * ($philhealth_rate / 100)) / 2;
      }
      ...
      $earnings                               = 0;
      $deductions                             = 0;
      $net_income                             = $gross_income - $wtax - $ee_difference_total - $loan_total;
      ...
      $employeeObject->salary_rate            = number_format($salary_rate, 2);             //SALARY RATE
      ...
      $employeeObject->deduct_total           = number_format(0, 2);
      ...
      $employeeObject->sss_er_current         = number_format((float)$sss_er_diff, 2);
      $employeeObject->sss_ec_er_current      = number_format((float)$sss_ec_er_diff, 2);
      $employeeObject->pagibig_er_current     = number_format((float)$pagibig_er_diff, 2);
      $employeeObject->philhealth_er_current  = number_format((float)$philhealth_er_diff, 2);


      $employeeObject->benefits_col           = $DISP_ATTENDANCE_LOCK_BENEFITS;
  ...
  function published_view_details()
  {
    ...
    // ($specific_payroll_sched->chk_earnings == null || $specific_payroll_sched->chk_earnings == 1) ? $en_ti = 1 : $en_ti = 0; 
    // ($specific_payroll_sched->chk_deductions == null || $specific_payroll_sched->chk_deductions == 1) ? $en_nti = 1 : $en_nti = 0; 
    ...
    $data['DISP_EMPLOYEES']                   =  $matchedEmployees;
    $employeePayrollSummary = [];
    ...
    //   $cmid                                   = $employee->col_empl_cmid;
    //   $employeeObject                         = new stdClass();
    //   $employeeObject->id                     = $empl_id;
    //   $employeeObject->salary_rate            = number_format($salary_rate, 2);
    //   $employeeObject->salary_type            = $salary_type;
    //   $employeeObject->fullname               = $fullname;
    //   $employeeObject->cmid                   = $cmid;
    //   $employeeObject->period                 = $period;
    //   if ($salary_type == "Daily" || $salary_type == "daily") {
    //     $daily_salary       = $salary_rate;
    //     $hourly_salary      = $daily_salary / 8;
    //   } else {
    //     $payroll_monthly_cons = $this->payrolls_model->GET_PAYROLL_MONTHLY_CONSTANT();
    //     $daily_salary       =  $salary_rate * 12 / $payroll_monthly_cons;
    //     $hourly_salary      = $daily_salary / 8;
    //   }
    //   $employeeObject->daily_salary             = number_format($daily_salary, 2);
    //   $employeeObject->hourly_salary            = number_format($hourly_salary, 2);
    //   $DISP_ATTENDANCE_LOCK =  isset(($this->payrolls_model->GET_ATTENDANCE_LOCK($empl_id, $period))[0]) ? ($this->payrolls_model->GET_ATTENDANCE_LOCK($empl_id, $period))[0] : 0;
    //   $mul_present        = 1;
    //   $mul_absent         = 1;
    //   $mul_tardiness      = 1;
    ...
    //   $employeeObject->mul_sperest_nd         = $mul_sperest_nd;
    //   $employeeObject->mul_sperest_ndot       = $mul_sperest_ndot;
    //   $employeeObject->present                = number_format((float)$tot_present, 2);
    //   $employeeObject->absent                 = number_format($tot_absent, 2);
    //   $employeeObject->tardiness              = number_format($tot_tardiness, 2);
    //   $employeeObject->undertime              = number_format($tot_undertime, 2);
    //   $employeeObject->paid_leave             = number_format((float)$tot_paid_leave, 2);
    //   $employeeObject->reg_hours              = number_format($tot_reg_hours, 2);
    //   $employeeObject->reg_ot                 = number_format($tot_reg_ot, 2);
    //   $employeeObject->reg_nd                 = number_format($tot_reg_nd, 2);
    //   $employeeObject->reg_ndot               = number_format($tot_reg_ndot, 2);
    //   $employeeObject->rest_hour              = number_format($tot_rest_hours, 2);
    ...
    //   $employeeObject->spe_ot                 = number_format($tot_spe_ot, 2);
    //   $employeeObject->spe_nd                 = number_format($tot_spe_nd, 2);
    //   $employeeObject->spe_ndot               = number_format($tot_spe_ndot, 2);
    //   $employeeObject->sperest_hours          = number_format($tot_sperest_hours, 2);
    //   $employeeObject->sperest_ot             = number_format($tot_sperest_ot, 2);
    //   $employeeObject->sperest_nd             = number_format($tot_sperest_nd, 2);
    //   $employeeObject->sperest_ndot           = number_format($tot_sperest_ndot, 2);
    //   $employeeObject->earnings               = number_format($earnings, 2);
    //   $employeeObject->deductions             = number_format((float)$deductions, 2);
    //   $employeeObject->wtax                   = number_format((float)$wtax, 2);
    //   $employeeObject->net_income             = number_format((float)$net_income, 2);
    //   $employeeObject->gross_income             = number_format((float)$gross_income_total, 2);
    //   $employeeObject->tax_allowance_total    = number_format($tax_allowance_total, 2);
    //   $employeeObject->nontax_allowance_total = number_format($nontax_allowance_total, 2);
    //   $employeeObject->sss_ee_current         = number_format((float)$sss_ee_diff, 2);
    //   $employeeObject->sss_er_current         = number_format((float)$sss_er_diff, 2);
    ...
    //   $employeeObject->philhealth_er_current  = number_format((float)$philhealth_er_diff, 2);
    //   $employeeObject->loan_total             = number_format((float)$loan_total, 2);
    //   $employeeObject->ca_total               = number_format((float)$ca_total, 2);
    //   $employeeObject->deduct_total           = number_format((float)$deduct_total, 2);
    //   $employeeObject->loan_list              = $string_loan;
    //   $employeeObject->ca_list                = $string_ca;
    //   $employeeObject->deduct_list            = $string_deduct;
    //   $employeeObject->id_sss                 = $ID_SSS;
    //   $employeeObject->id_pagibig             = $ID_PAGIBIG;
    //   $employeeObject->id_philhealth          = $ID_PHILHEALTH;
    //   $employeeObject->id_tin                 = $ID_TIN;
    //   $employeePayrollSummary[] = $employeeObject;
    // }
    $data['DISP_PAYROLL_SUMMARY'] = $employeePayrollSummary;
  ...
  function insert_payroll_sched()
  {
    ...
    $input_earnings                 = $this->input->post('input_earnings');
    ...
    ($input_earnings != null) ? $input_earnings = 0 : $input_earnings = 1;
    ($input_deductions != null) ? $input_deductions = 0 : $input_deductions = 1;
