...
class payrolls extends CI_Controller
{
  ...
  function index()
  {
    ...
    //   array("title" => "Attendance Records Lock",         "value" => "Attendance Records Lock",         "icon" => "fa-duotone fa-file-lock",     "url" => "payrolls/attendance_records_lock",  "access" => "Payroll", "id" => "attendance_records_lock"),
      ...
      array("title" => "BIR 2316 Form",         "value" => "Payroll BIR2316",        "icon" => "fa-kit fa-bir2316",                             "url" => "payrolls/bir_form2316",             "access" => "BRI_2316", "id" => "payroll_bri2316"),
      ...
      array("title" => "SSS Contribution Table",          "value" => "SSS Contribution",              "icon" => "fa-kit fa-sss",      "url" => "payrolls/payroll_sss",                "access" => "Payroll", "id" => "payroll_sss"),
      array("title" => "PhilHealth Contribution Table",   "value" => "Payroll PhilHealth",            "icon" => "fa-kit fa-phic",     "url" => "payrolls/payroll_philhealth",         "access" => "Payroll", "id" => "payroll_philhealth"),
      array("title" => "HDMF Contribution Table",         "value" => "Payroll HDMF",                  "icon" => "fa-kit fa-hdmf",     "url" => "payrolls/payroll_hdmf",               "access" => "Payroll", "id" => "payroll_hdmf"),
      array("title" => "Withholding Tax Table",           "value" => "Payroll Tax",                   "icon" => "fa-kit fa-wtax",     "url" => "payrolls/payroll_tax",                "access" => "Payroll", "id" => "payroll_tax"),
      array("title" => "Government Contribution",         "value" => "Government Contribution",       "icon" => "fa-kit fa-wtax",     "url" => "payrolls/government_contribution",    "access" => "Payroll", "id" => "government_contribution"),
      array("title" => "Payroll Summary",                 "value" => "Payroll Summary",               "icon" => "fa-kit fa-wtax",     "url" => "payrolls/payroll_summary",            "access" => "Payroll", "id" => "payroll_summary"),
      array("title" => "Payroll Status",                  "value" => "Payroll Status",                "icon" => "fa-kit fa-wtax",     "url" => "payrolls/payroll_status",             "access" => "Payroll", "id" => "payroll_status"),
    );
...
//   
  ...
  function payroll_summary(){
    ...
    $data['DISP_EMPLOYEES']                   = $employees = $this->payrolls_model->GET_EMPLOYEELIST($offset, $row);
    ...
    foreach ($employees as $employee) {
      ...
      if($salary_type == "Daily" || $salary_type == "daily"){
        ...
        $daily_salary       =  $salary_rate * 12 / 313;
        $hourly_salary      = $daily_salary / 8;
      }
      ...
      $employeeObject->sss                    = number_format((float)$sss_ee_diff, 2);
      $employeeObject->pagibig                = number_format((float)$pagibig_ee_diff, 2);
      $employeeObject->philhealth             = number_format((float)$philhealth_ee_diff, 2);
    ...
    $data['SPECIFIC_PAYROLL_SCHEDULES']               = $this->payrolls_model->GET_SPECIFIC_PAYROLL_SCHEDULE($period);
  ...
  function payroll_status(){
    ...
    $data['PERIOD']                             = $period;
    $data['PAYROLL_PAYSLIP_LIST']               = $pasyslip_list = $this->payrolls_model->GET_PAYROLL_PAYSLIPS($period);
    ...
    $data['DISP_EMP_LIST']                      = $filteredEmployees;
    $this->load->view('templates/header');
    $this->load->view('modules/payrolls/payroll_summary_status_views', $data);
  }

  function payroll_generated(){
    $this->load->view('templates/header');
    $this->load->view('modules/payrolls/payslip_generated_views');
  }
  ...
  function insert_payroll_sched(){
    ...
    $this->payrolls_model->INSERT_PAYROLL_SCHEDULE($title,$year,$start_date,$end_date,$payout_date,$pay_freq,$con_period_1,$con_period_2,$con_period_3,$con_period_4,$con_period_5,$status,$input_sss,$input_phil,$input_pagibig,$input_wtax,$input_ta,$input_nta,$input_loans,$input_tard);
    $this->session->set_userdata('SESS_SUCC_MSG_PAY_SCHED', 'Added Successfully!');
    redirect('payrolls/payroll_schedules');
  }
  ...
  function edit_specific_payroll_sched($id){
    ...
    $this->payrolls_model->EDIT_PAYROLL_SCHEDULE($title,$year,$start_date,$end_date,$payout_date,$pay_freq,$con_period_1,$con_period_2,$con_period_3,$con_period_4,$con_period_5,$status,$input_sss,$input_phil,$input_pagibig,$input_wtax,$input_ta,$input_nta,$input_loans,$input_tard,$id);
    $this->session->set_userdata('SESS_SUCC_MSG_PAY_SCHED', 'Updated Successfully!');
    redirect('payrolls/payroll_schedules');
  }
