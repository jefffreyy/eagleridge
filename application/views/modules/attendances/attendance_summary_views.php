...
<div id="loadingOverlay" class="loading-overlay" hidden>
  ...
  <div class="container-fluid p-4">
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title d-flex align-items-center" id="page_title"><a href="<?= base_url() . 'attendances'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
      ...
      <div class="col-md-6 button-title ">
        ...
        <?php echo form_open_multipart('attendances/attendance_summary_assumedate'); ?>
        <div class="d-flex align-items-center gap-3 justify-content-end">

            <input type="hidden" class="form-control" name="cut_off_period" id="cut_off_period" value="<?= $CUTOFF_PERIOD_ASSUME ?>" required style="margin-right: 5px">
            <!-- Start Date Group -->
            <div class="d-flex align-items-center gap-2 mb-0">
                <label for="start_date" class="mb-0" style="margin-right: 5px"> Start&nbsp;Date:</label>
                <input type="date" class="form-control" name="start_date" id="start_date" value="<?= ($DISP_ASSUME_DATES) ? $DISP_ASSUME_DATES->start_date : ""; ?>" required style="margin-right: 5px">
            </div>

            <!-- End Date Group -->
            <div class="d-flex align-items-center gap-2 mb-0">
                <label for="end_date" class="mb-0" style="margin-right: 5px">End&nbsp;Date:</label>
                <input type="date" class="form-control" name="end_date" id="end_date" value="<?= ($DISP_ASSUME_DATES) ? $DISP_ASSUME_DATES->end_date : ""; ?>" required style="margin-right: 5px">
            </div>

            <!-- Submit Button -->
            <div class="d-flex align-items-center mb-0">
                <button type="submit" id="btn_add" class="btn btn-primary shadow-none rounded" style="margin-right: 5px">Update Assume Date</button>
            </div>

            <!-- Delete Assume Date Link -->
          <!-- <a href="<?= base_url('attendances/delete_assume_dates/'.$CUTOFF_PERIOD_ASSUME ) ?>" class="btn btn-danger shadow-none rounded deleteAssumeDataBtn"><i class="fa-duotone fa-trash"></i>&nbsp;Delete Assume Date</a> -->
          <button data-assume-id="<?= $CUTOFF_PERIOD_ASSUME ?>"  class="btn btn-danger shadow-none rounded deleteAssumeDataBtn"><i class="fa-duotone fa-trash"></i>&nbsp;Delete Assume Date</button>
         

        </div>
        <?php echo form_close(); ?>

        <!-- <a href="<?= base_url('attendances/delete_assume_dates') ?>" class="btn btn-primary" id="btn-delete_assumedate"><i class="fa-duotone fa-pen-to-square"></i>&nbsp;Delete Assume Date</a> -->
      </div>
    ...
    <div class="d-flex justify-content-between align-items-end row">
      ...
      <div class='d-lg-flex d-block justify-content-end mr-1 col-12 col-lg-8'>
        <a  onclick="afterRenderFunction()" href="<?= base_url('attendances/attendance_basic_view') ?>" class="btn btn-primary mr-1">
          Basic View
        </a>
        <button class="btn btn-primary " id="btn-insert"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="" />
          Endorse to Payroll</button>
        <button type="button" class="btn btn-primary ml-1" data-toggle="modal" data-target="#settingsModal" id="hide_col_btn"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/eye-slash-duotone.svg') ?>" alt="" />
          &nbsp;Hide Columns</button>
        <button type="button" class="btn btn-primary ml-1 mt-1 mt-lg-0" id="printButton"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/print-duotone_sm.svg') ?>" alt="" />&nbsp;Print</button>
        <button onclick="exportToExcel()" class=" mt-1 mt-lg-0 btn btn-primary ml-1 shadow-none rounded d-flex align-items-center ml-1" style="width: auto; display: inline-block !important;">
          <img style="margin-bottom: 4px;" src="<?= base_url('assets_system/icons/file-export-solid.svg') ?>" alt="">&nbsp;Export XLSX
        </button>
      </div>
    </div>
    <div class="card border-0 p-0 m-0">
      <div class="card border-0 pt-1 m-0">
        ...
        <div class="p-2">
          <div class="d-flex">
            <div class="mx-2 legend-container">
              <!-- <p>Legend : </p> -->
              ...
              <span class="d-block legend">RC SUB - Rice Subsidy</span>
              <span class="d-block legend">RC ALO - Rice Allowance</span>
              <span class="d-block legend">OVM ALO - Overtime Meal Allowance</span>
            </div>

            <div class="ml-2 legend-container">
              <span class="d-block legend">PAID - Paid Leave</span>
              <span class="d-block legend">LWOP - Leave Without Pay</span>
              <span class="d-block legend">AWOL - Absent Without Official Leave</span>
              <span class="d-block legend">TARD - Tardiness</span>
              <span class="d-block legend">UT - Undertime</span>
              <span class="d-block legend">EARB - Early Break</span>
              <span class="d-block legend">OVRB - Over Break</span>
            </div>
            ...
            <div class="ml-1 legend-container">
              ...
              <span class="d-block legend">RST + LEG NDOT - Rest + Legal Regular Night Differential / Overtime</span>
            </div>
            ...
            <div class="ml-1 legend-container">
              ...
              <span class="d-block legend">RST + SPE NDOT - Rest + Special Regular Night Differential / Overtime</span>
            </div>
...
<script>
  ...
  const dateHeaders             = [];
  const presentColumns          = ['DAYS', 'ASSUME DAYS', 'ASSUME DATES'];
  const sliders                 = [''];
  const benefitsColumns         = ['RC SUB', 'RC ALO', 'OVM ALO', 'TRANS ALLO', 'MEAL ALLO'];
  const attendanceAbsences      = ['PAID', 'LWOP', 'AWOL', 'PREV ASSUME ABSENT', 'PREV ASSUME ABSENT DATES'];
  const attendanceDeductions    = ['TARD', 'UT', 'EARB', 'OVRB', 'PREV TARD'];
  ...
  let transpo_alo              = <?php echo json_encode($TRANSPO_ALO); ?>;
  let meal_alo                  = <?php echo json_encode($MEAL_ALO); ?>;
  
  
  
  if (rc_sub == 0) {
      const index = benefitsColumns.indexOf('RC SUB');
      if (index !== -1) {
          benefitsColumns.splice(index, 1);
      }
  }
  if (rc_alo == 0) {
      const index = benefitsColumns.indexOf('RC ALO');
      if (index !== -1) {
          benefitsColumns.splice(index, 1);
      }
  }
  if (ovm_alo == 0) {
      const index = benefitsColumns.indexOf('OVM ALO');
      if (index !== -1) {
          benefitsColumns.splice(index, 1);
      }
  }
  if (transpo_alo == 0) {
      const index = benefitsColumns.indexOf('TRANS ALLO');
      if (index !== -1) {
          benefitsColumns.splice(index, 1);
      }
  }
  if (meal_alo == 0) {
      const index = benefitsColumns.indexOf('MEAL ALLO');
      if (index !== -1) {
          benefitsColumns.splice(index, 1);
      }
  }
  ...
  if (rc_sub === 1 && !benefitsColumns.includes('RC SUB')) {
      benefitsColumns.push('RC SUB');
  }
  if (rc_alo === 1 && !benefitsColumns.includes('RC ALO')) {
      benefitsColumns.push('RC ALO');
  }
  if (ovm_alo === 1 && !benefitsColumns.includes('OVM ALO')) {
      benefitsColumns.push('OVM ALO');
  }
  if (transpo_alo === 1 && !benefitsColumns.includes('TRANS ALLO')) {
      benefitsColumns.push('TRANS ALLO');
  }
  if (meal_alo === 1 && !benefitsColumns.includes('MEAL ALLO')) {
      benefitsColumns.push('MEAL ALLO');
  }
  ...
  var combinedData = employee_list.map(function(empData) {
    ...
    dateHeaders.forEach(function(dateHeader, index) {
      ...
      if (attendanceData) {
        ...
        const raw_time_out          = attendanceData.raw_time_out[index];
        ...
        if(raw_time_out != 0){
          if(raw_time_out >= "21:00" && empData.position.toLowerCase().indexOf("driver".toLowerCase()) !== -1){
            sumOfTransportation_allowance += 60;
          }
          if(raw_time_out >= "22:00" && (empData.position.toLowerCase().indexOf("driver".toLowerCase()) !== -1)==false ){
            sumOfTransportation_allowance += 14;
            // console.log('time:'+raw_time_out + ' position:'+empData.position)
          }
        }
    ...
    let total_meal_allowance = "";

    // data from controller attendance_summary()
    const attendances = zkteco_attendance_data.find((data) => data.id === empData.id);
    
    if (attendances) {
      $assume_dates = attendances.assume_dates;
      $sum_awol = attendances.sum_awol
      $sum_lwop = attendances.sum_lwop
      $sum_present = attendances.sum_present
      $assume_present = attendances.assume_present

      $prev_assume_absent = attendances.prev_assume_absent
      $prev_assume_absent_dates = attendances.prev_assume_absent_dates
      $prev_assume_tardiness = attendances.prev_assume_tardiness;
      ...
      total_meal_allowance = attendances.total_meal_allowance

      $reg_reg = attendances.sum_reg_hours;
      sum_reg_regot = attendances.sum_reg_regot
      sum_reg_nd = attendances.sum_reg_nd
      sum_reg_ndot = attendances.sum_reg_ndot
      ...
      sum_leg_regot = attendances.sum_leg_regot
      sum_leg_nd = attendances.sum_leg_nd
      sum_leg_ndot = attendances.sum_leg_ndot

      $rst_leg_reg = attendances.sum_legrest_hours;
      sum_legrest_regot = attendances.sum_legrest_regot;
      sum_legrest_ndot = attendances.sum_legrest_ndot;
      sum_legrest_nd = attendances.sum_legrest_nd;
      
      $sum_spe_hours = attendances.sum_spe_hours;
      sum_spe_regot = attendances.sum_spe_regot;
      sum_spe_nd = attendances.sum_spe_nd;
      sum_spe_ndot = attendances.sum_spe_ndot;
      
      $sum_sperest_hours = attendances.sum_sperest_hours;
      sum_sperest_regot = attendances.sum_sperest_regot;
      sum_sperest_ndot = attendances.sum_sperest_ndot;
      sum_sperest_nd = attendances.sum_sperest_nd;
    ...
    presentColumns.forEach(function(presentColumn) {
      ...
      if (presentColumn == "ASSUME DAYS") {
        combinedRow[presentColumn] = ($assume_present) ? $assume_present : "";
      }

      if (presentColumn == "ASSUME DATES") {
        combinedRow[presentColumn] = ($assume_dates) ? $assume_dates : "";
      }
    ...
    benefitsColumns.forEach(function(column) {
      ...
      if(column == "MEAL ALLO"){
        combinedRow[column] = total_meal_allowance > 0 ? total_meal_allowance : '';
      }
    })
    ...
    attendanceAbsences.forEach(function(attendanceAbsence) {
      ...
      if (attendanceAbsence == "AWOL") {
       ...
       if (attendanceAbsence == "PREV ASSUME ABSENT") {
          combinedRow[attendanceAbsence] = ($prev_assume_absent) ? $prev_assume_absent : "";
        }
        if (attendanceAbsence == "PREV ASSUME ABSENT DATES") {
          combinedRow[attendanceAbsence] = ($prev_assume_absent_dates) ? $prev_assume_absent_dates : "";
        }
    })
    ...
    attendanceDeductions.forEach(function(attendanceDeduction) {
      ...
      if (attendanceDeduction == "PREV TARD") {
        combinedRow[attendanceDeduction] = ($prev_assume_tardiness) ? $prev_assume_tardiness : "";
      }
    ...
    typeREG.forEach(function(regRow) {
      ...
      if (regRow == "REG OT") {
        combinedRow[regRow] = (sum_reg_regot) ? sum_reg_regot : "";
      }
      if (regRow == "REG ND") {
        combinedRow[regRow] = (sum_reg_nd) ? sum_reg_nd : "";
      }
      if (regRow == "REG NDOT") {
        combinedRow[regRow] = (sum_reg_ndot) ? sum_reg_ndot : "";
      }
    })
    ...
    typeRST.forEach(function(rstRow) {
      if (rstRow == "RST REG") {
        combinedRow[rstRow] = (sum_rest_hours) ? sum_rest_hours : "";
      }
      if (rstRow == "RST OT") {
        combinedRow[rstRow] = (sum_rest_regot) ? sum_rest_regot : "";
      }
      if (rstRow == "RST ND") {
        combinedRow[rstRow] = (sum_rest_nd) ? sum_rest_nd : "";
      }
      if (rstRow == "RST NDOT") {
        combinedRow[rstRow] = (sum_rest_ndot) ? sum_rest_ndot : "";
      }
    })
    ...
    typeLEG.forEach(function(rstRow) {
      ...
      if (rstRow == "LEG OT") {
        combinedRow[rstRow] = (sum_leg_regot) ? sum_leg_regot : "";
      }
      if (rstRow == "LEG ND") {
        combinedRow[rstRow] = (sum_leg_nd) ? sum_leg_nd : "";
      }
      if (rstRow == "LEG NDOT") {
        combinedRow[rstRow] = (sum_leg_ndot) ? sum_leg_ndot : "";
      }
    })
    ...
    typeRST_LEG.forEach(function(rstRow) {
      ...
      if (rstRow == "RST+LEG OT") {
        combinedRow[rstRow] = (sum_legrest_regot) ? sum_legrest_regot : "";
      }
      if (rstRow == "RST+LEG ND") {
        combinedRow[rstRow] = (sum_legrest_nd) ? sum_legrest_nd : "";
      }
      if (rstRow == "RST+LEG NDOT") {
        combinedRow[rstRow] = (sum_legrest_ndot) ? sum_legrest_ndot : "";
      }
    })
    ...
    typeSPE.forEach(function(rstRow) {
      ...
      if (rstRow == "SPE OT") {
        combinedRow[rstRow] = (sum_spe_regot) ? sum_spe_regot : "";
      }
      if (rstRow == "SPE ND") {
        combinedRow[rstRow] = (sum_spe_nd) ? sum_spe_nd : "";
      }
      if (rstRow == "SPE NDOT") {
        combinedRow[rstRow] = (sum_spe_ndot) ? sum_spe_ndot : "";
      }
    })
    ...
    typeRST_SPE.forEach(function(rstRow) {
      ...
      if (rstRow == "RST+SPE OT") {
        combinedRow[rstRow] = (sum_sperest_regot) ? sum_sperest_regot : "";
      }
      if (rstRow == "RST+SPE ND") {
        combinedRow[rstRow] = (sum_sperest_nd) ? sum_sperest_nd : "";
      }
      if (rstRow == "RST+SPE NDOT") {
        combinedRow[rstRow] = (sum_sperest_ndot) ? sum_sperest_ndot : "";
      }
    })
  ...
  let nestedHeaders = [
    ...
    ['ID', selectHeaderLabel, 'Employee ID', 'Employee Name', ...formattedDates, ...presentColumns, ...sliders, ...benefitsColumns, ...attendanceAbsences, ...attendanceDeductions,
      ...typeREG, ...typeRST, ...typeLEG, ...typeRST_LEG, ...typeSPE, ...typeRST_SPE,
      ...benefitsTypeHeaders, ...adjustmentTypeHeaders,
    ],
  ]
  ...
  let col_header_title = function(col, TH) {
    ...
    switch (colName) {
        ...
        case 'OVM ALO':
            ...
            break;
        case 'TRANS ALLO':
            TH.setAttribute('title', 'Transportation Allowance');
            break;
        case 'PAID':
  ...
  // Now, retrieve the column headers after initialization
  let colHeaders = hot.getColHeader();
  const hiddenColumns = {
    columns: [0, colHeaders.indexOf('ASSUME DATES')],  // Hide first column and 'ASSUME DATES'
  };

  // Update the hidden columns in the Handsontable instance
  hot.updateSettings({
  ...
  insert.addEventListener('click', function() {
    ...
    const filteredData = updatedData.filter(row => row && row[1] === true);
    const filteredCombinedData = combinedData.filter(item => item.isSelected === true);
    

    const requestData = {
      updatedData: filteredData, //updatedData
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    $('.deleteAssumeDataBtn').on('click', function(e) {
      e.stopPropagation();
      var assumeDataId = this.getAttribute('data-assume-id');
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          fetch("<?php echo base_url('attendances/delete_assume_dates'); ?>" + `/${assumeDataId}`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
