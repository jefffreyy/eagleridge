...
<body>
    ...
    <div class="container-fluid p-4">
      <div class="row pt-1">
        ...
        <div class="col-md-6 button-title">
          <button href="#" class=" btn technos-button-green shadow-none rounded" onclick="exportToExcel()"
          id="btn_export"><img style="margin-bottom: 5px;" src="<?= base_url('assets_system/icons/file-export-solid.svg') ?>" alt="">
      ...
      <div class="row mb-4">
        ...
        <div class="d-flex row filter-container">
      ...
      <div class="card border-0 p-0 m-0">
        ...
        <div class="card border-0 p-0 m-0">
          <div class="p-2 d-flex justify-content-between align-items-end">
            <div class="" <?php echo (true ? "" : "hidden") ?>>
              ...
              <div class="d-flex">
                ...
                <button id="btnFilter" class="btn technos-button-green shadow-none rounded ml-1" onclick="toggleFilter()">Advance Filter</button>
                <a href="<?= base_url('employees/approval_routes') ?>" id="btn_clear_filter" class="btn technos-button-green mx-1">Clear</a>
              </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      function filter_data(page_row) {
        ...
        window.location = base_url + "employees/approval_routes" + page_row + "&branch=" + branch +
  ...
  <script>
    var url = '<?= base_url() ?>';
    var data = <?php echo json_encode($DISP_APPROVERS); ?>;
    ...
    nestedHeaders= [
          ...
          // ['', '',
          //   // {label: 'Employee',colspan: 2},
          //   {label: 'Approver 1-A',colspan: 2},
          //   {label: 'Approver 1-B',colspan: 2},
          //   {label: 'Approver 2-A',colspan: 2},
          //   {label: 'Approver 2-B',colspan: 2},
          //   {label: 'Approver 3-A',colspan: 2},
          //   {label: 'Approver 3-B',colspan: 2},
          // ],
          ['id', 'Employee',
            'Approver 1A', 'Approver 1B',
            'Approver 2A', 'Approver 2B',
            'Approver 3A', 'Approver 3B',
          ],
        ]
    function initializeHandsontable(data) {
      ...
      hot = new Handsontable(container, {
        ...
        //       if (colIndex === 3) { //  ID column
        //         var selectedID = data[rowIndex].ID;
        //         alert(data[rowIndex].ID)
        //         // var name = getNameForID(selectedID); 
        //         // data[rowIndex].Name = name;
        //         // hot.setDataAtCell(rowIndex, 1, name); 
        //       }
        //     });
        //   }
        // }
    ...
    update_data.addEventListener('click', function() {
      ...
      updatedData.map((item) => {
        if (item[2]) {
          const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[2]);
          if (check) {
            item[2] = check.cmid;
          } else {
            invalidId = true
        ...
        if (item[6]) {
          const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[6]);
          if (check) {
            item[6] = check.cmid;
          } else {
            invalidId = true
        ...
        if (item[7]) {
          const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[7]);
          if (check) {
            item[7] = check.cmid;
          } else {
            invalidId = true
      ...
      const apiUrl = url + 'employees/update_approval_routes';
      const data = {
        updatedData,
      };
      // console.log('data', data);
