        ...
        <div class="card border-0 p-0 m-0">
          <div class="p-2">
            ...
            <div class="float-right mb-2 d-none d-lg-flex" id="published_view_details">
              ...
              <a <?= ($PAYROLL_PAYSLIP_PUBLISHED_LIST) ? "" : "hidden" ?> href="<?= base_url() . 'attendances/published_view_details?period=' . $PERIOD . '&tab=' . $TAB; ?>" class="btn btn-primary shadow-none rounded"><img style="height: 15px; width: 15px;" class="mb-1" src="<?= base_url('assets_system/icons/eye-solid_new.svg') ?>" alt="">&nbsp;Published View Details</a>
                        ...
                        <?php if ($PAYROLL_PAYSLIP_PUBLISHED_LIST) {
                          foreach ($PAYROLL_PAYSLIP_PUBLISHED_LIST as $PUBLISHED_LIST) { ?>
                            <tr>
                              ...
                              <td class="text-left">
                                ₱<?= number_format($PUBLISHED_LIST->NET_INCOME, 2); ?>
  ...
  <script>
    $(document).ready(function() {
          ...
          Promise.all(promises)
            .then((results) => {
              ...
              results.forEach((res, index) => {
                ...
                // Description
                if (record.VAC_USED) {
                  doc.text(xcoor, ycoor += increment_by, record.VAC_USED, {
                    align: 'right'
                  });
                }

                if (record.SICK_USED) {
                  doc.text(xcoor, ycoor += increment_by, record.SICK_USED, {
                    align: 'right'
                  });
                }
                ...
                increment_by = 2.5;
                if (record.VAC_BAL) {
                  doc.text(xcoor, ycoor += increment_by, record.VAC_BAL, {
                    align: 'right'
                  });
                }

                if (record.SICK_BAL) {
                  doc.text(xcoor, ycoor += increment_by, record.SICK_BAL, {
                    align: 'right'
                  });
                }
      ...
      function filter_data(tabId) {
        ...
        window.location = base_url + "attendances/published?period=" + cut_off;
      }
