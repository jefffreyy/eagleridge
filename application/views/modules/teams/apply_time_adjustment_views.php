    ...
    <div class="container-fluid p-4">
      <div class="row pt-1">
        <div class="col-md-6">
          <h1 class="page-title d-flex align-items-center"><a onclick="afterRenderFunction()" href="<?= base_url('teams') ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            </a>&nbsp;Apply Time Adjustments<h1>
        </div>
        <?php if ($DISP_VIEW_REQUIREDAPPROVERS == 1) { ?>
          <div class="col-md-6 button-title">
            <a href="<?= base_url('teams/request_times_adjustment') ?>" class=" btn btn-primary shadow-none rounded d-flex align-items-center"><img src="<?= base_url('assets_system/icons/plus-solid.svg') ?>" alt="">
              &nbsp;Add Request</a>
        ...
        <?php if ($DISP_VIEW_REQUIREDAPPROVERS == 0) { ?>
          <div class="col-md-6 button-title">
            ...
            <a href="<?= base_url('teams/request_times_adjustment') ?>" class=" btn technos-button-green shadow-none rounded"><img src="<?= base_url('assets_system/icons/file-export-solid.svg') ?>" alt="">&nbsp;Add Request</a>
          </div>
        <?php } ?>
      </div>
      <hr>
      <div class="filter-container <?= $filter ? 'visible' : '' ?> ">
        <div class="row mb-4">
          ...
          <!-- <div class="col-md-2">
            ...
            <a href="<?= base_url('teams/apply_time_adjustments') ?>" id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
          </div> -->
      ...
      <div class="p-2">
        <div class="card-header p-0">
          <div class="">
            <div class="d-flex row align-items-end mb-1">
              <div class="col-12 col-lg-6 d-flex m-1 ">
                <div class="row justify-content-center justify-content-lg-start">

                  <p class="col-11 mb-1 text-secondary">Employee</p>

                  <select class="col-12 col-md-4 select-employee form-control" id="search_data" style="">
                    <option value=''>All</option>
                  ...
                  <a href="<?= base_url('teams/apply_time_adjustments') ?>" id="btn_clear_filter" class="mt-2 mt-lg-0 btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="">&nbsp;Clear</a>
                </div>

              </div>

              <div class="col-12 col-lg-6 col-xl-5 ml-auto my-2 my-lg-0 row align-items-center d-lg-flex d-none">
                <div class="col-12 col-lg-10 d-flex justify-content-center align-items-end ">
                  <div class="d-flex align-items-center row">
                    <div class="d-inline d-flex col-12 col-lg-6 col-xl-5 justify-content-lg-end justify-content-center align-items-center">
                ...
                <div class="col-sm-3 col-md-2 col-lg-1 d-none d-lg-flex align-items-center justify-content-center justify-content-lg-end mr-lg-0 ml-auto">
      ...
      <?php if ($DISP_VIEW_REQUIREDAPPROVERS == 1) { ?>
        <div class="table-responsive">
          <table class="table table-hover table-bordered m-0" id="table_main" style="width:100%">
            <thead>
              ...
              <th class="text-left" style="width:15%">TIME IN</th>
              <th class="text-left" style="width:15%">TIME OUT</th>
              ...
              <?php if ($TABLE_DATA) {  ?>
                <?php foreach ($TABLE_DATA as $row_data) { ?>
                  <tr class="hover" data-id="<?= $row_data->id ?>" data-toggle="modal" data-target="#modal_approval">
                    ...
                    <td class="text-left"><?= $row_data->date_adjustment ?></td>
                    <td class="text-left"><?= $row_data->time_in_1 ?></td>
                    <td class="text-left"><?= $row_data->time_out_1 ?></td>
        ...
        <div class="col-sm-3 col-md-2 col-lg-1 d-flex d-lg-none align-items-center justify-content-center justify-content-lg-end mr-lg-0 mr-2">
          <p class="p-0 m-0 d-inline" style="color: gray">&nbsp;&nbsp;Rows:&nbsp;</p>
          <select id="row_dropdown" class="custom-select" style="width: auto;">
            <?php
            foreach ($C_ROW_DISPLAY as $C_ROW_DISPLAY_ROW) { ?>
        ...
        update_data.addEventListener('click', function() {
          ...
          for (var i = 0; i < changes.length; i++) {
            ...
            if (changes[i].employee) {
              ...
              if (isValidDateFormat(changes[i].date_adjustment)) {
                const parts = changes[i].date_adjustment.split('/');
              ...
              // if (!(!isNaN(changes[i].hours) && Number(changes[i].hours) > 0)) {
              //   changesInvalid.push(changes[i].c_id);  continue;
              // }
              console.log('valid');
              console.log(valid)
              if (valid) {
                changesFinal.push(temp);
                changesSecondary.push(changes[i].c_id);
              }
            }
          }
          ...
          const changesLeavesInvalidString =
            changesInvalid.length > 0 ? `Time Adjustment${changesInvalid.length > 1? 's': ''} ${changesInvalid.join(', ')} ${changesInvalid.length > 1? 'have': 'has'} invalid input and will not be updated` :
            '';
          ...
          var url = '<?= base_url() ?>';
          const apiUrl = url + 'teams/update_time_adjustment_direct';
          console.log('changesFinal', changesFinal);
          fetch(apiUrl, {
              method: 'POST',
              headers: {
  ...
  <script>
    $(document).ready(function() {
      ...
      $('a.paginate').on('click', function() {
        ...
        window.location.href = "<?= base_url('teams/apply_time_adjustments') ?>" + $(this).attr('href') + filter_url;
        return false;
      })
      ...
      function reloadPage() {
        ...
        window.location.href = "<?= base_url('teams/apply_time_adjustments') ?>" + "?status=" + status + "&page=" + page + "&row=" + row + '&search=' + search + filter_url;
      }
    })
  </script>
  <?php if ($DISP_VIEW_REQUIREDAPPROVERS == 1) { ?>
