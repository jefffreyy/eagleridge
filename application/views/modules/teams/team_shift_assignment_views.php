...
$yearmonth = $YEARMONTH;
$yearmonth_from = $YEARMONTH_FROM;
$yearmonth_to = $YEARMONTH_TO;
if (isset($_GET['row'])) {
  $row = $_GET['row'];
} else {
  $row = 25;
...
<body>
  <div class="content-wrapper">
    <div class="container-fluid p-4">
      <div class="row">
        <div class="col-md-6">
          <h1 class="page-title d-flex align-items-center"><a onclick="afterRenderFunction()" href="<?= base_url() . 'teams/myshifts'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            </a>&nbsp;Shift Assignment</h1>
        </div>
        <div class="col-md-6 button-title d-flex justify-content-end">
          <!-- <button type="button" class="btn btn-primary ml-1" id="printButton"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/print-duotone_sm.svg') ?>" alt="" />&nbsp;Submit</button> 
      ...
      <div class="row mb-4">
        <div class="col-md-3">
          <label for="search_employees">Cut-Off Period</label>
          <select class="form-control cut_off_period">
            <?php
            foreach ($DISP_CUTOFF_PERIOD as $CUTOFF_PERIOD_ROW) { ?>
              <option value=<?= $CUTOFF_PERIOD_ROW->id ?> <?= ($PERIOD == $CUTOFF_PERIOD_ROW->id) ? 'selected' : '' ?>> <?= $CUTOFF_PERIOD_ROW->name ?> </option>
            <?php } ?>
          </select>
        </div>
      </div>
        ...
        <div class='p-2'>
          <div class="row mt-3 py-1 justify-content-between">
            <div class="col-12 col-lg-3 d-flex justify-content-lg-start justify-content-center">
              ...
              <!-- <button class="mr-1 btn btn-primary" id="btn-update"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="" />
                &nbsp;Update
              </button> -->
              <button class="btn btn-success" id="btn-submit"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="" />
                &nbsp;Submit for Approval
              </button>
            </div>
            <!-- <div class='col-12 col-lg-6 d-lg-flex d-none justify-content-lg-end'>
              <div class="d-inline d-flex col-12 col-lg-6 justify-content-lg-end justify-content-center align-items-center">
                <p class="p-0 m-0 text-center nowrap" style="color: gray">Showing <?= $low_limit ?> to <?= $high_limit ?> of <?= $C_DATA_COUNT ?> entries&nbsp;</p>
              </div>
              <div class="d-lg-inline col-12 col-lg-4 ">
                <ul class="ml-0 ml-lg-4 pagination m-0 p-0 mx-auto ">
              ...
              <div class="col-sm-3 col-md-2 col-lg-3 d-flex align-items-center  justify-content-end mr-lg-0 ml-auto">
        ...
        <div class="row">
          <div class="col">
            <div class="pt-2">
              ...
              <div class='col-sm-3 col-lg-5 d-lg-none justify-content-lg-end'>
                <div class="d-inline d-flex col-12 col-lg-6 justify-content-lg-end justify-content-center align-items-center">
                ...
                <div class="col-sm-3 col-md-2 col-lg-3  d-flex align-items-center justify-content-center justify-content-lg-start mr-lg-0 mr-2">
                  <p class="p-0 m-0 d-inline" style="color: gray">&nbsp;&nbsp;Rows:&nbsp;</p>
                  <select id="row_dropdown" class="custom-select m-0" style="width: auto;">
                    <?php
                    foreach ($C_ROW_DISPLAY as $C_ROW_DISPLAY_ROW) { ?>
  ...
  <div class="modal fade  class_modal_update_list" id="modela_update" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <form action="<?= base_url() . 'teams/update_shift'; ?>" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
          <div class="modal-body">
            <div class="row">
              <div class="col-md-12">
            ...
            <div class="row mb-4">
              <div class="col-md-6">
                ...
                <input class="custom-select " type="date" id="" name="DATE_FROM" value="<?= $yearmonth_from ?>" readonly style="background-color: #eeeeee">
              </div>
              <div class="col-md-6">
                <label for="search_employees">Period To</label>
                <input class="custom-select " type="date" id="" name="DATE_TO" value="<?= $yearmonth_to ?>" readonly style="background-color: #eeeeee">
              </div>
            </div>
            <div class="row">
              <div class="col-md-12">
  ...
  <script>
    ...
    var shiftAssignedApproval = <?php echo json_encode($SHIFT_ASSIGNED_APPROVAL); ?>;
    ...
    var combinedData = dispEmpList.map(function(empData) {
      ...
      dateHeaders.forEach(function(dateHeader) {
        ...
        const shiftDataItem = shiftDataDateRange.find(item => item.date === dateHeader && item.empl_id === empData.empl_id);
        ...
        if (shiftDataItem) {
          ...
          customColorStyleRenderer = function(instance, td, row, col, prop, value, cellProperties) {
            ...
            const shiftDataItem = shiftDataDateRange.find(
              item => item.date === dateHeader && item.empl_id === empData.empl_id
            );
            ...
            if (shiftDataItem) {
              ...
              // if(shiftDataItem.lock_shift == 1){

              // }
            }

            shiftAssignedApproval.forEach(function(data) {
              // console.log(data)
              if (data.empl_id == empData.empl_id && (data.status == "Pending" || data.status == "Approved" || data.status == "Checked")) {
                cellProperties.readOnly = true;
                td.style.backgroundColor = '#ffffc2';
              }
            })
    ...
    hot = new Handsontable(container, {
      ...
      colHeaders: ['Id', 'Employee Id', 'Name', ...colDateHeaders],
      rowHeaders: true,
      ...
      afterChange: function(changes, source) {
        if (['edit', 'paste', 'Autofill.fill'].includes(source)) {
          autoUpdateData();
        }
        if (source !== 'loadData') {
          autoUpdateData();
      ...
      fetch(url + 'teams/update_data', {
        ...
        .then(result => {
          ...
          if (result.success_message) {
            ...
            // location.reload();
            // hot.loadData(result.updatedData);

            // Ensure result.updatedData is an array of objects or array of arrays
            if (Array.isArray(result.updatedData)) {
              hot.loadData(result.updatedData);
            } else {
              console.error('Invalid data format received from server:', result.updatedData);
            }
          }
    ...
    var cutOffPeriodValue = document.querySelector('.cut_off_period').value;
    var shift_submit = document.getElementById('btn-submit');

    shift_submit.addEventListener('click', function() {
      updatedData = hot.getData();
      // Filter out rows that contain any empty cell
      var filteredData = updatedData.filter(row => row.every(cell => cell !== "" && cell !== null && cell !== undefined));

      var filteredEmployeeData = employeeData.filter(empRow =>
        filteredData.some(filteredRow => filteredRow[1] === empRow[1])
      );

      const requestData = {
        updatedData: filteredData,
        employeeData: filteredEmployeeData,
        cutOffPeriodValue: cutOffPeriodValue,
      };

      fetch(url + 'teams/update_shift_assignment', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  <script>
    $(document).ready(function() {
      ...
      $('select.cut_off_period').on('change', function() {
        afterRenderFunction();
        window.location.href = "<?= base_url('teams/shift_assignment?period=') ?>" + $(this).val();
      });
    })
  </script>
  <script>
