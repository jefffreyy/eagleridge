        ...
        <div class="p-3">
            <div class="flex-fill">
                <div class="row pr-3 mb-3">
                    <div class="col">
                    <h1 class="page-title"><a href="<?= base_url('teams/apply_overtimes') ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            </a>&nbsp;Add Overtimes<h1>
                        <!-- <h1 class="page-title"><a href="<?= base_url('teams/apply_overtimes')?>"><i class="fa-duotone fa-circle-left"></i></a>&nbsp;Add Overtimes</h1> -->
                    </div>
    ...
    <script>
        ...
        function initializeHandsontable(data) {
            ...
            //   columns: [0],
                ...
                {data:'date_ot',readOnly: false,type: 'date',
        ...
        insert.addEventListener('click', function() {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].date_ot) {
                    if (isValidDateFormat(data[i].date_ot)) {
                    const parts = data[i].date_ot.split('/');
                ...
                if (data[i].hours) {
                    if (!isNaN(data[i].hours) && Number(data[i].hours) >= 0.5 && Number(data[i].hours) <= 24) {
                } else {
                        requestDurationInvalid = `Overtime Hours is Invalid.`;
                        valid = false;
                    }
                } else {
                    requestDurationInvalid = `Overtime Hours is Empty.`;
                    valid = false;
                }
                if (valid) {
            ...
            var url          = '<?= base_url() ?>'; 
            fetch(url + 'overtimes/insert_overtimes_direct', {
                method: 'POST',
                headers: {
