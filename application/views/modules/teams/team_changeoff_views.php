    ...
    <div class="container-fluid p-4">
      <div class="row pt-1">
        ...
        <div class="col-md-6 button-title">
          ...
          <a onclick="afterRenderFunction()" href="<?= base_url('teams/request_team_change_off') ?>" class=" btn btn-primary shadow-none rounded d-flex align-items-center"><img src="<?= base_url('assets_system/icons/plus-solid.svg') ?>" alt="">
            &nbsp;Add Request</a>
          ...
          <div class="table-responsive">
            ...
            <table class="table table-bordered m-0" id="table_main" style="width:100%">
              <thead>
                ...
                <th style='width:7%;'>DATE FROM</th>
                <th style='width:10%;'>CURRENT SHIFT FROM</th>
                <th style='width:10%;'>REQUEST SHIFT FROM</th>

                <th style='width:7%;'>DATE TO</th>
                <th style='width:10%;'>CURRENT SHIFT TO</th>
                <th style='width:10%;'>REQUEST SHIFT TO</th>
              ...
              <tbody id="tbl_application_container">
                <?php if ($TABLE_DATA) {  ?>
                  <?php foreach ($TABLE_DATA as $row_data) {
                    $formatted_date               = ($row_data->date_shift) ? date(($DATE_FORMAT) ? $DATE_FORMAT : "d/m/Y", strtotime($row_data->date_shift)) : ""; 
                    $formatted_date_to            = ($row_data->date_shift_to) ? date(($DATE_FORMAT) ? $DATE_FORMAT : "d/m/Y", strtotime($row_data->date_shift_to)) : ""; 
                    $formatted_requested_date     = date(($DATE_FORMAT) ? $DATE_FORMAT : "d/m/Y", strtotime($row_data->create_date)); ?>
                    <tr>
                      <!-- <td><?= $id_prefix . str_pad($row_data->id, 5, '0', STR_PAD_LEFT) ?></td> -->
                      <td><?= $row_data->employee ?></td>
                      <td><?= $formatted_requested_date ?></td>
                      <td><?= $formatted_date ?></td>
                      <td><?= $row_data->current_shift ?></td>
                      <td><?= $row_data->request_shift ?></td>

                      <td><?= $formatted_date_to ?></td>
                      <td><?= $row_data->current_shift_to ?></td>
                      <td><?= $row_data->request_shift_to ?></td>
                      ...
                      <td class="text-center">
                        ...
                        <?php } elseif ($row_data->status == "Withdrawn" || $row_data->status == "Withdrawed" || $row_data->status == "Cancelled") { ?>
                      ...
                      <td class="text-center">
                        ...
                        <?php
                          ...
                          $disabled = "";
                          if ($row_data->status == 'Approved' || $row_data->status == 'Withdrawn' || $row_data->status == 'Withdrawed' || $row_data->status =='Rejected') {
                            $disabled = "disabled";
                          }

                        ?>
                        <a href="<?= base_url('teams/changeoff_withdraw/' . $row_data->id) ?>" class="btn btn-default withdraw_req <?=$disabled?>"  >Withdraw</a>
  ...
  <script>
    $(document).ready(function() {
      ...
      function reloadPage() {
        ...
        var page = "<?= $PAGE ?>";
        window.location.href = "<?= base_url('teams/team_change_off') ?>" + "?status=" + status + "&page=" + page + "&row=" + row;
      }
    })
  </script>
