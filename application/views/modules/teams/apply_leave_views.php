      ...
      <div class="row pt-1">
        <div class="col-md-6">
          <h1 class="page-title"><a onclick="afterRenderFunction()" href="<?= base_url('teams') ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            </a>&nbsp;Apply Leave Requests<h1>
        </div>
        <?php if ($DISP_VIEW_REQUIREDAPPROVERS == 1) { ?>
          <div class="col-md-6 button-title">

            <a onclick="afterRenderFunction()" href="<?= base_url('teams/request_leave') ?>" class=" btn btn-primary shadow-none rounded d-flex align-items-center"><img src="<?= base_url('assets_system/icons/plus-solid.svg') ?>" alt="">
              &nbsp;Add Request</a>
                ...
                <div class="d-flex row align-items-end">
                  <div class="'col-12 col-xl-6 d-flex ml-1">
                    <div class="row justify-content-center justify-content-lg-start">
                      ...
                      <a href="<?= base_url('teams/apply_leaves') ?>" id="btn_clear_filter" class="mt-2 mt-lg-0 btn btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="">&nbsp;Clear</a>
                    </div>
                  </div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered m-0" id="table_main" style="width:100%">
                <thead>
                  ...
                  <th style='width:15%;text-align: left;'>EMPLOYEE</th>
                ...
                <tbody id="tbl_application_container">
                  <?php if ($LEAVES) {  ?>
                    <?php foreach ($LEAVES as $leave) { ?>
                      <tr data-leave_id="<?= $leave->id ?>" data-toggle="modal" data-target="#modal_approval">
                        <td class="text-center" id="select_item" hidden>
                          <input type="checkbox" name="brand" class="check_single" row_id="56">
                        </td>
                        <td class="text-left" style="width:5%"><?= $id_prefix . str_pad($leave->id, 5, '0', STR_PAD_LEFT) ?></td>
                        <!-- <td class="text-center hover td-directs text-primary " style="width:10%" data-empl_id="<?= $leave->assigned_table_id ?>">
                          <?= $leave->assigned_by ?>
                        </td> -->
                        <td class="text-left hover td-directs text-primary " data-empl_id="<?= $leave->employee_table_id ?>">
                          <?= $leave->employee ?>
                        </td>
                        <!-- <td class="text-left"><?= $leave->leave_date ?></td> -->
                        <td class="text-left"> <?= date(($DATE_FORMAT) ? $DATE_FORMAT: "d/m/Y", strtotime($leave->leave_date)) ?></td>
                        <td class="text-left" style="width:15%"><?= $leave->type ?></td>
                        ...
                        <td class="text-center">
                          ...
                          <?php if ($leave->status == 'Pending 1') { ?>
                            <!--<a class = "select_edit_row p-2 edit_data_id"  href="<?= base_url('leaves/edit_leaves/' . $leave->id) ?>"  style ="color: gray; cursor: pointer; !important" row_id="56">-->
  ...
  <script>
    $(document).ready(function() {
      ...
      $('a.paginate').on('click', function() {
        ...
        window.location.href = "<?= base_url('teams/apply_leaves') ?>" + $(this).attr('href') + filter_url;
        return false;
      })
      ...
      function reloadPage() {
        ...
        window.location.href = "<?= base_url('teams/apply_leaves') ?>" + "?status=" + status + "&page=" + page + "&row=" + row + '&search=' + search + filter_url;
      }
