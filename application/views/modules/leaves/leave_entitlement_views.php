...
<body>
  ...
  <div class="content-wrapper">
    <div class="container-fluid p-4">
      ...
      <div class="row  pt-1">
        <!-- Title Text -->
        <div class="col-md-6">
          <h1 class="page-title d-flex align-items-center"><a href="<?= base_url() . 'leaves'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            </a>&nbsp;Leave Entitlement
        ...
        <div class="col-md-6 button-title d-flex justify-content-end">
          <button class="mr-1 btn btn-primary" id="btn-update"><img style="width: 18px; height: 18px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="" />
            Update</button>
          <a href="<?= base_url() . 'leaves/leave_parameter'; ?>" id="" class="btn btn-primary shadow-none"><img style="width: 18px; height: 18px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/gears-duotone_xs.svg') ?>" alt="" />
            </i>&nbsp;Settings</a>
        ...
        <div class="row mb-4">
          <div class="col-md-2">
            ...
            <select name="filter_year" id="filter_year" class="form-control">
              ...
              ?>
                  <!-- <option value="<?= $DISP_YEARS_ROW->id ?>" <?= $year == $DISP_YEARS_ROW->id ? 'selected' : ($CURRENT_YEAR == $DISP_YEARS_ROW->name ? 'selected' : '') ?>> -->
                  <option value="<?= $DISP_YEARS_ROW->id ?>" <?= $year == $DISP_YEARS_ROW->id ? 'selected' :  '' ?>>
                    <?= $DISP_YEARS_ROW->name ?>
                  </option>
              <?php
                }
        ...
        <div class="">
          ...
          <div class="card border-0 p-0 m-0">
            <div class=" <?= $SYSTEM_LEAVE_SETTING == '1' ? 'd-none' : ''; ?>">
              <!-- <a href="#" class=" btn technos-button-gray shadow-none rounded" id="update" data-toggle="modal" data-target="#modela_update"><img style="height: 1rem; width: 1rem; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-check-solid_mark.svg') ?>" alt="">
                &nbsp;Bulk Entitlement Assign</a> -->

            </div>
            <div class="p-2 ">
              <div class="">
                <div class="row">
                  <div class="col-md-12 col-lg-12 mx-1">
                    <div class="d-flex row align-items-center">
                      ...
                      <div class="row d-flex justify-content-center justify-content-lg-start col-12 col-md-3 col-lg-6 ">
                        ...
                        <button id="btnFilter" class="my-1 my-lg-0 mx-1 btn btn-primary shadow-none rounded" onclick="toggleFilter()"><img src="<?= base_url('assets_system/icons/advance_filter.svg') ?>" style="margin-bottom: 2px;" alt="" />&nbsp;Advance Filter</button>
                        <a href=<?= base_url() . "leaves/entitlements" ?> id="btn_clear_filter" class="my-1 my-lg-0 btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" style="margin-bottom: 2px;" alt="" />&nbsp;Clear</a>

                      </div>

                      <div class="d-none d-lg-flex col-sm-7 col-md-6 col-lg-6 justify-content-between align-items-end my-lg-0 my-2">
                        <div class="col-12 col-lg-6 d-flex justify-content-lg-end align-items-end">
                          <div class="d-flex align-items-center row">
                            <div class="d-inline col-12 col-lg-6">
                              <p class="p-0 m-0 mx-auto text-center text-lg-nowrap" style="color: gray">Showing <?= $low_limit ?> to <?= $high_limit ?> of <?= $C_DATA_COUNT ?> entries&nbsp;</p>
                        ...
                        <div class="col-12 col-md-2 col-lg-3 d-none d-lg-flex justify-content-center align-items-center">
            ...
            <!-- <div class="" id="auto_table_data" <?= $SYSTEM_LEAVE_SETTING == '1' ? '' : 'hidden'; ?>></div> -->
            <div class="" id="auto_table_data">
              ...
              <table class="table table-bordered m-0" id="TableToExport" style="width:100%">
                <thead>
                  ...
                  <?php foreach ($DISP_LEAVE_TYPES as $DISP_LEAVE_TYPES_ROW) { ?>
                    <th class="text-center std"><?= strtoupper($DISP_LEAVE_TYPES_ROW->name) ?>
                    </th>
                  <?php } ?>
                </thead>
                  ...
                  ?>
                      <tr>
                        <td class="text-center" id="select_item">
                          <input type="checkbox" name="brand" class="check_single" empl_name="<?= $name
                        ...
                        foreach ($DISP_LEAVE_TYPES as $DISP_LEAVE_TYPES_ROW) {
                          $entitlement_value = 0;
                          $leave_type = $DISP_LEAVE_TYPES_ROW->name;
                          if ($DISP_ENTITLEMENT) {
                        ...
                        ?>
                          ...
                          <td style="padding: 1px !important;">
                            <form class="bg-transparent">
                              ...
                              <input type="hidden" class="type_data" name="type_data" value="<?= $leave_type ?>">
  ...
  <?php $this->session->unset_userdata('success_copy_shift');
  } ?>
  ...
  <script>
    ...
    let colHeaders = [];
    let columns = [];
    var SYSTEM_LEAVE_SETTING = <?php echo json_encode($SYSTEM_LEAVE_SETTING); ?>;

    // console.log('empl_list', empl_list);
    // console.log('SYSTEM_LEAVE_SETTING', SYSTEM_LEAVE_SETTING);
    if (SYSTEM_LEAVE_SETTING == 1) {
      // console.log('SYSTEM_LEAVE_SETTING', true)
      colHeaders = ["Id", "Employee ID", "Name", "Date Hired", "Month Count", ];
      columns = [{
          data: 'id',
        }, {
          data: 'col_empl_cmid',
      ...
      if (empl_list && Array.isArray(empl_list) && empl_list.length > 0) {
        data = empl_list;
      } else {
        data = [{
          "id": '',
      ...
      initializedHandson(data);
    } else if (SYSTEM_LEAVE_SETTING == 0) {
      // console.log('SYSTEM_LEAVE_SETTING', false)
      var DISP_LEAVE_TYPES = <?php echo json_encode($DISP_LEAVE_TYPES); ?>;
      var DISP_EMP_LIST = <?php echo json_encode($DISP_EMP_LIST); ?>;
      var DISP_ENTITLEMENT = <?php echo json_encode($DISP_ENTITLEMENT); ?>;
      var YEAR_INITIAL = <?php echo json_encode($YEAR_INITIAL); ?>;
      var DISP_YEARS = <?php echo json_encode($DISP_YEARS); ?>;
      var year = <?php echo json_encode($year); ?>;
      colHeaders = DISP_LEAVE_TYPES.map(obj => obj.name);
      colHeaders = ['ID', 'Employee', ...colHeaders];
      // console.log('DISP_LEAVE_TYPES', DISP_LEAVE_TYPES);
      // console.log('DISP_ENTITLEMENT', DISP_ENTITLEMENT);
      // console.log('DISP_EMP_LIST', DISP_EMP_LIST);
      // console.log('YEAR_INITIAL', YEAR_INITIAL);
      // console.log('DISP_YEARS', DISP_YEARS);
      // console.log('colHeaders', colHeaders);
      ...
      data = DISP_EMP_LIST.map(item => {
        let name = '';
        if (item.col_empl_cmid) name += item.col_empl_cmid + '-';
        if (item.col_last_name) name += item.col_last_name + ' ';
        if (item.col_suffix) name += item.col_suffix;
        if (item.col_frst_name) name += ', ' + item.col_frst_name + ' ';
        if (item.col_midl_name) name += item.col_midl_name[0] + '.';
        let employee = {
          empl_id: item.id,
          name,
        }
        ...
        for (let index = 0; index < colHeaders.length; index++) {
          ...
          if (!columnsDone && index > 1) {
            // columns = [...columns, {data: header.replace(/ /g, '_'),type: 'numeric',format: '0', readOnly:false,width:90}];
            columns = [...columns, {
              data: header,
              type: 'numeric',
              format: '0',
          ...
          for (let index2 = 0; index2 < DISP_ENTITLEMENT.length; index2++) {
            // let done = false;
            const filter = ['ID', 'Employee']
            if (employee.empl_id == DISP_ENTITLEMENT[index2].empl_id && DISP_ENTITLEMENT[index2].type == header) {
              //   employee[DISP_ENTITLEMENT[index2].type.replace(/ /g, '_')] = DISP_ENTITLEMENT[index2].value;
              // }else if(!(employee.hasOwnProperty(header.replace(/ /g, '_'))) && !(filter.includes(header))){
              <?php if ($isLeaveHours == 1) { ?>
                employee[DISP_ENTITLEMENT[index2].type] = DISP_ENTITLEMENT[index2].value;
              <?php } ?>

              <?php if ($isLeaveHours == 0) { ?>
                let dividedValue;
                // Check if value is less than 8
                if (DISP_ENTITLEMENT[index2].value < 8) {
                  dividedValue = DISP_ENTITLEMENT[index2].value >= 4 ? 0.5 : 0;
                } else {
                  dividedValue = Math.floor(DISP_ENTITLEMENT[index2].value / 8);
                  if (DISP_ENTITLEMENT[index2].value % 8 >= 4) {
                    dividedValue += 0.5;
                  }
                }
                employee[DISP_ENTITLEMENT[index2].type] = dividedValue;
              <?php } ?>


            } else if (!(employee.hasOwnProperty(header)) && !(filter.includes(header))) {
              // console.log('has own propert negate')
              // console.log('employee', deepCopy(employee))
              // console.log(`DISP_ENTITLEMENT[${index}]`, DISP_ENTITLEMENT[index])
              employee[header] = 0;
              // employee[header.replace(/ /g, '_')] = 0;
            }
    ...
    function initializedHandson(data) {
      var screenHeight = window.innerHeight;
      var elementOffset = container.getBoundingClientRect().top;
      var remainingHeight = screenHeight - elementOffset - 20;
    ...
    update_data.style.display = <?= $SYSTEM_LEAVE_SETTING == '1' ? "'none'" : "'inline-block'"; ?>;
    update_data.addEventListener('click', function() {
      ...
      const apiUrl = url + 'leaves/update_entitlement_api';
      // console.log('data', data);
      let changes = [];
      let keysArray1 = Object.keys(data[0]);
      keysArray1 = keysArray1.filter(item => item !== "name" && item !== "empl_id");
      // console.log('keysArray1', keysArray1);

      function areKeyValuesEqual(obj1, obj2, key) {
        return obj1[key] == obj2[key];
      }
      for (let i = 0; i < data.length; i++) {
        empl_id = data[i].empl_id;
        for (const key of keysArray1) {
          if (!areKeyValuesEqual(data[i], dataClone[i], key)) {
            // console.log(`The ${key} key-value pair is equal.`);
            changes.push({
              empl_id,

              <?php if ($isLeaveHours == 1) { ?>[key]: data[i][key]
              <?php } ?>

              <?php if ($isLeaveHours == 0) { ?>[key]: data[i][key] * 8
              <?php } ?>
            })
          }
        }
      }
      const yearId = $("#filter_year").find(":selected").val();
...
// console.log(optionValue)
      ...
      $('.entitlement_val').on('change', function() {
        ...
        let year = $("#filter_year").find(":selected").val();
        window.location = base_url + "leaves/process_assigning/" + employee + "/" + entitlement_val + "/" + year + "?type=" + type;
