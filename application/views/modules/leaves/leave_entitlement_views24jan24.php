...
<body>
      ...
      <div class="card border-0 p-0 m-0">
        <div class="">
          ...
          <div class="card border-0 p-0 m-0">
            ...
            <div class="p-2 d-flex justify-content-between align-items-end">
              ...
              <div class="my-1">
                ...
                <div class="d-flex">
                  ...
                  <button id="btnFilter" class="ml-1 btn technos-button-green shadow-none rounded" onclick="toggleFilter()"><img src="<?= base_url('assets_system/icons/advance_filter.svg')?>" style="margin-bottom: 1px" alt=""/>&nbsp;Advance Filter</button>
                  <a href=<?= base_url() . "leaves/entitlements" ?> id="btn_clear_filter" class="btn technos-button-green mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg')?>" alt=""/>&nbsp;Clear</a>
                </div>
            ...
            <!-- <div class="" id="auto_table_data"></div> -->
                ...
                <div class="d-flex justify-content-between align-items-start">
                  <div class="d-flex  align-items-center">
                    <div class="profile-pic m-0 p-0">
                      <img class="img-circle rounded-circle avatar m-3  elevation-2" id="employee_img" style="cursor: pointer;" data-toggle="tooltip" data-placement="right" title="Profile Image" src="<?= base_url() ?>/assets_system/images/default_user.jpg">
                    </div>
  ...
  <script>
    ...
    if (SYSTEM_LEAVE_SETTING == 1) {
      ...
      const data = DISP_EMP_LIST.map(item=>{
          let name='';
          ...
          let employee = {empl_id: item.id,name,}

          for (let index = 0; index < colHeaders.length; index++) {
            const header = colHeaders[index];
            if (!columnsDone && index > 1) {
              columns = [...columns, {data: header.toLowerCase().replace(/ /g, '_'),width:90}]
              if(index == colHeaders.length -1){
                columnsDone = true;
              }
            }
            for (let index = 0; index < DISP_ENTITLEMENT.length; index++) {
              // let done = false;
              
              if (employee.empl_id == DISP_ENTITLEMENT[index].empl_id && DISP_ENTITLEMENT[index].type == header) {
                employee[DISP_ENTITLEMENT[index].type.toLowerCase().replace(/ /g, '_')] = DISP_ENTITLEMENT[index].value;
              }else if(!(employee.hasOwnProperty(header.toLowerCase().replace(/ /g, '_')))){
                ...
                employee[DISP_ENTITLEMENT[index].type.toLowerCase().replace(/ /g, '_')] = 0;
              }
      ...
      columns = [{data:'empl_id'},{data:'name', width: 240},...columns];
      // columns= [{
      //     data: 'id',
      //   }, {
      //     data: 'empl_id',
      //   }, {
      //     data: 'service_incentive_leave_(sil)',
      //   }, {
      //     data: 'vacation_leave',
      //   }, {
      //     data: 'sick_leave',
      //   },
      //   {data: 'bereavement_leave',},{data: 'maternity_leave',},
      //   {data: 'paternity_leave',},{data: 'solo-parent_leave',},
      //   {data: 'marriage_leave',},{data: 'emergency_leave',},
      //   {data: 'test',},
      // ]
    ...
    function initializedHandson(data){
      ...
      var remainingHeight = screenHeight - elementOffset - 30;
