...
<div class="content-wrapper">
    <div class="container-fluid p-4">
                            ...
                            <tbody>
                                ...
                                if ($DISP_PAYROLL_DATA) {
                                    foreach ($DISP_PAYROLL_DATA as $DISP_PAYROLL_DATA_ROW) {
                                        $application_id = 'PS' . str_pad($DISP_PAYROLL_DATA_ROW->id, 5, '0', STR_PAD_LEFT);
                                ...
                                ?>
                                            <tr class="payslip_row">
                                                ...
                                                <td hidden>
                                                    <input id="position" value="<?= $employee[0]->col_empl_posi ?>" hidden />
                                                </td>
                                                ...
                                                <td>
                                                    <a href="#" data-action="view" pagibig_loan_balance_amount="<?= $pagibig_loan_balance_amount ?>" pagibig_loan_amount_paid="<?= $DISP_PAYROLL_DATA_ROW->loan_pagibig_salary + $DISP_PAYROLL_DATA_ROW->loan_pagibig_calamity ?>" sss_loan_balance_amount="<?= $loan_balance_amount ?>" sss_loan_amount_paid="<?= $DISP_PAYROLL_DATA_ROW->loan_sss_salary + $DISP_PAYROLL_DATA_ROW->loan_sss_calamity ?>" used_sick_leave="<?= $used_sick_leave ?>" used_vacation_leave="<?= $used_vacation_leave ?>" payout="<?= $payout ?>" payroll_period_name="<?= $payroll_period[0]->name ?>" db_payroll_period="<?= $payroll_period[0]->db_name ?>" payslip_id="<?= $DISP_PAYROLL_DATA_ROW->id ?>" empl_sect="<?= $employee[0]->col_empl_sect ?>" empl_dept="<?= $employee[0]->col_empl_dept ?>" class="download_pdf">
                                                        <p class="btn btn-success mr-2 mb-0">
                                                            <i class="fas fa-eye"></i>
                                                        </p>
                                                        <div class="spinner-border text-danger loading_indicator" style="display:none;"></div>
                                                    </a>
                                                    <a href="#" data-action="download" pagibig_loan_balance_amount="<?= $pagibig_loan_balance_amount ?>" pagibig_loan_amount_paid="<?= $DISP_PAYROLL_DATA_ROW->loan_pagibig_salary + $DISP_PAYROLL_DATA_ROW->loan_pagibig_calamity ?>" sss_loan_balance_amount="<?= $loan_balance_amount ?>" sss_loan_amount_paid="<?= $DISP_PAYROLL_DATA_ROW->loan_sss_salary + $DISP_PAYROLL_DATA_ROW->loan_sss_calamity ?>" used_sick_leave="<?= $used_sick_leave ?>" used_vacation_leave="<?= $used_vacation_leave ?>" payout="<?= $payout ?>" payroll_period_name="<?= $payroll_period[0]->name ?>" db_payroll_period="<?= $payroll_period[0]->db_name ?>" payslip_id="<?= $DISP_PAYROLL_DATA_ROW->id ?>" empl_sect="<?= $employee[0]->col_empl_sect ?>" empl_dept="<?= $employee[0]->col_empl_dept ?>" class="download_pdf">
                                    ...
                                    <tr class="table-active">
                                        <td colspan="7">
                                            <div class="text-center">No Payslips Yet</div>
                                        </td>
                                    </tr>
                                <?php
                                }
        ...
        $dots = false;
        echo "<right><ul class='pagination mr-auto ml-auto'>";
        $show = 0;
        if ($page > 1) {
...
<script>
    $(document).ready(function() {
        ...
        $('.download_pdf').click(function(e) {
            ...
            let btnAction = $(this).attr("data-action");
            ...
            get_payslip_data(url4, payslip_id).then(function(data) {
                ...
                sss_loan = parseFloat(data.payslip_data[0].loan_sss_salary_calamity_total).toFixed(2);
                pagibig_loan = parseFloat(data.payslip_data[0].loan_pagibig_salary_calamity_total).toFixed(2);
                total_earnings = parseFloat(data.payslip_data[0].total_earnings).toFixed(2);
                total_deductions = parseFloat(data.payslip_data[0].total_deductions).toFixed(2);
                ...
                getEmployeeData(url3, employee_id).then(function(x) {
                    ...
                    get_empl_leave_data(url_get_leave, employee_id).then(function(leave) {
                        Array.from(leave).forEach(function(employee_data) {
                            ...
                            doc.line(160, 55, 200, 55) 
                            doc.line(160, 60, 200, 60) 
                            doc.line(160, 105, 200, 105) 
                            doc.line(40, 30, 40, 35) 
                            doc.line(58, 30, 58, 35)
                            doc.line(115, 30, 115, 35) 
                            doc.line(133, 30, 133, 35) 
                            doc.line(85, 30, 85, 118) 
                            doc.line(160, 30, 160, 118) 
                            ...
                            doc.text(14, 83.5, SIL_label);
                            if (ti_rest_sp_hol_mul != 0) {
                                doc.text(14, 86.5, 'Rest + Special Holiday');
                            }
                            if (ti_rest_legal_hol_mul != 0) {
                                doc.text(14, 89.5, 'Rest + Legal Holiday');
                            }
                            doc.setFont(undefined, 'bold');
                            doc.text(12, 93, 'ADJUSTMENTS');
                            doc.setFont(undefined, 'normal');
                            doc.text(14, 96, 'Meal Allowance');
                            doc.text(14, 99, 'Govt. Conributions');
                            doc.text(14, 102, 'Others');
                            doc.text(14, 105, '13th Month Pay');
                            doc.text(14, 108, 'Tax Refund');
                            doc.text(56, 41, "" + ti_basic_sal_mul + "", {
                                align: 'right'
                            }); 
                            doc.text(56, 46, "" + ti_leave_mul + "", {
                                align: 'right'
                            }); 
                            if (ta_daily_allowance > 0) {
                            ...
                            if (ti_rest_sp_hol_mul != 0) {
                                doc.text(56, 86.5, "" + ti_rest_sp_hol_mul + "", {
                                    align: 'right'
                                });
                            }
                            if (ti_rest_legal_hol_mul != 0) {
                                doc.text(56, 89.5, "" + ti_rest_legal_hol_mul + "", {
                                    align: 'right'
                                });
                            }
                            ...
                            if (ti_rest_sp_hol_mul != 0) {
                                doc.text(80, 86.5, "" + ti_rest_sp_hol_total + "", {
                                    align: 'right'
                                }); 
                            }
                            if (ti_rest_legal_hol_mul != 0) {
                                doc.text(80, 89.5, "" + ti_rest_legal_hol_total + "", {
                                    align: 'right'
                                });
                            }
                            doc.text(80, 96, "" + ti_meal_earning + "", {
                                align: 'right'
                            });
                            doc.text(80, 99, "" + ti_gov_cont_earning + "", {
                                align: 'right'
                            }); 
                            doc.text(80, 102, "" + ti_others_earning + "", {
                                align: 'right'
                            });
                            doc.text(80, 105, "-", {
                                align: 'right'
                            ...
                            doc.line(35, 125, 85, 125) 
                            doc.text(15, 130, 'Date:');
                            doc.line(35, 130, 85, 130);

                            if (btnAction == "download") {
...
<script>
    document.getElementById("btn_export").addEventListener('click', function() {
        ...
        XLSX.writeFile(wb, "mypayslips.xlsx");
    });
</script>
</body>
