...
<div class="content-wrapper" style="min-height: 624px;">
    <div class="container-fluid px-4">
        <div class='row mt-3'>
            <div class='col-md-8'>
                <h2><a href="<?= base_url('selfservices/my_overtimes') ?>"><img style="width: 32px; height: 32px; " class="mb-1" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt=""></a></h2>
            </div>
        </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="mr-2" style="float: right !important">
                                        <button type='button' onclick="submitForm()" class="btn btn-primary shadow-none rounded">
                                            Submit</button>
                                    </div>
                                </form>
                            </div>
...
$(document).ready(function() {
    ...
    let sessionUserId = "<?= $this->session->userdata('SESS_USER_ID'); ?>";
    
    let date = ''; // Initialize the date variable outside the event handler
    let currentShiftStartTime = null;
    
    // Function to calculate Early OT based on time range and shift start time
    function calculateEarlyOT() {
        let timeRange = $('#time-range').val();
        let earlyOtInput = $('#input_early_ot');
        
        // Only proceed if we have a shift start time and time range
        if (timeRange && currentShiftStartTime) {
            let timeRangeParts = timeRange.split(' - ');
            
            if (timeRangeParts.length === 2) {
                let startRange = timeRangeParts[0];
                let endRange = timeRangeParts[1];
                
                // Parse times (assuming format is "HH:mm:ss" from daterangepicker)
                let rangeStartTime = moment(startRange, "HH:mm:ss");
                let rangeEndTime = moment(endRange, "HH:mm:ss");
                let shiftStartTime = moment(currentShiftStartTime, "HH:mm:ss");
                
                // Check if time range starts earlier than shift start time
                if (rangeStartTime.isBefore(shiftStartTime)) {
                    // Early OT is the MINIMUM of:
                    // 1. Time from range start to shift start
                    // 2. Entire range duration (if range ends before shift start)
                    
                    let earlyDiffMinutes;
                    
                    // If entire range ends before shift start
                    if (rangeEndTime.isBefore(shiftStartTime)) {
                        // Early OT = entire range duration
                        earlyDiffMinutes = rangeEndTime.diff(rangeStartTime, 'minutes');
                        // Set input_hours to 0 since entire range is early
                        $('#input_hours').val('0');
                    } else {
                        // Only part of range is before shift start
                        // Early OT = from range start to shift start
                        earlyDiffMinutes = shiftStartTime.diff(rangeStartTime, 'minutes');
                        
                        // Calculate regular hours (after shift start)
                        let regularDiffMinutes = rangeEndTime.diff(shiftStartTime, 'minutes');
                        let regularDiffHours = regularDiffMinutes / 60;
                        $('#input_hours').val(regularDiffHours.toFixed(2));
                    }
                    
                    let earlyDiffHours = earlyDiffMinutes / 60;
                    
                    // Round to nearest 0.5
                    earlyDiffHours = Math.round(earlyDiffHours * 2) / 2;
                    
                    // Set Early OT value
                    earlyOtInput.val(earlyDiffHours.toFixed(1));
                    
                } else {
                    // If not early, set Early OT to 0
                    earlyOtInput.val('0');
                }
            }
        } else if (!timeRange) {
            // If time range is cleared, set both to 0
            $('#input_early_ot').val('0');
            $('#input_hours').val('0');
        }
    }
    
    // Fetch shift type
    $('#input_date_ot').on('change', function() {
        // Use the session user ID directly
        let empl_id = sessionUserId; // Directly use session variable
        date = $('#input_date_ot').val();
        ...
        $.post("<?= base_url('overtimes/get_shift_type') ?>", {
            ...
            if (res) {
                ...
                // Store shift start time for calculations
                currentShiftStartTime = start_time;
                
                // If time range is already selected, calculate Early OT
                if ($('#time-range').val()) {
                    setTimeout(calculateEarlyOT, 100);
                }
            }
            if (res.length <= 0) {
                $('#shift_type').html(`<span>No shift assign</span>`);
    ...
    // Get attendance and overtime values
    function ot_value(){
        // Use the session user ID directly
        let empl_id = sessionUserId; // Directly use session variable
        let ot_date = $('#input_date_ot').val();
    ...
    // Time range picker initialization
    ...
    $('#time-range').on('apply.daterangepicker', function(ev, picker) {
        ...
        // Store the total hours temporarily
        let totalHours = overtimeHours.toFixed(2);
        
        // Now calculate Early OT which will adjust the hours if needed
        calculateEarlyOT();
        
        // If Early OT didn't set input_hours (e.g., not early), set it to total hours
        if ($('#input_hours').val() === '0' && $('#input_early_ot').val() === '0') {
            $('#input_hours').val(totalHours);
        }
    });
        ...
        fetch(apiUrl, {
            ...
            .then(data => {
                ...
                if (data.messageError) {
                    ...
                    setTimeout(() => {
                        window.location.href = '<?= base_url() ?>selfservices/my_overtimes';
                    }, 1200);
                }
            })
            .catch(error => {
