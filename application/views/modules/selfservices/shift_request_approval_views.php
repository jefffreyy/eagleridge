...
<div class="content-wrapper">
    <div class="container-fluid p-4">
        ...
        <div class="filter-container  <?= $filter ? 'visible' : '' ?>">
                ...
                <div class="col-md-2" <?php echo ($DISP_VIEW_COMPANY ? "" : "hidden") ?>>
                    ...
                    <select name="dept" id="filter_by_company" class="form-control">
                        ...
                        ?>
                            ...
                            ?>
                                    <option value="<?= $DISP_DISTINCT_COMPANY_ROW->id ?>" <?php echo $param_comp == $DISP_DISTINCT_COMPANY_ROW->id ? 'selected' : '' ?>>
            ...
            <a href=<?= base_url() . "selfservices/leave_approval" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
            </div> -->
            </div>
            ...
            <div class="card border-0 p-0 m-0">
                <div class="">
                    <div>
                            ...
                            <div class=' align-items-center mt-2'>
                                <div class='row d-flex align-items-center'>
                                    ...
                                    <div class="d-flex col-sm-12 col-lg-2">
                                        <!-- <select class='filter custom-select w-100 mt-1'>
                        ...
                        <a href="<?= base_url('selfservices/leave_approval') ?>" id="btn_clear_filter" class="mt-1 btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="">&nbsp;Clear</a>
                    </div> -->
                            ...
                            <button id="bulk_approve_btn" class="btn technos-button-green shadow-none rounded bulk-button" style="height: 36px; margin-right: 6px;">
                                <img style="height: 1rem; width: 1rem; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-check-solid_mark.svg') ?>" alt="" />&nbsp;Bulk Note
                            </button>

                            <button id="bulk_reject_btn" class="btn btn-danger shadow-none rounded bulk-button" style="height: 36px;">
                                <img style="height: 1rem; width: 1rem; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-x-solid_mark.svg') ?>" alt="" />&nbsp;Bulk Reject
                            </button>
                            ...
                            <div class='row mt-3 py-1 justify-content-between'>
                                <div class='col-12 col-lg-4 d-flex justify-content-lg-start justify-content-center'>
                                    <a href="<?= base_url() . 'selfservices/view_shifts?page=' . $current_page . '&row=' . $row  ?> " id="" class="mt-1 btn btn-primary mx-1"> &nbsp;View Shifts</a>
                    ...
                    <table class="table table-bordered table-hover m-0" id="TableToExport" style="width:100%">
                                    ...
                                    <tr id="<?= $row_id ?>">
                                        ...
                                        <td class="text-center">
                                            ...
                                            if ($DISP_SHIFT_ROW->status == "Checked") {
                                                $color2 = 'background-color: #ff5c0a; color: #fff;';
                                            }
                                            ...
                                            ?>
                                            <span class="btn <?= $color ?> disabled px-1" style="width:70px;  font-size: 13px; <?= $color2 ?>">
                                                <?php
                        ...
                        <a class="select_row p-1 request_id" data-leave_id="<?= $DISP_SHIFT_ROW->id ?>" data-toggle="modal" data-target="#modal_approval">
                             <img src="<?= base_url('assets_system/icons/eye-sharp-solid_dark.svg') ?>" alt="">
                                        ...
                                        $disabled = '';
                                        if ($DISP_SHIFT_ROW->checked_by == $user_id) {
                                            $button = "Check";

                                            if ($DISP_SHIFT_ROW->status != "Pending") {
                                                $disabled = 'disabled';
                                            }
                                        }
                                        if ($DISP_SHIFT_ROW->noted_by == $user_id) {
                                            $button = "Note";

                                            if ($DISP_SHIFT_ROW->status != "Pending" && $DISP_SHIFT_ROW->status != "Checked") {
                                                $disabled = 'disabled';
                                            }
                                        }
                                        ...
                                        foreach ($DISP_SHIFT as $DISP_SHIFT_ROW) {
                                            if (in_array($DISP_SHIFT_ROW->status, ["Pending", "Checked"])) {
                                                $disable_bulk_buttons = false;
                                                break;
                                            }
                                        }
                                        ?>
                                        <td class="text-center">
                                            <!-- <a class="btn btn-info view-btn" style="width:70px;  font-size: 13px;"> -->
                                            <a class="btn btn-info view-btn" style="width:70px;  font-size: 13px;" href="<?= base_url() . 'selfservices/requested_shift/' . $DISP_SHIFT_ROW->empl_id . '/' . $DISP_SHIFT_ROW->cutoff_period . '?page=' . $current_page . '&row=' . $row . '&row_id=' . $row_id ?>">
                                                View
                                            </a>
                                            <!-- <button class="btn btn-danger btn_reject" style="width:100px;  font-size: 13px;">Reject</button> -->
                                            <button class="btn btn-success btn_approve" shift_status="<?= $button ?>" approve_id="<?= $DISP_SHIFT_ROW->id ?>" empl_id="<?= $DISP_SHIFT_ROW->empl_id ?>" date_from="<?= $DISP_SHIFT_ROW->date_from ?>" date_to="<?= $DISP_SHIFT_ROW->date_to ?>" style="width:70px;  font-size: 13px;" <?php if ($disabled === 'disabled'): ?>disabled<?php endif; ?>><?= $button ?> </button>
                                            <button class="btn btn-danger btn_reject" approve_id="<?= $DISP_SHIFT_ROW->id ?>" empl_id="<?= $DISP_SHIFT_ROW->empl_id ?>" date_from="<?= $DISP_SHIFT_ROW->date_from ?>" date_to="<?= $DISP_SHIFT_ROW->date_to ?>" style="width:70px;  font-size: 13px;" <?php if ($disabled === 'disabled'): ?>disabled<?php endif; ?>>Reject</button>
...
<div class="modal fade class_modal_approve" id="modal_bulk_approved" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="<?php echo base_url(); ?>selfservices/shift_bulk_approve" id="form_bulk_approve" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    ...
    window.addEventListener('load', () => {
        ...
        if (scrollPosition !== null) {
            ...
            sessionStorage.removeItem('scrollPosition'); // Optionally remove the scroll position
        }
    });
</script>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#row_dropdown').on('change', function(e) {
            ...
            window.location = base_url + "selfservices/shift_request_approval" + data;
            // filter_data(data);
        });
        ...
        function filter_data(page_row) {
            ...
            window.location = base_url + "selfservices/shift_request_approval" + page_row + filterUrl;
        }
        ...
        $('#bulk_approved').click(function() {
            ...
            if (selected_id.length > 0) {
                ...
                selected_row_id.forEach(function(data) {
                    $('#approve_list_id').append(`<li class="col-md-6">ID : <strong>SRA${data}</strong></li>`);
                })
            } else {
        ...
        $('#bulk_reject').click(function() {
            ...
            if (selected_id.length > 0) {
                ...
                selected_row_id.forEach(function(data) {
                    $('#reject_list_id').append(`<li class="col-md-6">ID : <strong>SRA${data}</strong></li>`);
                })
            } else {
        ...
        //Select only if the status is Pending or Checked
        $('#check_all').click(function() {
            if (this.checked == true) {
                $('.check_single').each(function() {
                    const row = $(this).closest('tr');

                    const isPending = row.find('.btn-warning').length > 0; // Check for 'Pending' status
                    const isChecked = row.find('.btn')
                        .filter(function() {
                            return $(this).css('background-color') === 'rgb(255, 92, 10)'; // Check for the "Checked" status
                        }).length > 0;

                    if (isPending || isChecked) {
                        $(this).prop('checked', true);
                        row.css('background', '#e7f4e4');
                    } else {
    ...
    //Disabled bulk buttons if the status is Approved, Rejected or Withdrawed
        $('.check_single').on('change', function() {
            var allCheckedRows = $('.check_single:checked'); 
            var disableBulkButtons = false;

            allCheckedRows.each(function() {
                var row = $(this).closest('tr'); 
                var status = row.find('td').eq(4).text().trim(); 

                if (status === "Approved" || status === "Rejected" || status === "Withdrawed") {
                    disableBulkButtons = true;
                    return false; 
                }
            });

            if (disableBulkButtons) {
                $('#bulk_approve_btn').prop('disabled', true);
                $('#bulk_reject_btn').prop('disabled', true);
            } else {
                $('#bulk_approve_btn').prop('disabled', false);
                $('#bulk_reject_btn').prop('disabled', false);
            }
        ...
        function disabled_btn(button, status) {
            // Update the span element based on the new status
            let newColor = "";
            let newColor2 = "";
            if (status == "Checked") {
                newColor2 = 'background-color: #ff5c0a; color: #fff;';
            }
            if (status == "Rejected") {
                newColor = "btn-danger";
            } else if (status == "Pending") {
                newColor = "btn-warning";
            } else if (status == "Withdrawed") {
                newColor = "btn-secondary";
            } else if (status == "Approved") {
                newColor = "btn-success";
            }

            // Find the specific row containing the clicked button
            let $row = $(button).closest('tr');
            let $span = $row.find('span.btn');

            $span.attr('class', 'btn ' + newColor + ' disabled px-1')
                .attr('style', 'width:70px; font-size: 13px;' + newColor2)
                .text(status);

            // Disable both buttons in the same row
            $row.find('.btn_approve, .btn_reject').attr('disabled', true);
        }
        ...
        $('.btn_approve').click(function(e) {
            ...
            var status = $(this).attr('shift_status');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = '<?= base_url() ?>';
                    const apiUrl = url + 'selfservices/update_shift_approval';
...
<script>
    $('#bulk_approve_btn').on('click', function() {
        ...
        if (selectedIds.length === 0) {
            alert('No shift request selected');
            return;
        }
        ...
        $.ajax({
            url: '<?= base_url("selfservices/bulk_approve_shifts") ?>',
            type: 'POST',
            ...
            success: function(response) {
                ...
                const message = response.success_message || response.warning_message;
                alert(message);
                location.reload();
            },
    ...
    $('#bulk_reject_btn').on('click', function() {
        const selectedIds = getSelectedShiftIds();
        if (selectedIds.length === 0) {
            alert('No shift request selected');
            return;
        ...
        $.ajax({
            url: '<?= base_url("selfservices/bulk_reject_shifts") ?>',
            type: 'POST',
            contentType: 'application/json',
            dataType: 'json',
            data: JSON.stringify(payload),
    ...
    function getSelectedShiftIds() {
        let selectedIds = [];
        $('.check_single:checked').each(function() {
            selectedIds.push($(this).attr('row_id'));
        });
        return selectedIds;
    }
</script>
