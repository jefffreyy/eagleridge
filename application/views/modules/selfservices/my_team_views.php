      ...
      <div class='col-md-8'>
        <h2 style="font-size: 24px;">
          <a onclick="afterRenderFunction()" href="<?= base_url() . 'selfservices'; ?>">
            <img style="width: 24px; height: 24px; margin-bottom: 10px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
          </a>&nbsp;My Team
        </h2>
      </div>
          ...
          callback: function() {
            ...
            for (let i = 0; i < res_data.length; i++) {
              if (res_data[i].reporting_to) {
                ...
                data.addRows([
                  ['<img class"user_image" onerror="setDefaultImage(this)" src="<?= base_url() ?>' + res_user + '" width=50px height=50px style="object-fit: scale-down; border-radius:100%; border: solid white 2px;">' + '<p>' + res_data[i].name + '</p>' + '<p><b>' + res_data[i].position + '</b></p>',
                    '<img class"user_image" onerror="setDefaultImage(this)" src="<?= base_url() ?>' + res_superior + '" width=50px height=50px style="object-fit: scale-down; border-radius:100%; border: solid white 2px;">' + '<p>' + res_data[i].reporting_to + '</p>' + '<p><b>' + res_data[i].superior_position + '</b></p>', res_data[i].position
                  ],
                ]);
              }
            }
