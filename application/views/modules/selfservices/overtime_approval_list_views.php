...
<div class="content-wrapper">
  <div class="container-fluid p-4">
    ...
    <div class="filter-container  <?= $filter ? 'visible' : '' ?>">
      <div class=" row mb-4 col-md-12 d-flex">
        ...
        <!-- <div class="col-md-2">
          ...
          <a href=<?= base_url() . "selfservices/overtime_approval" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
        </div> -->
      </div>
            ...
            <!-- <div class='justify-content-between'>
              <div class='align-items-center mt-2'>
                <div class='row d-flex align-items-center'>
                  ...
                  <!-- <div class="d-flex col-sm-12 col-lg-4 justify-content-lg-start justify-content-center">
                    ...
                    <a href="<?= base_url('selfservices/overtime_approval') ?>" id="btn_clear_filter" class="mt-1 btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="">&nbsp;Clear</a>
                  </div>
                </div>
              </div> -->
            ...
            <table class="table table-bordered table-hover m-0" id="TableToExport" style="width:100%">
              <thead>
                <tr>
                  ...
                  <th class="text-left">OVERTIME ID</th>
                  <th class="text-left" style='min-width:350px;'>EMPLOYEE</th>
                  <th class="text-left">TYPE</th>
                  <th class="text-left">DATE OT</th>
              ...
              <tbody id="tbl_application_container">
                <?php if ($DISP_OVERTIME) {
                  foreach ($DISP_OVERTIME as $DISP_OVERTIME_ROW) { ?>
                    <tr data-id="<?= $DISP_OVERTIME_ROW->id ?>" data-toggle="modal" data-target="#modal_approval" class="hover">
                      <!-- <td class="text-center select-item-td" id="select_item">
                        <input type="checkbox" name="approval_name" class="check_single" approval_id="<?= convert_cmid($DISP_EMPLOYEES, $DISP_OVERTIME_ROW->empl_id) ?>" employee_id="<?= $DISP_OVERTIME_ROW->empl_id ?>" row_id="<?= $DISP_OVERTIME_ROW->id ?>" value="<?= $DISP_OVERTIME_ROW->empl_id ?>" checkbox_stat="">
                      </td> -->
                      <td class="text-left">OVA<?= str_pad($DISP_OVERTIME_ROW->id, 5, '0', STR_PAD_LEFT)  ?></td>
                      <td class="text-left hover td-directs text-primary" data-empl_id="<?= $DISP_OVERTIME_ROW->empl_id ?>">
                        <?= convert_cmid($DISP_EMPLOYEES, $DISP_OVERTIME_ROW->empl_id) ?> - <?= convert_data($DISP_EMPLOYEES, $DISP_OVERTIME_ROW->empl_id) ?>
                      </td>
                      <td class="text-left"> <?= $DISP_OVERTIME_ROW->type ?></td>
                      <td class="text-left"> <?= date(($DATE_FORMAT) ? $DATE_FORMAT: "d/m/Y", strtotime($DISP_OVERTIME_ROW->date_ot)) ?> </td>
                      <td class="text-left"> <?= $DISP_OVERTIME_ROW->hours ?></td>
                      <td class="text-left"> <?= $DISP_OVERTIME_ROW->reason ?></td>
                      <!-- <td class="text-center">

                      <span class="btn btn-warning disabled">
                      <?php
                              preg_match('/pending/i', $DISP_OVERTIME_ROW->status, $matches);
                              if (!empty($matches)) {
                                echo 'Pending';
                              } else {
                                echo $DISP_OVERTIME_ROW->status;
                              }
                      ?>
                      </span>
                      </td> -->
                      ...
                      <td class="text-center">
                        <?php
                        if ($DISP_OVERTIME_ROW->status == "Approved") { ?>
                          <div class=' technos-button-green p-2 rounded disabled m-auto' style="width:100px"><?= $DISP_OVERTIME_ROW->status ?></div>
                        <?php } elseif ($DISP_OVERTIME_ROW->status == "Rejected") { ?>
                          <div class='bg-danger p-2 rounded disabled m-auto' style="width:100px"><?= $DISP_OVERTIME_ROW->status ?></div>
                        <?php } elseif ($DISP_OVERTIME_ROW->status == "Cancelled") { ?>
                          <div class='bg-secondary p-2 rounded disabled m-auto' style="width:100px"><?= $DISP_OVERTIME_ROW->status ?></div>
                        <?php } elseif ($DISP_OVERTIME_ROW->status == "Withdrawed") { ?>
                          <div class='bg-secondary p-2 rounded disabled m-auto' style="width:100px"><?= $DISP_OVERTIME_ROW->status ?></div>
                        <?php } elseif (preg_match('/Pending/i', $DISP_OVERTIME_ROW->status)) { ?>
                      ...
                      <!-- <td style="width:15%" class="text-center">
                        <a class="select_row p-1 request_id" data-id="<?= $DISP_OVERTIME_ROW->id ?>" data-toggle="modal" data-target="#modal_approval">
                           <img src="<?= base_url('assets_system/icons/eye-sharp-solid_dark.svg') ?>" alt="" s>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="border-bottom: none;">
        <h4 class="modal-title ml-1" id="exampleModalLabel">Overtime Bulk Approve
        </h4>
      ...
      <form action="<?php echo base_url(); ?>selfservices/overtime_bulk_approve" id="form_bulk_approve" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
...
<div class="modal fade class_modal_reject" id="modal_bulk_reject" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="border-bottom: none;">
        <h4 class="modal-title ml-1" id="exampleModalLabel">Overtime Bulk Reject
        </h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;
          </span>
      ...
      <form action="<?php echo base_url(); ?>selfservices/overtime_bulk_reject" id="form_bulk_reject" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
...
<script>
  $(document).ready(function() {
    ...
    function filter_data(page_row) {
      ...
      let filterUrl = page_row + "&status="+status; //'&company=' + company + '&branch=' + branch + '&department=' + department + '&division=' + division + '&section=' + section + "&group=" + group + "&team=" + team + "&line=" + line;
      if (document.querySelector('.filter-container').classList.contains('visible')) {
        filterUrl = filterUrl + '&filter=1';
      }
      // window.location = base_url + "selfservices/overtime_approval" + page_row + filterUrl;
    ...
    $('.request_id').on('click', function() {
      ...
      fetch(url)
        ...
        .then((data) => {
          console.log(data);
          $('#overtime_id').val('OVA' + data[0]['id'].padStart(5, '0'));
          $('#request_by').val(data['name']);
          $('#overtime_type').val(data[0]['type']);
          $('#date_ot').val(data[0]['date_ot']);
