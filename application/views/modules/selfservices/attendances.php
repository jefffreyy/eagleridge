...
class attendances extends CI_Controller
{
  ...
  function index()
  {
    ...
    $data["settings"] = array(
      array("title" => "settings", "url" => "attendances/settings", "button" => "btn btn-info")
    );
    ...
    $data["Modules"] =  array(
      ...
      array("title" => "Undertime",             "value" => "Attendance Undertime",       "info" => "Views essential information for each employee, status, designation.", "icon" => "file-import-duotone-att.svg",               "url" => "attendances/undertime",            "access" => "Attendance",  "id" => "undertime"),
  ...
  function exempt_undertime(){
    ...
    //   return;
      ...
      if($res){
          $this->session->set_flashdata('SUCC','Successfully exempted the undertime');
          ...
          redirect('attendances/undertime');
      }
  }
    ...
    try {
      ...
      foreach ($combinedData as $index => $data_row) {
        ...
        if ($id == $value && $selected == true) {
          $this->attendance_model->upload_data($data_row, $cutOffVal);
        }
      }
  ...
  function add_attendance_summary()
  {
    ...
    $this->load->view('templates/header');
    $this->load->view('modules/attendances/add_attendance_summary_views');
  }
  ...
  function edit_attendance_summary()
  {
    ...
    $data['DISP_ATTENDANCE_RECORDS']          = $this->attendance_model->GET_ALL_ATTENDANCE_RECORD($empl_id, $date_period);
  ...
  function edit_attendance_record()
  {
    ...
    try {
      foreach ($data as $data_row) {
        $this->attendance_model->EDIT_ATTENDANCE_RECORD($data_row);
      }
      $response = array('success_message' => 'Data updated successfully');
    } catch (Exception $e) {
      $response = array('warning_message' => 'Error updating data: ' . $e->getMessage());
  ...
  function daily_attendances()
  {
    ...
    $employees = $this->attendance_model->GET_EMPLOYEELIST_DAILY_ALL($param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line, $param_company, $param_type);
    $data['C_DATA_COUNT']               = $this->attendance_model->GET_EMPLOYEELIST_DAILY_COUNT($param_branch, $param_dept, $param_division, $param_section, $param_group, $param_team, $param_line, $param_company, $param_type);
    ...
    foreach ($employees as $employee) {
      ...
      $employeeObject->shiftBreakIn             = $LOG_act_break_in ? date("H:i", strtotime($LOG_act_break_in)) : "";
      $employeeObject->shiftBreakOut            = $LOG_act_break_out ? date("H:i", strtotime($LOG_act_break_out)) : "";
  ...
  {
    $user = $this->session->userdata('SESS_USER_ID');
    $data["view_type"]        = $view_type    = ['all', 'employee_id', $user];
    $data["module_name"]      = $module       = 'attendances';
    $data["page_name"]        = $page_name    = 'holidays';
    $data["model_name"]       = $model        = "main_table_02_model";
    $data["table_name"]       = $table        = "tbl_std_holidays";
    $data["module"]           = [base_url() . $module, "Attendance", "Holidays"];
    $data["id_prefix"]        = "HOL";
    $data["excel_import"]     = [false];
    $data["excel_output"]     = [false, "holidays.xlsx"];
    $data["add_button"]       = [true, false];
    $data["status_text"]      = ["", "", "", ""];
    $data["C_ROW_DISPLAY"]    = $filter_row = [25, 50, 100];
    ...
    foreach ($years as $year) {
      $c_data_tab[] = array($year->name, "year", (int)$year->name, 0);
    }

    // $c_data_tab             = array(
    //   array(2024, "year", 2024, 0),
    ...
    $data["C_DB_DESIGN"]  =
      array(
        ...
        array("year", "Year", "fixed-sel-direct", "2024;2023;2022;2021", 1, 15, 1, 1, 1, 1, 1, 1),
  ...
  function zkteco_attendance()
  {
    ...
    $data["C_ROW_DISPLAY"]                      =  [25, 50, 100];
    $user_id = null;
    if ($search && $search != 'all') $user_id   = $search;
    $page                                       = $this->input->get('page');
    $row                                        = $this->input->get('row');
    if ($row == null) {
      $row                                      = 25;
    ...
    foreach ($attendance  as $attendance_row) {
      ...
      foreach ($employees as $employee_row) {
        ...
        if ($code_id == $employee_row->id) {
          ...
          if (!empty($employee_row->col_suffix)) $result[$index]['employee_name'] = $result[$index]['employee_name'] . ' ' . $employee_row->col_suffix;
          if ($employee_row->col_frst_name) $result[$index]['employee_name'] = $result[$index]['employee_name'] . ', ' . $employee_row->col_frst_name;
          if ($employee_row->col_midl_name) $result[$index]['employee_name'] = $result[$index]['employee_name'] . ' ' . $employee_row->col_midl_name;
        }
      }
      $index++;
    ...
    foreach ($employees as $employee_row) {
      ...
      if ($zkteco_code) {
        ...
        if (!empty($employee_row->col_suffix)) $employee_zkteco[$employee_zkteco_index]['name'] = $employee_zkteco[$employee_zkteco_index]['name'] . ' ' . $employee_row->col_suffix;
        if ($employee_row->col_frst_name) $employee_zkteco[$employee_zkteco_index]['name'] = $employee_zkteco[$employee_zkteco_index]['name'] . ', ' . $employee_row->col_frst_name;
        if ($employee_row->col_midl_name) $employee_zkteco[$employee_zkteco_index]['name'] = $employee_zkteco[$employee_zkteco_index]['name'] . ' ' . $employee_row->col_midl_name;
