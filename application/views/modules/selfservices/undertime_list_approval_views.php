...
<div class="content-wrapper">
    <div class="container-fluid p-4">
        <div class="">
            <h1 class="page-title d-flex align-items-center"><a onclick="afterRenderFunction()" href="<?= base_url('selfservices/undertime_request_approval') ?>"> <img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
                </a>&nbsp;Undertime Approval List<h1>
        </div>
        <hr class="title-line">
                    ...
                    <tbody id="tbl_application_container">
                        <?php if ($DISP_SHIFT) {
                            foreach ($DISP_SHIFT as $DISP_SHIFT_ROW) {
                                ...
                                $formatted_date = date(($DATE_FORMAT) ? $DATE_FORMAT : "d/m/Y", strtotime($DISP_SHIFT_ROW->date_undertime));
                                $formatted_applied_date = date(($DATE_FORMAT) ? $DATE_FORMAT : "d/m/Y", strtotime($DISP_SHIFT_ROW->create_date)); ?>
                      ...
                      <input type="checkbox" name="approval_name" class="check_single" approval_id="<?= convert_cmid($DISP_EMPLOYEES, $DISP_SHIFT_ROW->empl_id) ?>" employee_id="<?= $DISP_SHIFT_ROW->empl_id ?>" row_id="<?= $DISP_SHIFT_ROW->id ?>" value="<?= $DISP_SHIFT_ROW->empl_id ?>" checkbox_stat="">
                                    ...
                                    <td class="text-left hover td-directs text-primary" data-empl_id="<?= $DISP_SHIFT_ROW->empl_id ?>"> <?= $DISP_SHIFT_ROW->employee ?> </td>
                                    <td> <?= $formatted_applied_date; ?> </td>
                                    <td> <?= $formatted_date; ?> </td>
                                    <td> <?= $DISP_SHIFT_ROW->current_shift; ?> </td>
                                    <td> <?= $DISP_SHIFT_ROW->request_time_in; ?> </td>
                                    <td> <?= $DISP_SHIFT_ROW->request_time_out; ?> </td>
                                    <td> <?= $DISP_SHIFT_ROW->reason; ?> </td>
                                    ...
                                    <!-- <td>
                                        <a href="<?= base_url('teams/undertime_approval/approve/' . $DISP_SHIFT_ROW->id) ?>" class="btn btn-success approve_req">Approve</a>
                                        <a href="<?= base_url('teams/undertime_approval/reject/' . $DISP_SHIFT_ROW->id) ?>" class="btn btn-danger reject_req">Reject</a>
                                    </td> -->
...
<script>
    $(document).ready(function() {
        ...
        //     window.location.href = "<?= base_url('selfservices/undertime_request_approval') ?>" + "?status=" + status + "&page=" + page + "&row=" + row;
        ...
        var baseUrl = '<?= base_url() . 'assets_user/files/teams/' ?>';
