  ...
  <div class="container-fluid p-4">
    <div class="">
      <h1 class="page-title d-flex align-items-center"><a onclick="afterRenderFunction()" href="<?= base_url('selfservices') ?>"> <img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
        </a>&nbsp;Backup Approvals<h1>
    </div>
    <hr class="title-line">

    <div class="pb-1">
      <ul class="nav nav-pills nav-fill backup_approvals-nav">
        <li class="nav-item <?= $approval_type == 'leaves' || !$approval_type ? 'bg-primary' : '' ?>">
          <a class="nav-link" href="?approval=leaves">Leaves</a>
        </li>
        <li class="nav-item <?= $approval_type == 'overtime' ? 'bg-primary' : '' ?>">
          <a class="nav-link" href="?approval=overtime">Overtime</a>
        </li>
        <li class="nav-item <?= $approval_type == 'time_adjustments' ? 'bg-primary' : '' ?>">
          <a class="nav-link" href=?approval=time_adjustments>Time Adjusments</a>
        </li>
        <li class="nav-item <?= $approval_type == 'holiday_work' ? 'bg-primary' : '' ?>">
          <a class="nav-link" href="?approval=holiday_work">Holiday Work</a>
        </li>
      </ul>
    </div>
    <hr>
    <div class="row mb-4">
      ...
      <div class="col-md-2">
        ...
        <a href=<?= base_url() . "selfservices/backup_approvals" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="" />Clear Filter</a>
      </div>
    ...
    <div class="card border-0 p-0 m-0">
      ...
      <div class="card border-0 p-0 m-0">
                ...
                <div class='col-12 col-lg-7 d-lg-flex d-none justify-content-lg-end'>
                  <div class="col-12 col-lg-6 ml-auto my-2 my-lg-0 row">
                    <div class="d-inline d-flex col-12 col-lg-8 justify-content-lg-end justify-content-center align-items-center">
                      <p class="ml-0 ml-lg-4 pagination m-0 p-0" style="color: gray">Showing <?= $low_limit ?> to <?= $high_limit ?> of <?= $C_DATA_COUNT ?> entries&nbsp;</p>
                    </div>
        ...
        <?php if ($approval_type != 'time_adjustments') {

          $this->load->view('modules/partials/_backup_approval_table');
        } else {
          $this->load->view('modules/partials/_backup_time_adj_approval_table');
        }
        ...
        ?>
          <div class='col-12 col-lg-7 d-lg-none d-flex justify-content-lg-end'>
                ...
                <div class="col-sm-3 col-md-2 col-lg-1 d-flex d-lg-none align-items-center justify-content-center justify-content-lg-start mr-lg-0 m-2 ">
                  <p class="p-0 m-0 d-inline" style="color: gray">&nbsp;&nbsp;Rows:&nbsp;</p>
...
<div class="modal fade class_modal_approve" id="modal_bulk_approved" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="border-bottom: none;">
        <h4 class="modal-title ml-1" id="exampleModalLabel">Bulk Approve
        </h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;
          </span>
      ...
      <form action="<?= base_url($APPROVE_URL); ?>" id="form_bulk_approve" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
...
<div class="modal fade class_modal_reject" id="modal_bulk_reject" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header" style="border-bottom: none;">
        <h4 class="modal-title ml-1" id="exampleModalLabel">Bulk Reject
        </h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;
          </span>
      ...
      <form action="<?= base_url($REJECT_URL) ?>" id="form_bulk_reject" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
...
function convert_data($array, $id)
{
  ...
  foreach ($array as $row) {
    if ($row->id == $id) {
      if (!empty($row->col_last_name)) $name = $row->col_last_name;
      if (!empty($row->col_suffix)) $name = $name . ' ' . $row->col_suffix;
      if (!empty($row->col_frst_name)) $name = $name . ', ' . $row->col_frst_name;
      if (!empty($row->col_midl_name)) $name = $name . ' ' . $row->col_midl_name[0] . '.';
      // $name = $row->col_last_name . ' ' . $row->col_frst_name . ' ' . $row->col_midl_name[0] . '.';
    }
  }
  return $name;
}
...
<script>
  $(document).ready(function() {
    ...
    $('#modal_approval').on('show.bs.modal', function(e) {
      ...
      var url = "<?= base_url('selfservices/get_leave_approval') ?>";
      if (approval_type === 'overtime') {
        url = "<?= base_url('selfservices/get_overtime_approval') ?>";
      }
      if (approval_type === 'time_adjustments') {
        url = "<?= base_url('selfservices/get_time_adjustment_approval') ?>";
      }
      if (approval_type === 'holiday_work') {
        url = "<?= base_url('selfservices/get_holiday_work_approval') ?>";
      }
    ...
    function filter_data(page_row) {
      ...
      let approval_type = "<?= $approval_type ?>";
      let status = $("#filter_by_status").find(":selected").val();


      window.location = base_url + "selfservices/backup_approvals" + page_row + "&company=" + company + "&branch=" + branch +
        "&dept=" + department + "&division=" + division +
        "&section=" + section + "&group=" + group +
        "&team=" + team + "&line=" + line + '&approval=' + approval_type;
    }
    ...
    $('#bulk_approved').click(function() {
      ...
      if (selected_id.length > 0) {
        ...
        selected_row_id.forEach(function(data) {
          let prefix = "<?= $PREFIX ?>";
          let format_id = prefix + data.padStart(5, 0);
          $('#approve_list_id').append(`<li class="col-md-6">ID : <strong>${format_id}</strong></li>`);
        })
      } else {
