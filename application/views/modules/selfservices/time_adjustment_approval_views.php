  ...
  <div class="container-fluid p-4">
    ...
    <div class="filter-container <?= $filter ? 'visible' : '' ?> ">
      <div class="row mb-4  d-flex ">
        ...
        <!-- <div class="col-md-2">
          ...
          <a href=<?= base_url() . "selfservices/time_adjustment_approval" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
        </div> -->
    ...
    <div class="card border-0 p-0 m-0">
          ...
          <div class='justify-content-between'>
            <div class='align-items-center mt-2'>
              <div class='row d-flex align-items-center justify-content-lg-start justify-content-center'>
                ...
                <a href="<?= base_url('selfservices/time_adjustment_approval') ?>" id="btn_clear_filter" class="mt-1 btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="">&nbsp;Clear</a> -->

              </div>
            </div>

            <div class='row mt-3 py-1  align-items-center'>
              <!-- <div class='col-12 col-lg-4 d-flex justify-content-lg-start justify-content-center'>
                <button class="btn technos-button-green shadow-none rounded bulk-button" style="height: 36px; margin-right:6px;" id="bulk_approved" data-toggle="modal" data-target="#modal_bulk_approved">
                  <img style="height: 1rem; width: 1rem; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-check-solid_mark.svg') ?>" alt="" />&nbsp;Bulk as Approve</button>
                <button class="btn btn-danger shadow-none rounded bulk-button" style="height: 36px;" id="bulk_reject" data-toggle="modal" data-target="#modal_bulk_reject">
                  <img style="height: 1rem; width: 1rem; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-x-solid_mark.svg') ?>" alt="" />&nbsp;Bulk as Reject</button>
              </div> -->

              <div class="col-12 col-lg-11 d-lg-flex d-none justify-content-lg-end">
          ...
          <tbody id="tbl_application_container">
            <?php if ($DISP_TIME_ADJUSTMENT) {
              foreach ($DISP_TIME_ADJUSTMENT as $DISP_TIME_ADJUSTMENT_ROW) { ?>
                <tr data-id="<?= $DISP_TIME_ADJUSTMENT_ROW->id ?>" data-toggle="modal" data-target="#modal_approval" class="hover">
                  ...
                  <td class="text-left">TAD<?= str_pad($DISP_TIME_ADJUSTMENT_ROW->id, 5, '0', STR_PAD_LEFT) ?></td>
                  <td class="text-left hover td-directs text-primary" data-empl_id="<?= $DISP_TIME_ADJUSTMENT_ROW->empl_id ?>">
                    <?= convert_cmid($DISP_EMPLOYEES, $DISP_TIME_ADJUSTMENT_ROW->empl_id) ?> - <?= convert_data($DISP_EMPLOYEES, $DISP_TIME_ADJUSTMENT_ROW->empl_id) ?>
                  </td>
                  <td class="text-left"> <?= date(($DATE_FORMAT) ? $DATE_FORMAT: "d/m/Y", strtotime($DISP_TIME_ADJUSTMENT_ROW->date_adjustment)) ?></td>
                  <td class="text-left"> <?= $DISP_TIME_ADJUSTMENT_ROW->shift_type ?></td>
                  <td class="text-left"> <?= $DISP_TIME_ADJUSTMENT_ROW->time_in_1 ?> </td>
                  <td class="text-left"> <?= $DISP_TIME_ADJUSTMENT_ROW->time_out_1 ?></td>
                  <td class="text-left"> <?= $DISP_TIME_ADJUSTMENT_ROW->reason ?></td>
                  <td class="text-center">

                <!-- <div class='bg-warning p-2 rounded disabled m-auto' style="width:100px"> -->
                <span class="btn btn-warning disabled">
                <?php
                        preg_match('/pending/i', $DISP_TIME_ADJUSTMENT_ROW->status, $matches);
                        if (!empty($matches)) {
                          echo 'Pending';
                        } else {
                          echo $DISP_TIME_ADJUSTMENT_ROW->status;
                        }
                ?>
                </span>
                  ...
                  <td style="width:15%" class="text-center">
                    <a class="select_row p-1 request_id" data-id="<?= $DISP_TIME_ADJUSTMENT_ROW->id ?>" data-toggle="modal" data-target="#modal_approval">
      ...
      <div class="col-12 col-lg-7 d-lg-none d-block justify-content-lg-end">
        ...
        <div class="col-sm-3 col-md-2 col-lg-2  d-flex align-items-center justify-content-center justify-content-lg-start mr-lg-0 m-2">
          <p class="p-0 m-0 d-inline " style="color: gray">&nbsp;&nbsp;Rows:&nbsp;</p>
          <select id="row_dropdown" class="custom-select" style="width: auto;">
            <?php
            foreach ($C_ROW_DISPLAY as $C_ROW_DISPLAY_ROW) { ?>
...
<script>
  $(document).ready(function() {
    ...
    function filter_data(page_row) {
      ...
      let filterUrl = page_row + "&company=" + company + "&branch=" + branch +
      ...
      window.location = base_url + "selfservices/time_adjustment_approval" + filterUrl;
    }
