    ...
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title d-flex align-items-center"><a href="<?= base_url() . 'benefits'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
          </a>&nbsp;Other Deductions<h1>
      </div>
      <div class="col-md-6 button-title">
        <a href="<?= base_url() . 'benefits/other_deductions_type' ?>" id="btn_new" class=" btn btn-primary shadow-none rounded"><img style="width: 17px; height: 17px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/gear-solid_xs.svg') ?>" alt="">&nbsp;Other Deduction Type</a>
      </div>
    </div>
    <hr>
    <div class=" p-3 row d-flex">
      ...
      <div class="col-4 col-xl-3 m-1">
        ...
        <select class="form-control type_filter" id="filter_by_type">
          <?php foreach ($OTHER_DEDUCTIONS_TYPE_LIST as $type) { ?>
...
<script>
  ...
  update_date.addEventListener('click', function() {
    ...
    fetch(url + 'benefits/update_other_deductions_data', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
<script>
  $(document).ready(function() {
    ...
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('benefits/other_deductions?period=') ?>" + $(this).val() + "&type=" + filter_type ;
    })
    $('select.type_filter').on('change', function() {
      window.location.href = "<?= base_url('benefits/other_deductions?period=') ?>" + filter_period + "&type=" + $(this).val() ;
    })
    $('select.income_type').on('change', function() {
      window.location.href = "<?= base_url('benefits/other_deductions?income_type=') ?>" + $(this).val();
    })
