    ...
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title d-flex align-items-center"><a href="<?= base_url() . 'benefits'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
          </a>&nbsp;Non-Taxable Allowances<h1>
      </div>
      <div class="col-md-6 button-title d-flex justify-content-center justify-content-lg-end ">
        <a href="<?= base_url() . 'benefits/add_nontaxable_category' ?>" id="btn_new" class="mr-1 btn btn-primary shadow-none rounded"><img style="width: 17px; height: 17px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/plus-solid.svg') ?>" alt="">
          &nbsp;Categories</a>
          <a <?= ($NIGHTSHIFT_ALLOWANCE_NONTAX == 0) ? 'hidden' : ""; ?> href="<?= base_url() . 'benefits/add_nightshift_category_nontax' ?>" id="btn_new" class="mr-1 btn btn-primary shadow-none rounded"><img style="width: 17px; height: 17px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/plus-solid.svg') ?>" alt="">
          &nbsp;Night Shift Categories</a>
        <a href="<?= base_url() . 'benefits/nontaxable_type' ?>" id="btn_new" class="btn btn-primary shadow-none rounded"><img style="width: 17px; height: 17px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/gear-solid_xs.svg') ?>" alt="">
          &nbsp;Types</a>

        <!-- <button class="d-block d-lg-none btn btn-primary" id="btn-update">
          <img style="width: 17px; height: 17px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="">&nbsp;Update
        </button> -->
      </div>
    </div>
      ...
      <div class="col-12 col-lg-5 d-flex">
        <div class="col-6 col-lg-4">
          ...
          <select class="form-control type_filter" id="filter_by_type">
            <?php
            if (!empty($NONTAXABLE_TYPE_LIST)) {
              foreach ($NONTAXABLE_TYPE_LIST as $type) { ?>
        ...
        <div class="col-6 col-lg-4" hidden>
    ...
    <div class="card border-0 p-0 m-0">
      <div class="card border-0 p- m-0">
        ...
        <div>
          ...
          <button class="d-none d-lg-block btn btn-primary" id="btn-update">
            <img style="width: 17px; height: 17px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="">&nbsp;Update
          </button>
        </div>
...
<script>
  var url                                 = '<?= base_url() ?>';
  var benefits_fixed                      = <?= json_encode($DISP_BENEFITS_NONTAXABLE_ASSIGN); ?>;
  var employeeList                        = <?= json_encode($DISP_EMPLOYEELIST); ?>;
  var dynamic_categories                  = <?= json_encode($DISP_NONTAXABLE_CATEGORIES); ?>;
  var tax_nontax_allowance                = <?= json_encode($tax_nontax_allowance); ?>;
  var transportation_allowance_nontax     = <?= json_encode($DISP_TRANSPORTATION_ALLOWANCE_NONTAX); ?>;
  var nightshift_allowance_nontax         = <?= json_encode($NIGHTSHIFT_ALLOWANCE_NONTAX); ?>;
  
  var nightshift_categories_nontax        = <?= json_encode($DISP_NIGHTSHIFT_CATEGORIES_NONTAX); ?>;
  ...
  let nightshiftCategories = nightshift_categories_nontax.map(function(item){
    return item.name;
  });
  ...
  var combinedData = employeeList.map(function(employee) {
    ...
    var transportation_allowance_nontax_val = transportation_allowance_nontax.find(function (trans) {
    ...
    return {
      ...
      transportation: transportation_allowance_nontax_val ? transportation_allowance_nontax_val.nightshift_category : '',
      release_date: benefit_val ? benefit_val.release_date : '', 
    };
  });
  ...
  const onetimeTypeElement = document.querySelector('#filter_by_type option[selected]');
  const onetimeType = onetimeTypeElement?.dataset?.onetimeAttendance ?? '';
  ...
  if (nightshift_allowance_nontax && nightshift_allowance_nontax === '0') {
  ...
  update_date.addEventListener('click', function() {
    ...
    fetch(url + 'benefits/update_nontaxable_data', { //benefits/update_fixed_data
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
<script>
  $(document).ready(function() {
    ...
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('benefits/nontaxable?type=') ?>" + filter_type
    })
    $('select.type_filter').on('change', function() {
      window.location.href = "<?= base_url('benefits/nontaxable?type=') ?>" + $(this).val();
    })
    $('select.income_type').on('change', function() {
      window.location.href = "<?= base_url('benefits/nontaxable?income_type=') ?>" + $(this).val();
    })
  })
</script>
</body>
