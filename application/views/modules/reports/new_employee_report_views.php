  ...
  <div class="container-fluid p-4">
    <div class="row pt-1">
      <div class="col-md-6">
        ...
        <h1 class="page-title d-flex align-items-center"><a href="<?= base_url('reports/employee_informations'); ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" /></a>&nbsp;Newly Hired Employees<h1>
      </div>
        ...
        <div class="table-responsive" style="max-height: 75vh">
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                <th class="text-left" style="min-width: 100px !important">DATE HIRE</th>
            ...
            <tbody id="tbl_application_container">
              <?php if ($EMPLOYEES) {
                foreach ($EMPLOYEES as $employee) { ?>
                  <tr>
                    <td class="text-left"><?= date_format(date_create($employee->col_hire_date), "d/m/Y") ?></td>
...
<script>
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : EMPLOYEES,
      colHeaders: ['ID', 'Date Hire', 'Employee ID', 'Employee Name'],
      rowHeaders: true,
      height: 'auto',
        ...
        <table>
                ...
                <tr>
                  <th colspan="3">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Newly Hired Employees</h5>
            ...
            <tbody>
                <tr>
                  ${EMPLOYEES.map(item => `
                    <tr>
                      <td>${item.col_hire_date? formatDateOnly(item.col_hire_date) : ''}</td>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/new_employees?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/new_employees?period=') ?>" + $(this).val()
    })
    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/new_employees?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/new_employees?period=') ?>" + period;
      }
    }
    ...
    //     let url="<?= base_url('reports/get_new_employees_data?date_from=' . date_format(date_create($START_DATE), 'Y-m-d') . '&date_to=' . date_format(date_create($END_DATE), 'Y-m-d')) ?>"          
    //     fetch(url)
    //     .then(res=>res.json())
    //     .then((data)=>{
    //         let data_formatted=[];
    ...
    //             // name,
    //             // data[i].col_hire_date]
    //             data_formatted.push([data[i].col_empl_cmid,
    //             data[i].col_last_name+' '+data[i].col_frst_name+','+data[i].col_midl_name,
    //             name,
    //             data[i].col_hire_date])
    //         }
    ...
    //         const title = 'Newly Hired Employees';
    ...
    //         doc.autoTable({
    //             startY: 40,
    //             head: [['Employee ID', 'Employee', 'Date Hire']],
    //             headStyles: {
