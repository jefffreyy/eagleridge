  ...
  <div class="container-fluid p-4">
    ...
    <div class="row my-3">
      ...
      <div class="col-12 col-lg-2 d-flex align-items-end">
        <a href="<?= base_url('reports/leaves') ?>" id="btn_clear_filter" class="btn btn-primary">
    ...
    <!-- <div class="card border-0 p-0 m-0">
      <div class="card border-0 py-1 m-0">
        <div>
          <p class=" m-2 bg-primary p-1  text-center ml-auto" style="color: gray;width:150px;border-radius:15px">Showing <?= count($LEAVES) ?> entries&nbsp;</p>
        </div>
          ...
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                ...
                <th class="text-left" style="min-width: 100px !important">DURATIONS</th>
              </tr>
            </thead>
            <tbody id="tbl_application_container">
              <?php if ($LEAVES) {
                foreach ($LEAVES as $leave) { ?>
                  <tr>
                    <td class="text-left"><?= date_format(date_create($leave->leave_date), "d/m/Y") ?></td>
                    <td class="text-left"><?= $leave->col_empl_cmid ?></td>
                    <td class="text-left"><?= $leave->fullname ?></td>
                    <td class="text-left"><?= $leave->type ?></td>
                    <td class="text-left"><?= $leave->duration . ' hours' ?></td>
                  </tr>
                <?php  }
              } else { ?>
...
<script>
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : LEAVES,
      colHeaders: ['ID','Date', 'Employee ID', 'Employee Name', 'Type', 'Duration'],
      rowHeaders: true,
      height: 'auto',
...
<script>
  function printFunction() {
    ...
    var leavesData = <?php echo json_encode($LEAVES); ?>;
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/leaves?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/leaves?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/leaves?period=') ?>" + period;
      }
    }
    ...
    //     let url                 ="<?= base_url('reports/get_leave_data?date_from=') ?>"+date_from_formatted+'&date_to='+date_to_formatted;
    //     fetch(url)
    //     .then(res=>res.json())
    //     .then((data)=>{ 
    //         let data_formatted=[];
    ...
    //             data_formatted[i]=[data[i].leave_date,
    //             // data[i].col_last_name+' '+data[i].col_frst_name+','+data[i].col_midl_name,
    //             name,
    //             data[i].type,data[i].duration+' hours']
    //         }
    //         const doc = new jsPDF();
    ...
    //         const title = 'Approved Leave Report';
    //         // const cutoffDate = new Date().toLocaleDateString();
    //         let cutoffDateText=`Cut-off Period: ${date_from} - ${date_to}`;
    ...
    //         doc.autoTable({
    //             startY: 40,
    //             head: [['Date', 'Employee', 'Type','Duration']],
    //             headStyles: {
...
<script>
    $(document).ready(function() {
      ...
      $('input[name="daterange"]').daterangepicker({
        ...
        window.location.href = "<?= base_url('reports/leaves?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD') + '&search=' + employeeId;
      });
      ...
      $('select.cut_off_period').on('change', function() {
        window.location.href = "<?= base_url('reports/leaves?period=') ?>" + $(this).val()
      })
      $('#daily_add').on('change', function() {
        window.location.href = "<?= base_url('reports/leaves?date=') ?>" + $(this).val();
      })
...
function exportData() {
  ...
  // Download Excel file
  XLSX.writeFile(wb, "approved_leaves_report.xlsx");
}
