  ...
  <div class="container-fluid p-4">
    <div class="row pt-1">
      <div class="col-md-6">
        ...
        <h1 class="page-title d-flex align-items-center"><a href="<?= base_url('reports/employee_informations'); ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" /></a>&nbsp;Changed Position Employees<h1>
      </div>
          ...
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: -1;">
              <tr>
                ...
                <th class="text-left" style="min-width: 100px !important">CHANGED FROM</th>
                <th class="text-left" style="min-width: 100px !important">CHANGED TO</th>
              </tr>
            </thead>
              ...
              <?php if ($EMPLOYEES) {
                foreach ($EMPLOYEES as $employee) { ?>
                  <tr>
                    ...
                    <td class="text-left"><?= $employee->from_val == 0 ? '' : $employee->from_val ?></td>
                    <td class="text-left"><?= $employee->to_val ?></td>
                  </tr>
                <?php  }
              } else { ?>
...
<script>
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : EMPLOYEES,
      colHeaders: ['ID', 'Date', 'Employee ID', 'Employee Name', 'Changed From', 'Changed To'],
      rowHeaders: true,
      height: 'auto',
      ...
      columns: [
        ...
        { data: 'formatted_date', readOnly: true },
        { data: 'col_empl_cmid', readOnly: true },
        { data: 'fullname', readOnly: true },
        { data: 'from_val', readOnly: true,},
        { data: 'to_val', readOnly: true,},
      ],
    ...
    var printContents = ` 
        <table>
            <thead>
                <tr>
                  <th colspan="5">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Changed Position Employees</h5>
                ...
                <th class="bg-gray-ben">Changed From</th>
                <th class="bg-gray-ben">Changed To</th>
                </tr>
            </thead>
            <tbody>
                <tr>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/promoted_employees?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/promoted_employees?period=') ?>" + $(this).val()
    });

    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/promoted_employees?date=') ?>" + $(this).val();
    })
    // Handle event to change filter type
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/promoted_employees?period=') ?>" + period;
      }
    }
    ...
    //   let url="<?= base_url('reports/get_promoted_data?date_from=' . date_format(date_create($START_DATE), 'Y-m-d') . '&date_to=' . date_format(date_create($END_DATE), 'Y-m-d')) ?>"
    //     fetch(url)
    //     .then(res=>res.json())
    //     .then((data)=>{
    //         let data_formatted=[];
    ...
    //         const title = 'Promoted Employees';
    ...
    //         doc.autoTable({
    //             startY: 40,
    //             head: [['Date', 'Employee','Promoted From','Promoted To']],
    //             headStyles: {
