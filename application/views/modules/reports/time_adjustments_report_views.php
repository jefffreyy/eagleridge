  ...
  <div class="container-fluid p-4">
    <div class="row pt-1">
      ...
      <div class="col-md-6 button-title d-none d-lg-flex justify-content-end">
        <button type="button" class="btn btn-primary ml-1" id="printButton" onclick="printFunction()">
          <img style="width: 24px; height: 20px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/print-duotone_sm.svg') ?>" alt="" />
          &nbsp;Print
        </button>
     ...
     <div class="row my-3">
      ...
      <div class="col-12 col-lg-2 d-flex align-items-end">
        <a href="<?= base_url('reports/time_adjustments') ?>" id="btn_clear_filter" class="btn btn-primary">
    ...
    <!-- <div class="card border-0 p-0 m-0">
      <div class="card border-0 py-1 m-0">
        <div>
          <p class=" m-2 bg-primary p-1  text-center ml-auto" style="color: gray;width:150px;border-radius:15px">Showing <?= count($TIME_ADJS) ?> entries&nbsp;</p>
        </div>
        <div class="table-responsive" style="max-height: 75vh">
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            ...
            <tbody id="tbl_application_container">
              <?php if ($TIME_ADJS) {
                foreach ($TIME_ADJS as $adj_time) { ?>
                  <tr>
                    <td class="text-left"><?= date_format(date_create($adj_time->date_adjustment), "d/m/Y") ?></td>
                    <td class="text-left"><?= $adj_time->col_empl_cmid ?></td>
                    <td class="text-left"><?= $adj_time->fullname ?></td>
                    <td class="text-left"><?= $adj_time->time_in_1 ?></td>
                    <td class="text-left"><?= $adj_time->time_out_1 ?></td>
                  </tr>
                <?php  }
              } else { ?>
...
<script>
  ...
  let TIME_ADJS = <?= json_encode($TIME_ADJS); ?>;
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : TIME_ADJS,
      colHeaders: ['ID','Date', 'Employee ID', 'Employee Name', 'Time In', 'Time Out'],
      rowHeaders: true,
      height: 'auto',
      ...
      columns: [
        ...
        { data: 'time_in_1', readOnly: true,},
        { data: 'time_out_1', readOnly: true,},
      ],
...
<script>
  function printFunction() {
    ...
    var TIME_ADJS = <?php echo json_encode($TIME_ADJS); ?>;
    console.log('TIME_ADJS', TIME_ADJS);
        ...
        <table>
            <thead>
                <tr>
                  <th colspan="7">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Approved Time Adjustments</h5>
                ...
                <tr>
                  ...
                  <th class="bg-gray-ben">Timed In</th>
                  <th class="bg-gray-ben">Timed Out</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                  ${TIME_ADJS.map(item => `
                    <tr>
                      <td>${item.date_adjustment}</td>
                      <td>${item.col_empl_cmid}</td>
                      <td>${item.fullname}</td>
                      <td>${item.time_in_1}</td>
                      <td>${item.time_out_1}</td>
                    </tr>
                  `).join('')}
                </tr>
            </tbody>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/time_adjustments?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/time_adjustments?period=') ?>" + $(this).val()
    })
    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/time_adjustments?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/time_adjustments?period=') ?>" + period;
      }
    }
    ...
    //     let url                 ="<?= base_url('reports/get_time_adjustments_data?date_from=') ?>"+date_from_formatted+'&date_to='+date_to_formatted;
    //     fetch(url)
    //     .then(res=>res.json())
    //     .then((data)=>{
    //         let data_formatted=[];
    ...
    //             data_formatted[i]=[data[i].date_adjustment,
    //             // data[i].col_last_name+' '+data[i].col_frst_name+','+data[i].col_midl_name,
    //             name,
    //             data[i].time_in_1,data[i].time_out_1,data[i].time_in_2,data[i].time_out_2]
    //         }
    //         const doc = new jsPDF();
    ...
    //         const title = 'Approved Time Adjustments';
    ...
    //         doc.autoTable({
    //             startY: 40,
    //             head: [['Date', 'Employee', 'Time In 1','Time Out 1','Time In 2','Time Out 2']],
    //             headStyles: {
...
<script>
    $(document).ready(function() {
      ...
      $('input[name="daterange"]').daterangepicker({
        ...
        window.location.href = "<?= base_url('reports/time_adjustments?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD') + '&search=' + employeeId;
      });
...
function exportData() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Time Adjustment Report");
  ...
  // Download Excel file
  XLSX.writeFile(wb, "approved_time_adjustment_report.xlsx");
}
