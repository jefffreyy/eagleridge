...
<div class="content-wrapper">
  <div class="container-fluid p-4">
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title d-flex align-items-center"><a href="<?= base_url('reports/employee_informations'); ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
          </a>&nbsp;Active Employees<h1>
      </div>
      <div class="col-md-6 button-title d-none d-lg-flex justify-content-end">
        ...
        <button type="button" class="btn btn-primary ml-1" id="printButton" onclick="printFunction()"><img style="width: 24px; height: 20px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/print-duotone_sm.svg') ?>" alt="" />
          &nbsp;Print</button>
      </div>
    </div>
    ...
    <div class="row my-3">
      <div class="col-12 col-lg-2 justify-content-center justify-content-lg-start">
        <h6 class="d-block">Select Date Range</h6>
          ...
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                ...
                <th class="text-left" style="min-width: 100px !important">POSITION</th>
                <th class="text-left" style="min-width: 100px !important">DESIGNATION</th>
              </tr>
            </thead>
              ...
              <?php if ($EMPLOYEES) {
                foreach ($EMPLOYEES as $employee) { ?>
                  <tr>
                    ...
                    <td class="text-left"><?= $employee->empl_position ?></td>
...
<script>
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : EMPLOYEES,
      colHeaders: ['ID','Employee ID', 'Employee Name', 'Position', 'Designation'],
      rowHeaders: true,
      height: 'auto',
      ...
      columns: [
        ...
        { data: 'empl_position', readOnly: true,},
        { data: 'designation', readOnly: true,},
      ],
...
<script>
  function printFunction() {
    ...
    var printContents = ` 
        <table>
            <thead>
                <tr>
                  <th colspan="4">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Active Employees</h5>
                ...
                <th class="bg-gray-ben">Position</th>
                <th class="bg-gray-ben" style="width: 54%;white-space: normal;">Designation</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                  ${EMPLOYEES.map(item => `
                    <tr>
                      ...
                      <td>${item.fullname}</td>
                      <td>${item.empl_position? item.empl_position: ''}</td>
    ...
    styleSheet += '@page { margin: 10px 10px; background-color: #fff !important;}';
    styleSheet += '* { background-color: #fff !important; }';
    styleSheet += 'body { background-color: #fff !important; }';
    styleSheet += 'table,tr { background: #fff !important;}';
    styleSheet += 'th, td { border: 1px solid #dddddd; text-align: center; font-size: 14px !important; background: #fff !important;}';
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/active_employees?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/active_employees?period=') ?>" + $(this).val()
    })

    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/active_employees?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/active_employees?period=') ?>" + period;
      }
    }

    function generateDesignate(items) {
      // Base case: If there are no items, return an empty string
      if (items.length === 0) {
        return '';
      }

      // If there are items, construct the hierarchy recursively
      const currentItem = items[0];
      const remainingItems = items.slice(1);

      if (remainingItems.length === 0) {
        // If this is the last item, return it as is
        return currentItem;
      } else {
        // Otherwise, concatenate the current item with the hierarchy of the remaining items
        if (currentItem) {
          return currentItem + ' > ' + generateDesignate(remainingItems);
        }
        return generateDesignate(remainingItems);
      }
    }

    // function generatePDF() {
    //   let cutoff = $('.cut_off_period option:selected');
    //   let cutoff_id = cutoff.val();
    //   let date_from = "<?= date_format(date_create($START_DATE), 'd/m/Y') ?>";
    //   let date_to = "<?= date_format(date_create($END_DATE), 'd/m/Y') ?>";
    //   let row = $('#row_dropdown').val();
    //   let page = "<?= $PAGE ?>";
    //   let daily = $('#daily_add').val();
    //   let printDate = "<?= date('d/m/Y') ?>";

    //   let url="<?= base_url('reports/get_active_employees_data?date_from=' . date_format(date_create($START_DATE), 'Y-m-d') . '&date_to=' . date_format(date_create($END_DATE), 'Y-m-d')) ?>"

    //   fetch(url)
    //     .then(res => res.json())
    //     .then((data) => {
    //       // $employee->empl_branch,$employee->empl_department,
    //       //     $employee->empl_division,$employee->empl_section,$employee->empl_group,$employee->empl_team,$employee->empl_line
    ...
    //           name,
    //           data[i].empl_position,,
    //           generateDesignate(['Company', data[i].empl_branch, data[i].empl_department,
    //             data[i].empl_division, data[i].empl_section,
    //             data[i].empl_group, data[i].empl_team, data[i].empl_line
    //           ])
    //         ]
    //       }
    //       const doc = new jsPDF();
