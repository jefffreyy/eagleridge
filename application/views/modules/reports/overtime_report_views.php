  ...
  <div class="container-fluid p-4">
    ...
    <div class="row my-3">
      ...
      <div class="col-12 col-lg-2 d-flex align-items-end">
        <a href="<?= base_url('reports/overtimes') ?>" id="btn_clear_filter" class="btn btn-primary">
    ...
    <div class="col-md-6 button-title d-lg-none d-flex justify-content-end my-2">
      <button type="button" class="btn btn-primary ml-1" id="printButton" onclick="printFunction()"><img style="width: 24px; height: 20px; margin-bottom: 4px;" src="<?= base_url('assets_system/icons/print-duotone_sm.svg') ?>" alt="" />&nbsp;Print</button>
    </div>
    ...
    <!-- <div class="card border-0 p-0 m-0">
      <div class="card border-0 py-1 m-0">
        <div>
          <p class=" m-2 bg-primary p-1  text-center ml-auto" style="color: gray;width:150px;border-radius:15px">Showing <?= count($OVERTIMES) ?> entries&nbsp;</p>
        </div>
          ...
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                ...
                <th class="text-left" style="min-width: 150px !important">DAY TYPE</th>
                ...
                <th class="text-left" style="min-width: 150px !important">SHIFT CODE</th>
                <th class="text-left" style="min-width: 150px !important">SHIFT OUT</th>
            ...
            <tbody id="tbl_application_container">
              <?php if ($OVERTIMES) {
                foreach ($OVERTIMES as $overtime) { ?>
                  <tr>
                    <td class="text-left"><?= date_format(date_create($overtime->date_ot), "d/m/Y") ?></td>
                    <td class="text-left"><?= $overtime->type ?></td>
                    <td class="text-left"><?= $overtime->col_empl_cmid ?></td>
                    <td class="text-left"><?= $overtime->fullname ?></td>
                    <td class="text-left"><?= $overtime->time_out ?></td>
                    <td class="text-left"><?= $overtime->code ?></td>
                    <td class="text-left"><?= $overtime->time_regular_end ?></td>
                    <td class="text-left"><?= $overtime->hours ?></td>
                  </tr>
                <?php  }
              } else { ?>
...
<script>
  ...
  let OVERTIMES = <?= json_encode($OVERTIMES); ?>;
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : OVERTIMES,
      colHeaders: ['ID','Date', 'Day Type', 'Employee ID', 'Employee Name', 'Time Out', 'Shift Code', 'Shift Out', 'No. of Hours'],
      rowHeaders: true,
      height: 'auto',
...
<script>
  function printFunction() {
    ...
    var OVERTIMES = <?php echo json_encode($OVERTIMES); ?>;
                ...
                <tr>
                  <th colspan="5">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Approved Overtimes</h5>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/overtimes?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/overtimes?period=') ?>" + $(this).val()
    })

    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/overtimes?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/overtimes?period=') ?>" + period;
      }
    }
    ...
    //     let url                 ="<?= base_url('reports/get_overtime_data?date_from=') ?>"+date_from_formatted+'&date_to='+date_to_formatted;
    //     fetch(url)
    //     .then(res=>res.json())
    //     .then((data)=>{
    //         let data_formatted=[];
...
<script>
    $(document).ready(function() {
      ...
      $('input[name="daterange"]').daterangepicker({
        ...
        window.location.href = "<?= base_url('reports/overtimes?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD') + '&search=' + employeeId;
      });
...
function exportData() {
  ...
  // Download Excel file
  XLSX.writeFile(wb, "approved_overtime_report.xlsx");
}
