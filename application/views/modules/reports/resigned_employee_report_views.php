        ...
        <div class="table-responsive" style="max-height: 75vh">
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                <th class="text-left" style="min-width: 100px !important">TERMINITION DATE</th>
            ...
            <tbody id="tbl_application_container">
              <?php if ($EMPLOYEES) {
                foreach ($EMPLOYEES as $employee) { ?>
                  <tr>
                    <td class="text-left"><?= date_format(date_create($employee->termination_date), "d/m/Y") ?></td>
...
<script>
  ...
  hot = new Handsontable(container, {
    data: EMPLOYEES,
    colHeaders: ['ID', 'Termination Date', 'Employee ID', 'Employee Name'],
    rowHeaders: true,
    height: 'auto',
    ...
    var printContents = ` 
        <table>
            <thead>
                <tr>
                  <th colspan="3">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Resigned Employees</h5>
                ...
                <tr>
                <th class="bg-gray-ben">Termination Date</th>
            ...
            <tbody>
                <tr>
                  ${EMPLOYEES.map(item => `
                    <tr>
                      <td>${item.termination_date? formatDateOnly(item.termination_date) : ''}</td>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/resigned_employees?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/resigned_employees?period=') ?>" + $(this).val()
    })
    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/resigned_employees?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/resigned_employees?period=') ?>" + period;
      }
    }
    ...
    //     let url                 ="<?= base_url('reports/get_resigned_employees_data?date_from=') ?>"+date_from_formatted+'&date_to='+date_to_formatted;
    ...
    //         const title = 'Resigned Employees';
    ...
    //         doc.autoTable({
    //             startY: 40,
    //             head: [['Employee ID', 'Employee', 'Termination Date']],
    //             headStyles: {
