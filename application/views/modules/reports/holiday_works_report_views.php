  ...
  <div class="container-fluid p-4">
    ...
    <div class="row my-3">
      ...
      <div class="col-12 col-lg-2 d-flex align-items-end">
        <a href="<?= base_url('reports/holiday_works') ?>" id="btn_clear_filter" class="btn btn-primary">
    ...
    <!-- <div class="card border-0 p-0 m-0">
      <div class="card border-0 py-1 m-0">
        <div>
          <p class=" m-2 bg-primary p-1  text-center ml-auto" style="color: gray;width:150px;border-radius:15px">Showing <?= count($HOLI_WORKS) ?> entries&nbsp;</p>
        </div>
          ...
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                ...
                <th class="text-left" style="min-width: 150px !important">DURATIONS </th>
              </tr>
            </thead>
            <tbody id="tbl_application_container">
              <?php if ($HOLI_WORKS) {
                foreach ($HOLI_WORKS as $work) { ?>
                  <tr>
                    <td class="text-left"><?= date_format(date_create($work->date), "d/m/Y") ?></td>
                    <td class="text-left"><?= $work->col_empl_cmid ?></td>
                    <td class="text-left"><?= $work->fullname ?></td>
                    <td class="text-left"><?= $work->hours . ' hour/s' ?></td>
                  </tr>
                <?php  }
              } else { ?>
...
<script>
  ...
  let HOLI_WORKS = <?= json_encode($HOLI_WORKS); ?>;
  ...
  const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
    ...
    hot = new Handsontable(container, {
      data : HOLI_WORKS,
      colHeaders: ['ID','Date', 'Employee ID', 'Employee Name', 'Durations'],
      rowHeaders: true,
      height: 'auto',
...
<script>
  function printFunction() {
    ...
    var HOLI_WORKS = <?php echo json_encode($HOLI_WORKS); ?>;
    console.log('HOLI_WORKS', HOLI_WORKS);
                ...
                <tr>
                  <th colspan="4">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Approved Holiday Works</h5>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/holiday_works?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    $('select.cut_off_period').on('change', function() {
      window.location.href = "<?= base_url('reports/holiday_works?period=') ?>" + $(this).val()
    })
    $('#daily_add').on('change', function() {
      window.location.href = "<?= base_url('reports/holiday_works?date=') ?>" + $(this).val();
    })
    ...
    function filter_type() {
      ...
      if (filter_type === 'period') {
        ...
        window.location.href = "<?= base_url('reports/holiday_works?period=') ?>" + period;
      }
    }
    ...
    //         const title = 'Approved Holiday Works';
...
<script>
    $(document).ready(function() {
      ...
      $('input[name="daterange"]').daterangepicker({
        ...
        window.location.href = "<?= base_url('reports/holiday_works?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD') + '&search=' + employeeId;
      });
...
function exportData() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Holiday works Report");
  ...
  // Download Excel file
  XLSX.writeFile(wb, "holiday_works_report.xlsx");
}
