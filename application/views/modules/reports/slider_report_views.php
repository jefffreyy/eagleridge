          ...
          <table class="table table-bordered table-hover m-0" id="TableToExport">
            <thead style="position: sticky; top: 0;">
              <tr>
                ...
                <th class="text-left" style="min-width: 100px !important">SHIFT TIME IN</th>
                <th class="text-left" style="min-width: 100px !important">TIME IN</th>
              </tr>
            </thead>
            <tbody id="tbl_application_container">
              <?php if ($EMPLOYEES) {
                foreach ($EMPLOYEES as $employee) { ?>
                  <tr>
                    <td><?= date_format(date_create($employee->date), 'm/d/Y') ?></td>
                    <td><?= $employee->col_empl_cmid ?></td>
                    <td class="text-left"><?= $employee->fullname ?></td>
                    <td><?= $employee->time_regular_start ?></td>
...
const customStyleRenderer_new = function(instance, td, row, col, prop, value, cellProperties) {
  ...
  hot = new Handsontable(container, {
    data : EMPLOYEES,
    colHeaders: ['ID','Date', 'Employee ID', 'Employee Name', 'Shift Time In', 'Time In'],
    rowHeaders: true,
    height: 'auto',
                ...
                <tr>
                  <th colspan="5">
                    <div style="
                      ...
                      <div>
                        <h5 style="margin: 0; padding: 2px 0;">Sliders</h5>
                ...
                <tr>
                  ${EMPLOYEES.map(item => `
                    <tr>
                      ...
                      <td>${item.time_regular_start? item.time_regular_start : ''}</td>
...
<script>
  $(document).ready(function() {
    $('input[name="daterange"]').daterangepicker({
      ...
      window.location.href = "<?= base_url('reports/sliders?date_from=') ?>" + start.format('YYYY-MM-DD') + '&date_to=' + end.format('YYYY-MM-DD');
    });
    ...
    //   let url="<?= base_url('reports/get_slider_data?date_from=' . date_format(date_create($START_DATE), 'Y-m-d') . '&date_to=' . date_format(date_create($END_DATE), 'Y-m-d')) ?>"
    ...
    //           name,
    //            data[i].time_regular_start,data[i].time_in,
    ...
    //         startY: 40,
    //         head: [
    //           ['Date', 'Employee', 'Shift Time In','Time In']
    //         ],
    //         headStyles: {
...
function exportData() {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Slider Report");
  ...
  // Download Excel file
  XLSX.writeFile(wb, "slider_report.xlsx");
}
