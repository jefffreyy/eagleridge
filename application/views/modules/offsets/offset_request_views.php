...
<body>
  ...
  <div class="container-fluid p-4">
    ...
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title"><a href="<?=base_url('offsets')?>"><i
              class="fa-duotone fa-circle-left"></i></a>&nbsp;Offset Requests<h1> 
      </div>
      ...
      <div class="col-md-6 button-title">
        ...
        <!--</form>-->
        <a href="<?=base_url('offsets/request_offset')?>"
          class=" btn technos-button-green shadow-none rounded"><i class="fas fa-plus"></i>&nbsp;Add Request</a>
...
<?php endforeach ?>
        ...
        <div class="col-md-2">
            ...
            <a href="<?=base_url('leaves/leave_lists')?>" id="btn_clear_filter"
...
<?php if($OFFSETS) {  ?>
    <?php foreach($OFFSETS as $offset) { ?>
              <tr data-offset_id="<?=$offset->id?>"  data-toggle="modal" data-target="#modal_approval">
                ...
                <td class="text-center" style="width:15%"><?=$offset->type?></td>
                <td class="text-center" style="width:15%"><?=date("d/m/Y", strtotime($offset->offset_date))?></td>
...
<?php }
    elseif($offset->status == "Withdrawed"){ ?>
...
<?php } ?>
                ...
                <td class="text-center">
                   <a class = "select_row p-2"  style ="color: gray; cursor: pointer; !important"  row_id="56" data-offset_id="<?=$offset->id?>"  data-toggle="modal" data-target="#modal_approval">
                       <i class="far fa-eye"></i></a> 
<?php if($offset->status=='Pending 1' ) { ?>
                    <a class = "select_edit_row p-2 edit_data_id"  href="<?=base_url('leaves/edit_leaves/'.$offset->id)?>"  style ="color: gray; cursor: pointer; !important" row_id="56">
    ...
    <script>
        $(document).ready(function() {
           ...
           $('#modal_approval').on('show.bs.modal', function (e) {
                ...
                var offset_id    = button.data('offset_id');  
                $.get("<?=base_url('offsets/get_offset_approval_status')?>"+'/'+offset_id,function(res){
            ...
            $('a.paginate').on('click',function(){
                ...
                window.location.href="<?=base_url('leaves/leave_lists')?>"+$(this).attr('href')+filter_url;
                return false;
            })
            function reloadPage(){
                var status      = $('.leave_status').val();
                ...
                var line        = $('#filter_by_line').val();
                var filter_url  = '&company='+company+'&branch='+branch+'&dept='+department+'&div='+division+'&section='+section+'&group='+group+'&team='+team+'&line='+line;
                window.location.href="<?=base_url('leaves/leave_lists')?>"+"?status="+status+"&page="+page+"&row="+row+'&search='+search+filter_url;
            }
