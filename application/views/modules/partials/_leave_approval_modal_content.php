    ...
    <div class="modal-body position-relative">
        <div class="mb-3">
            <div class="row">
                ...
                <div class="col-4 m-0">
                    <p class="m-0"><?= date(($DATE_FORMAT) ? $DATE_FORMAT : "m/d/Y", strtotime($LEAVE->leave_date)) ?></p>
                </div>
                <div class="col-2 m-0">
                ...
                <tbody>
                    <?php if ($tableData) {  ?>
                        <?php foreach ($tableData as $leave) { ?>
                            <tr>
                                <td><?= date_format(date_create($leave->leave_date), ($DATE_FORMAT) ? $DATE_FORMAT: "d/m/Y")?></td>
...
<?php $index--; } ?>
        <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center">
                <img src="<?= set_profile($LEAVE, 'employee') ?>" class="image_profile rounded-circle elevation-2" width='50' height='50' />
                <div class="ml-2">
                    <p class="p-0 m-0"><?= $LEAVE->status == 'Withdrawed' ? 'Withdrawed by' : 'Requested by' ?></p>
                    <p class="p-0 m-0"><?= $LEAVE->employee ?></p>
                    <p class="p-0 m-0"><?= $LEAVE->employee_email ?></p>
                </div>
            </div>
            <div>
                <span><?= date_format(date_create($LEAVE->create_date), ($DATE_FORMAT) ? $DATE_FORMAT: "d/m/Y" . ' H:i:s A') ?></span>
            </div>
        </div>
    </div>
    ...
    <form action="<?= base_url('selfservices/update_leave_assign') ?>" method="POST" id="form_approved">
        ...
        <input type="hidden" name="employee" value="<?= $LEAVE->employee ?>" />
        <input type='hidden' name="approver_1" value="<?= $LEAVE->approver1 ?>">
        <input type='hidden' name="approver_2" value="<?= $LEAVE->approver2 ?>">
        <input type='hidden' name="approver_3" value="<?= $LEAVE->approver3 ?>">
        <input type="hidden" name="empl_id" value="<?= $LEAVE->empl_id ?>">
        <input type="hidden" name="approver_1a" value="<?= $LEAVE->approver_1a ?>">
        <input type="hidden" name="approver_1b" value="<?= $LEAVE->approver_1b ?>">
        <input type="hidden" name="approver_2a" value="<?= $LEAVE->approver_2a ?>">
        <input type="hidden" name="approver_2b" value="<?= $LEAVE->approver_2b ?>">
        <input type="hidden" name="approver_3a" value="<?= $LEAVE->approver_3a ?>">
        <button class="btn btn-primary approve_btn" type="button" approved_id="<?= $LEAVE->id; ?>" <?=$btn_status?> >Approve</button>
    </form>
    <form action="<?= base_url('selfservices/reject_leave_assign') ?>" method="POST" id="form_reject">
        <input type="hidden" name="id" value="<?= $LEAVE->id ?>">
        <input type='hidden' name="approver_1" value="<?= $LEAVE->approver1 ?>">
        <input type='hidden' name="approver_2" value="<?= $LEAVE->approver2 ?>">
        <input type='hidden' name="approver_3" value="<?= $LEAVE->approver3 ?>">
        ...
        <button class="btn btn-danger reject_btn" type="button" reject_id="<?= $LEAVE->id?>" <?=$btn_status?> >Reject</button>
    </form>
