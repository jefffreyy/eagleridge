...
<div class="modal-content">
    <div class="modal-header">
        <h5 class="modal-title">Leave Request (<?= 'LEA' . str_pad($LEAVE->id, 5, '0', STR_PAD_LEFT) ?>)</h5>
...
<?php if (
    ...
    ($user_access == 2 || !in_array($LEAVE->status, $restrictedStatuses))
) : ?>

    <form action="<?= base_url('selfservices/cancel_approved_leave') ?>" method="POST" class="form_cancel">
        <input type="hidden" name="rowId" value="<?= $LEAVE->id ?>"/>
