        ...
        <div class="container-fluid p-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Setup Organizational Chart</li>
                </ol>
            </nav>

            <div class="row  pt-1">
                <div class="col-md-6 d-flex align-items-center">
                    <a href="javascript:history.go(-1);"><i class="h4 pb-0 mb-0 mr-3 fa-duotone fa-circle-left"></i></a>
                    <h1 class="page-title d-inline mb-0">Setup Organizational Chart</h1>
                </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-2">
                    ...
                    <a href="<?= base_url() . 'companies/setup_organization' ?>" id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
                </div>
            </div>
            <?php
            ?>
                                ...
                                foreach ($DISP_EMP_LIST as $DISP_EMP_LIST_ROW) { ?>
                                    <tr>
                                        <td class="text-center" id="select_item">
                                            <input type="checkbox" empl_id="<?= $DISP_EMP_LIST_ROW->id ?>" name="brand" class="check_single" empl_name="<?= $DISP_EMP_LIST_ROW->col_empl_cmid . ' - ' . $DISP_EMP_LIST_ROW->col_last_name . ', ' . $DISP_EMP_LIST_ROW->col_frst_name ?>" value="<?= $DISP_EMP_LIST_ROW->id ?>">
                                        </td>
                                        ...
                                        <td>
                                            <select class="employees custom-select" employee="<?= $DISP_EMP_LIST_ROW->id ?>">
                                                <option value="">None</option>
                                                <?php foreach ($ALL_EMPLOYEES as $Employee) { ?>
                                                    <?php if ($Employee->id != $DISP_EMP_LIST_ROW->id) { ?>
                                                        <option value='<?= $Employee->id ?>' <?= $Employee->id == $DISP_EMP_LIST_ROW->reporting_to ? 'selected' : '' ?>>
    ...
    <div class="modal fade  class_modal_update_list" id="modela_update" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id='update_form' action="<?= base_url() . 'companies/update_organization'; ?>" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group ">
                                    <label class="required" for="UPDT_ALLOWANCE_TYPE">Assign to</label>
                                    <select name="reporting_to" id="reporting_to" class="form-control  w-100 border border-danger" style='width:100%'>
                                        <?php
                                        foreach ($ALL_EMPLOYEES as $DISP_EMP_LIST_ROW) {
                                        ?>
                                            <option value="<?= $DISP_EMP_LIST_ROW->id ?>">
                                                <?= $DISP_EMP_LIST_ROW->col_empl_cmid . ' - ' . $DISP_EMP_LIST_ROW->col_last_name . ', ' . $DISP_EMP_LIST_ROW->col_frst_name ?>
                                            </option>
                                        <?php
                                        }
    ...
    <script>
        $(document).ready(function() {
            ...
            $('select.employees').on('change', function() {
                ...
                $('select#reporting_to').val($(this).val());
                $('#update_form').submit();
            })
            ...
            function filter_data(page_row) {
                ...
                window.location = base_url + "companies/setup_organization" + page_row + "&branch=" + branch +
            ...
            $('#update').click(function() {
                let selected_id = [];
                let att_empl_names = [];
                let user_id = "<?= $this->session->userdata('SESS_USER_ID') ?>";
                $('#UPDATE_ID').empty();
