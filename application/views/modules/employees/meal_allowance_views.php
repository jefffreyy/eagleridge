...
<?php
(isset($_GET['cutoff'])) ? $cutoff = $_GET['cutoff'] : $cutoff = "";
...
<body>
    <div class="content-wrapper">
        <div class="container-fluid p-4">
            ...
            <div class="row  pt-1"> <!-- Title starts -->
                <div class="col-md-6">
                    <h1 class="page-title"><a href="<?= base_url() . 'employees'; ?>"><i class="fa-duotone fa-circle-left"></i></a>&nbsp;Meal Allowance</h1>
                </div>
            ...
            <div class="row mb-4">
                <div class="col-md-2">
                    ...
                    <select name="cutoff_period" id="filter_by_cutoff" class="form-control">
                        <?php
                        if ($DISP_CUTOFF_PERIOD) {
                        ?>
                            <option value="all">All Cut-off Period</option>
                            <?php
                            foreach ($DISP_CUTOFF_PERIOD as $DISP_CUTOFF_PERIOD_ROW) {
                                if ($DISP_CUTOFF_PERIOD_ROW->name != '') {
                            ?>
                                    <option value="<?= $DISP_CUTOFF_PERIOD_ROW->id ?>" <?php echo $cutoff == $DISP_CUTOFF_PERIOD_ROW->id ? 'selected' : '' ?>>
                                        <?= $DISP_CUTOFF_PERIOD_ROW->name ?>
                                    </option>
                        <?php
                                }
                            }
                ...
                <div class="col-md-2" <?php echo ($DISP_VIEW_BRANCH ? "" : "hidden") ?>>
                    ...
                    <select name="branch" id="filter_by_branch" class="form-control">
                        <?php
                ...
                <div class="col-md-2">
                    ...
                    <a href=<?= base_url() . "employees/meal_allowance" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered m-0" id="TableToExport" style="width:100%">
                            <thead>
                                ...
                                <th class="text-center " style="background:#f6d6ff;">Approved Hours OT</th>
                            </thead>
                                        ...
                                        <tr>
                                            ...
                                            if ($DISP_APPROVED_OT) {
                                                foreach ($DISP_APPROVED_OT as $DISP_APPROVED_OT_ROW) {
                                                    if ($DISP_EMP_LIST_ROW->id == $DISP_APPROVED_OT_ROW[0]['empl_id']) {
                                                        $total_hours = $DISP_APPROVED_OT_ROW[0]['total_hours'];
                                                    }
                                                }
                                            }
                                            ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#filter_by_cutoff").on("change", function() {
                filter_data();
            })
            ...
            function filter_data(page_row) {
                ...
                let cutoff_period = $("#filter_by_cutoff").find(":selected").val();
                ...
                window.location = base_url + "employees/meal_allowance" + page_row + "&branch=" + branch +
                    "&dept=" + department + "&division=" + division +
                    "&section=" + section + "&group=" + group +
                    "&team=" + team + "&line=" + line + "&cutoff=" + cutoff_period;
            }
