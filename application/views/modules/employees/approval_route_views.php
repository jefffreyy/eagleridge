...
<body>
                  ...
                  <div class="col-md-12 col-lg-12">
                    <div class="d-flex row justify-content-between align-items-center">
                      <div class="row d-flex justify-content-center justify-content-lg-start col-12 col-lg-6 mx-1">
                        ...
                        <a href="<?= base_url('employees/approval_routes') ?>" id="btn_clear_filter" class="btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="" />&nbsp;Clear</a>
                      </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      function filter_data(page_row) {
        ...
        window.location = base_url + "employees/approval_routes" + filterUrl;
      }
  ...
  <script>
    ...
    (function() {
      ...
      var auto_approve = <?php echo json_encode($auto_approve); ?>;
      var employeeList = <?php echo json_encode($DISP_EMPLOYEES_NONFILTERED); ?>;
      var approver_num = <?= $NUM_APPROVERS ?>;
      ...
      let columnTable = [];
      for (let i = 1; i <= approver_num; i++) {
        headerTable.push({
          label: 'Approver ' + i,
          colspan: 2
        })
        subHeaderTable.push(`Approver ${i}A`);
        subHeaderTable.push(`Approver ${i}B`);
        columnTable.push({
          data: `approver_${i}a_name`,
          readOnly: false,
          type: 'dropdown',
          filter: true,
          source: employeeIds,
      ...
      // console.log('columnTable',columnTable); return;
      initializeHandsontable(data, columnTable);

      function initializeHandsontable(data, columnData = []) {
    ...
    hot.updateSettings({
      ...
      update_data.addEventListener('click', function() {
        ...
        updatedData.map((item) => {
          ...
          if (item[8]) {
            const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[8]);
            if (check) {
              item[8] = check.cmid;
            } else {
              invalidId = true
          ...
          if (item[9]) {
            const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[9]);
            if (check) {
              item[9] = check.cmid;
            } else {
              invalidId = true
          ...
          if (item[10]) {
            const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[10]);
            if (check) {
              item[10] = check.cmid;
            } else {
              invalidId = true
          ...
          if (item[11]) {
            const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[11]);
            if (check) {
              item[11] = check.cmid;
            } else {
              invalidId = true
          ...
          if (item[12]) {
            const check = employeeIdsCopywithCMID.find(obj => obj.employeeNameWithCMID === item[12]);
            if (check) {
              item[12] = check.cmid;
            } else {
              invalidId = true
