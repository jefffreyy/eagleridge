      ...
      <div class="card border-0 p-1 m-0">
        <div class=" py-3 w-25">
          ...
          <select class="form-control" id="table-id" onchange="pageTable(this.value)">
            ...
            <option value="Emergency Contacts" selected>Emergency Contacts</option>
          </select>
        </div>
...
<script>
  ...
  var dataCopy = null;
  var deletedId = [];
  const apiUrl = url + 'employees/get_tableplus_emergency_contacts_all';
  fetch(apiUrl
  ...
  function initializeHandsontable(data) {
    ...
    hot = new Handsontable(container, {
      data,
      colHeaders: ['ID', 'Employee ID', 'Name', 'Relationship', 'Mobile Number', 'Work Phone', 'Home Phone', 'Current Address'],
      rowHeaders: true,
      height: 'auto',
  ...
  update_data.addEventListener('click', function() {
    ...
    const apiUrl = url + 'employees/update_emergency_contacts_all';
    const data = {
      updatedData,
      deletedId
    };
