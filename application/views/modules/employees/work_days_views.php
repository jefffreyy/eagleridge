...
<body>
        ...
        <div class="container-fluid p-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Employee Work Days
                    </li>
                </ol>
            </nav>
            ...
            <div class="row  pt-1">
                <div class="col-md-6">
                    <h1 class="page-title d-flex align-items-center"><a href="<?= base_url() . 'employees'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
                        </a>&nbsp;Employee Work Days</h1>
                </div>
                <div class="col-md-6 button-title d-flex justify-content-end">
                    <button class="btn btn-primary" id="btn-update"><img style="width: 16px; height: 16px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-arrow-up-sharp-solid.svg') ?>" alt="" /></i>&nbsp;Update</button>
                </div>
            </div>
            <hr>
            ...
            <div class="filter-container <?= $filter ? 'visible' : '' ?>">
                <div class=" mb-4 d-flex row ">
                    ...
                    <div class="col-md-2" <?php echo (true ? "" : "hidden") ?>>
                        ...
                        <select name="year" id="filter_by_year" class="form-control">
                            <?php
                            if ($DISP_YEARS) {

                                foreach ($DISP_YEARS as $DISP_DISTINCT_LINE_ROW) {
                                    if ($DISP_DISTINCT_LINE_ROW->name != '') {
                            ?>
                                        <option value="<?= $DISP_DISTINCT_LINE_ROW->id ?>" <?php echo $year == $DISP_DISTINCT_LINE_ROW->id ? 'selected' : '' ?>><?= $DISP_DISTINCT_LINE_ROW->name ?></option>
                            <?php
                                    }
                                }
                            }
                    ...
                    <!-- <div class="col-md-2">
                        ...
                        <a href=<?= base_url() . "employees/work_days" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
                    </div> -->
                                ...
                                <div class="row">
                                    <div class="col-md-12 col-lg-12">
                                        <div class="d-flex row justify-content-between mx-1">
                                            <div class="form-group row d-flex col-12 col-lg-5 ">
                                                <label class="col-md-12 mb-1 p-0 text-secondary">Search Employee</label>

                                                <select id="search_select" class="px-1 col-12 col-md-5 employee_select form-control w-50">
                                                    <?php
                                                ...
                                                <a href="<?= base_url('employees/work_days') ?>" id="btn_clear_filter" class="btn btn-primary mx-1"><img src="<?= base_url('assets_system/icons/clear_filter.svg') ?>" alt="">&nbsp;Clear</a>
                                            </div>

                                            <div class="col-12 col-lg-6 d-flex justify-content-end align-items-center mr-3">
                                                <div class="row">
                                                    <div class="d-flex col-12 col-lg-11  align-items-center row">
                                                        <p class="p-0 m-0 d-inline col-5" style="color: gray">Showing <?= $low_limit ?> to <?= $high_limit ?> of <?= $C_DATA_COUNT ?> entries&nbsp;</p>
                                                        <ul class="d-inline-flex flex-nowrap pagination m-0 p-0 col-7">
    ...
    <script>
        ...
        var data = [];
        var deepCopy = [];
        var colHeaders = ['id', 'Employee', 'Days', ];
        var numColumns = colHeaders.length;
        if (dispEmpList !== null && dispEmpList !== undefined && Array.isArray(dispEmpList) && dispEmpList.length > 0) {
            data = dispEmpList;
            deepCopy = JSON.parse(JSON.stringify(dispEmpList));
        }

        console.log(dispEmpList);
        console.log(DISP_YEARS);
        ...
        hot = new Handsontable(container, {
            ...
            afterRender: function() {
                if (data.length === 0) {
                    ...
                    noRecordsCell.setAttribute('colspan', numColumns);
                    noRecordsCell.innerHTML = 'No records';
                    noRecordsCell.style.textAlign = 'center';
                    noRecordsRow.appendChild(noRecordsCell);

                    var tbody = container.querySelector('.htCore tbody');
        ...
        update.addEventListener('click', function() {
            ...
            function validatePositiveFloat(row, rowIndex, tableColumn, title) {
                if (!row) {
                    if (messageError) {
                        messageError = `${messageError}. Empty Days in row ${rowIndex+1}`;
                    } else {
                        messageError = `Empty Days in row ${rowIndex+1}`;
                    }
                } else if (isNaN(row)) {
                    if (messageError) {
                        messageError = `${messageError}. Invalid Number on row ${rowIndex+1}`;
                    } else {
                        messageError = `Invalid Days Number in row ${rowIndex+1}`;
                    }

                } else if (row < 1) {
                    if (messageError) {
                        messageError = `${messageError}. Zero or Negative is not valid on Days in row ${rowIndex+1}`;
                    } else {
                        messageError = `Zero or Negative is not valid on Days in row ${rowIndex+1}`;
                    }
            ...
            for (var rowIndex = 0; rowIndex < updatedData.length; rowIndex++) {
                ...
                validatePositiveFloat(row[2], rowIndex, null, 'Days');
            }
            ...
            let changes = [];
            const year = $("#filter_by_year").find(":selected").val();
            console.log('year', year);
            for (var i = 0; i < deepCopy.length; i++) {
                if (deepCopy[i].days != data[i].days) {
                    changes.push({
                        id: deepCopy[i].id,
                        days: data[i].days,
                        year
            ...
            fetch(url + 'employees/update_work_days', {
                    method: 'POST',
                    headers: {
    ...
    <script>
        $(document).ready(function() {
            ...
            function filter_data(page_row) {
                ...
                let year = $("#filter_by_year").find(":selected").val();
                let search = $("#search_select").find(":selected").val();

                filterUrl = page_row + "&branch=" + branch + "&dept=" + department + "&division=" + division +  "&clubhouse=" + clubhouse +
                    "&section=" + section + "&group=" + group + "&team=" + team + "&line=" + line + "&year=" + year + "&search=" + search.replace(/\s/g, '_');

                if (document.querySelector('.filter-container').classList.contains('visible')) {
                    filterUrl = filterUrl + '&filter=1';
                }
                window.location = base_url + "employees/work_days" + filterUrl;
            }
