        ...
        <div class="container-fluid p-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Non-Taxable Allowance
                    </li>
                </ol>
            </nav>
            ...
            <div class="row  pt-1"> 
                <div class="col-md-6">
                    <h1 class="page-title"><a href="<?= base_url() . 'employees'; ?>"><i class="fa-duotone fa-circle-left"></i></a>&nbsp;Non-Taxable Allowance</h1>
                </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-2">
                    ...
                    <a href=<?= base_url() . "employees/non_taxable_allowance_assign" ?> id="btn_clear_filter" class="col btn btn-secondary mx-1">Clear Filter</a>
                </div>
            </div>
                        ...
                        <table class="table table-bordered m-0" id="TableToExport" style="width:100%">
                            <thead>
                                ...
                                <?php foreach ($DISP_ALLOWANCE_TYPES as $DISP_ALLOWANCE_TYPES_ROW) { ?>
                                    <th class="text-center" <?= ($DISP_ALLOWANCE_TYPES_ROW->type == "Attendance") ? 'style="background:#f6d6ff;"' : 'style="background:#feffd6;"'  ?>><?= $DISP_ALLOWANCE_TYPES_ROW->name ?> <br> <?= $DISP_ALLOWANCE_TYPES_ROW->type ?></th>
                                <?php } ?>
                            </thead>
                                                ...
                                                if ($DISP_ALLOWANCE) {
                                                    foreach ($DISP_ALLOWANCE as $DISP_ALLOWANCE_ROW) {
                                                        ...
                                                        if ($DISP_ALLOWANCE_ROW->username == $DISP_EMP_LIST_ROW->id && $DISP_ALLOWANCE_TYPES_ROW->id == $DISP_ALLOWANCE_ROW->name) {
                                                            $val = number_format((float)$DISP_ALLOWANCE_ROW->value, 2, '.', '');
                                                            break;
                                                        }
                                                    }
                                                }
                                            ...
                                            ?>
                                                <td style="padding: 1px !important;">
                                                    <form class="bg-transparent">
                                                        ...
                                                        <input type="text" class="form-control allowance_non_tax_val" id="allowance_non_tax_val" name="allowance_non_tax_val" step="0.01" style="width:100%;" default=<?= $val ?> value=<?= $val ?>>
                                                    </form>
                                                </td>
                                            <?php } ?>
                                        </tr>
    ...
    <div class="modal fade  class_modal_update_list" id="modela_update" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form action="<?= base_url() . 'employees/update_allowance_nontax'; ?>" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="required" for="UPDT_ALLOWANCE_TYPE">Allowance Type</label>
    ...
    <script>
        $(document).ready(function() {
            ...
            function filter_data(page_row) {
                ...
                window.location = base_url + "employees/non_taxable_allowance_assign" + page_row + "&branch=" + branch +
            ...
            $('.allowance_non_tax_val').on('focusout', function(event) {
                    event.preventDefault();
                    let defaultVal    = isNaN(parseFloat($(this).attr('default'))) ? 0 : parseFloat($(this).attr('default'));
                    let employee      = $(this).siblings("#employee_data").val();
                    let type          = $(this).siblings("#type_data").val();
                    let allowance_val = isNaN(parseFloat($(this).val())) ? 0 : parseFloat($(this).val());
                    let year          = $("#filter_year").find(":selected").val();
                    if (defaultVal != allowance_val) {
                        window.location = base_url + "employees/process_allowance_assigning_nontax/" + employee + "/" + allowance_val + "/" + year + "/" + type;
                    }
                }),

                $('.allowance_non_tax_val').on('keydown', function(event) {
                    if (event.key === "Enter") {
                        event.preventDefault();
                        let defaultVal = isNaN(parseFloat($(this).attr('default'))) ? 0 : parseFloat($(this).attr('default'));
                        let employee = $(this).siblings("#employee_data").val();
