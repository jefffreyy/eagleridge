...
<body>
      ...
      <div class="row pt-1">
        <div class="col-md-6">
          <h1 class="page-title d-flex align-items-center"><a onclick="afterRenderFunction()" href="<?= base_url('requests') ?>"> <img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            ...
            </a>&nbsp;My Exempt Undertime Request<h1>
        </div>
        ...
        <div class="col-md-6 button-title">
          ...
          <a onclick="afterRenderFunction()" href="<?= base_url('requests/request_exemptut') ?>" class=" btn btn-primary shadow-none rounded d-flex align-items-center"><img src="<?= base_url('assets_system/icons/plus-solid.svg') ?>" alt="">
            &nbsp;Add Request</a>
          ...
          <div class="table-responsive">
            ...
            <table class="table table-bordered m-0" id="table_main" style="width:100%">
              <thead>
                ...
                <th style='width:15%;'>SHIFT OUT</th>
                <th style='width:15%;'>ACTUAL OUT</th>
                ...
                <?php if ($TABLE_DATA) {  ?>
                  <?php foreach ($TABLE_DATA as $row_data) {
                    ...
                    <tr>
                      ...
                      <td><?= $row_data->shift_out ?></td>
                      <td><?= $row_data->actual_out ?></td>
                      <td><?= $row_data->reason ?></td>
                      <!-- <td class="text-center"><?= $row_data->comment ?></td> -->
                      <td class="text-center" data-id="<?= $row_data->id ?>" data-toggle="modal" data-target="#modal_approval">
                        <?php
                      ...
                      <td class="text-center">
                        ...
                        if ($row_data->status == 'Pending 1' || $row_data->status == 'Pending 2' || $row_data->status == 'Pending 3' || $row_data->status == 'Approved') { 
                                                     
                          ?>
                          <a href="<?= base_url('selfservices/exemptut_withdraw_admin/'.$row_data->id) ?>" class="btn btn-default withdraw_req">Withdraw</a>
                        <?php } ?>
                      </td>
                    </tr>
                  <?php } ?>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#modal_approval').on('show.bs.modal', function(e) {
        ...
        $.get("<?= base_url('selfservices/get_exemptut_status') ?>" + '/' + id, function(res) {
      ...
      function reloadPage() {
        ...
        var page = "<?= $PAGE ?>";
        window.location.href = "<?= base_url('selfservices/exemptut') ?>" + "?status=" + status + "&page=" + page + "&row=" + row;
      }
    })
  </script>
  ...
  <script>
    ...
    // Add headers (skip first hidden column and last ACTION column)
    var headers = [];
    $('#table_main thead th').each(function(index) {
        // Skip first column (index 0) and last column (index 7)
        if(index !== 0 && index !== 7) {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Exempt Undertime");
    
    // Export
    XLSX.writeFile(wb, "My_Exempt_Undertime.xlsx");
});
  </script>
  <script>
