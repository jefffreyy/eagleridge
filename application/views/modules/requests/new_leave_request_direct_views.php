        ...
        <div class="p-3">
            <div class="flex-fill">
                <div class="row pr-3 mb-3">
                    <div class="col">
                        <h1 class="page-title"><a href="<?= base_url() . 'requests/leave_lists_multi'; ?>"><img style="width: 24px; height: 24px; margin-bottom: 3px;" src="<?= base_url('assets_system/icons/circle-left-duotone.svg') ?>" alt="" />
            </a>&nbsp;Add Leaves<h1>
                    </div>
    ...
    <script>
        ...
        insert.addEventListener('click', function() {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                <?php if($isLeaveHours == 1){?>
                    if (data[i].duration) {
                        if (!(!isNaN(data[i].duration) && Number(data[i].duration) > 0)) {
                            leaveDurationInvalid = `Leave Duration is Invalid.`;
                            valid = false;
                        }
                    } else {
                        leaveDurationInvalid = `Leave Duration is Empty.`;
                        valid = false;
                    }
                ...
                //     valid = false;
                // }
                const typeObject = leaveTypesRaw.find(obj => obj.name === data[i].type);
                if (!typeObject) {
                    leaveDurationInvalid = `Invalid Type.`;
                    valid = false;

                    console.log('data[i].type !type',data[i]);
                }else{
                    temp.type = typeObject.id; 
                }
                if (valid) {
                    <?php if($isLeaveHours == 0){?>
                        temp.duration = temp.duration * 8;
                    <?php } ?>
            ...
            if (changesFinal.length < 1) {
                $(document).Toasts('create', {
                    ...
                    title: 'Warning!',
                    subtitle: 'close',
                    body: "No rows with valid Employee, Leave Date, Leave Type and Leave Duration"
                });
                return;
            }
