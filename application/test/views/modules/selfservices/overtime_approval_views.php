  ...
  <div class="container-fluid p-4">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Overtime Approval
        </li>
      </ol>
    </nav>
    <div class="row pt-1">
      <div class="col-md-6">
        <h1 class="page-title">Overtime Approval<h1>
      </div>
          ...
          <table class="table table-hover m-0" id="TableToExport" style="width:100%">
            <thead>
              <tr>
                ...
                <th class="text-center">Date OT</th>
              ...
              <?php if ($DISP_OVERTIME) {
                foreach ($DISP_OVERTIME as $DISP_OVERTIME_ROW) { ?>
                  <tr>
                    ...
                    <td class="text-center">OVA<?= str_pad($DISP_OVERTIME_ROW->id, 5, '0', STR_PAD_LEFT)  ?></td>
                    <td class="text-center"><?= convert_cmid($DISP_EMPLOYEES, $DISP_OVERTIME_ROW->empl_id) ?> - <?= convert_data($DISP_EMPLOYEES, $DISP_OVERTIME_ROW->empl_id) ?></td>

                    <td class="text-center"> <?= $DISP_OVERTIME_ROW->type ?></td>
                    <td class="text-center"> <?= $DISP_OVERTIME_ROW->date_ot ?> </td>
                    <td class="text-center"> <?= $DISP_OVERTIME_ROW->hours ?></td>
                    <td class="text-center"> <?= $DISP_OVERTIME_ROW->status ?></td>



                    <td style="width:15%" class="text-center">
                      <a class="select_row p-1 request_id btn btn-warning" href="<?= base_url() ?>selfservices/get_specific_overtime_request/<?= $DISP_OVERTIME_ROW->id ?>" id="view" data-toggle="modal" data-target="#modal_leave_request" leave_id="<?= $DISP_OVERTIME_ROW->id; ?>"><i class="far fa-eye"></i></a>
                      <button class="select_edit_row p-1 approved_data btn btn-success" href="" approved_id="<?= $DISP_OVERTIME_ROW->id; ?>" row_id=""><i class="fas fa-check-circle" id="btn_approved"></i></button>
                      <button class="select_edit_row p-1 reject_data btn btn-danger" href="" reject_key="<?= $DISP_OVERTIME_ROW->id; ?>" row_id=""><i class="fas fa-times-circle" id="btn_reject"></i></button>
        ...
        <div class="modal-body">
          <div class="row">
            ...
            <div class="col-md-12">
              ...
              <div class="form-group">
                <label class="required " for="date_ot">Date OT
                </label>
                <input type="text" class="form-control" name="date_ot" id="date_ot" readonly>
              </div>
              <div class="form-group">
...
<script>
  $(document).ready(function() {
    ...
    function filter_data(page_row) {
      ...
      window.location = base_url + "selfservices/overtime_approval"+page_row;
    }
    ...
    $('.reject_data').click(function(e) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = "<?= base_url(); ?>selfservices/reject_overtime_assign?reject_id=" + reject_key;
        }
      })
    })
    ...
    $('.approved_data').click(function(e) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = "<?= base_url(); ?>selfservices/update_overtime_assign?approved_id=" + approved_id;
        }
      })
    })
