...
ob_start();
class rewardsandrecognitions extends CI_Controller
{
  ...
  function index(){
    // $data['DISP_ALL_DATA']  = $this->reimbursement_model->MOD_DISP_ALL_REQUEST();
    // $data['DISP_ROW_COUNT'] = $this->reimbursement_model->MOD_DISP_ALL_DATA_COUNT(); 
    $data["Modules"]=  array( 
      array("title"=>"Service Awards",       "value"=>"Service Awards", "icon"=>"fas fa-trophy",       "url"=>"rewardsandrecognitions/service_awards",        "access"=>"Rewards"),
      array("title"=>"Awards List",          "value"=>"Awards List",    "icon"=>"far fa-list-alt",     "url"=>"rewardsandrecognitions/awards_lists",          "access"=>"Rewards"),
      array("title"=>"Award Types",          "value"=>"Award Types",    "icon"=>"far fa-hand-pointer", "url"=>"rewardsandrecognitions/awards_types",          "access"=>"Rewards"),
      array("title"=>"Certificate Templates","value"=>"Award Types",    "icon"=>"fas fa-certificate",  "url"=>"rewardsandrecognitions/certificate_templates", "access"=>"Rewards"),
    );
    $data["title_page"]                 = "Rewards";
  ...
  function service_awards(){
  }
  function awards_lists(){
    ...
    $data["module_name"]      = $module       = 'rewardsandrecognitions';
    $data["page_name"]        = $page_name    = 'supports';
    $data["model_name"]       = $model        = "main_table_01_model"; 
    $data["table_name"]       = $table        = "tbl_hr_supports";
    $data["module"]           = ["https://testhr.technos.app/rewardsandrecognitions","Rewards and Recognitions", "Awards Lists"];         // Main Menu Path, Module, Page Title
    $data["id_prefix"]        = "WRN";
    $data["excel_output"]     = [true,"awards_lists.xlsx"];                                                       // Enable, File Name
    $data["add_button"]       = [true,"Add Award"];                                                                 // Enable, Button Name modal_add_enable   = true;
  ...
  function awards_types(){
    ...
    $data["model_name"]       = $model        = "main_table_01_model"; 
    $data["table_name"]       = $table        = "tbl_hr_supports";
    $data["module"]           = ["https://testhr.technos.app/rewardsandrecognitions","Rewards and Recognitions", "Award's Types"];         // Main Menu Path, Module, Page Title
    $data["id_prefix"]        = "WRN";
    $data["excel_output"]     = [true,"awards_types.xlsx"];                                                       // Enable, File Name
    $data["add_button"]       = [true,"Add Award's Type"];                                                                 // Enable, Button Name modal_add_enable   = true;
    $data["status_text"]      = ["Active","Inactive","","Pending"];                                                          //Green, Red, Orange, Gray
    $data["C_ROW_DISPLAY"]    = $filter_row = [10,25,50,100];
    $c_data_tab             = array(
                              array("All","status","All",0),
  ...
  function certificate_templates(){
    //------------------------------------------------------- START OF DYNAMIC PARAMETERS
    $user                     = $this->session->userdata('SESS_USER_ID');
    $data["view_type"]        = $view_type    = ['all','edit_user',$user]; //all or user
    $data["module_name"]      = $module       = 'rewardsandrecognitions';
    ...
    $data["model_name"]       = $model        = "main_table_01_model"; 
    $data["table_name"]       = $table        = "tbl_hr_supports";
    $data["module"]           = ["https://testhr.technos.app/rewardsandrecognitions","Rewards and Recognitions", "Certificate Templates"];         // Main Menu Path, Module, Page Title
    $data["id_prefix"]        = "WRN";
    $data["excel_output"]     = [true,"certificate_templates.xlsx"];                                                       // Enable, File Name
    $data["add_button"]       = [true,"Add Certificate Template"];                                                                 // Enable, Button Name modal_add_enable   = true;
    $data["status_text"]      = ["Active","Inactive","","Pending"];                                                          //Green, Red, Orange, Gray
    $data["C_ROW_DISPLAY"]    = $filter_row = [10,25,50,100];
    $c_data_tab             = array(
                              array("All","status","All",0),
  ...
  function add_data(){
    $data["model_name"]                       = $model  = "main_table_01_model";
    $this->logger->log_activity($this->session->userdata('SESS_USER_ID'), 'Added rewards and recognition record');
    $this->load->view('templates/header');
  ...
  function edit_row(){
    ...
    $this->logger->log_activity($this->session->userdata('SESS_USER_ID'), 'Edited rewards record');
  ...
  function add_row(){
    ...
    $this->logger->log_activity($this->session->userdata('SESS_USER_ID'), 'Added rewards record');
    $this->session->set_userdata('success', 'Submitted Successfully!');
    redirect($module_name . "/" . $page_name);
  }
  function delete_row(){
    ...
    $this->logger->log_activity($this->session->userdata('SESS_USER_ID'), 'Deleted rewards record');
  ...
  function edit_bulk_status(){
    ...
    $this->logger->log_activity($this->session->userdata('SESS_USER_ID'), 'Updated rewards bulk status');
    $this->session->set_userdata('success', 'Submitted Successfully!');
    //  var_dump($ids_int);
