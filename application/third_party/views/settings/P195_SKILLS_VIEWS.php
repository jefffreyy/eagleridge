    ...
    <div class="flex-fill">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">Skills
          </li>
        </ol>
      </nav>
      ...
      <div class="row mb-2">
        ...
        <div class="col ml-auto">
          <a class="btn btn-primary float-right" title="Add" href="#" data-toggle="modal" data-target="#modal_add_skills">
      ...
      <div class="card">
        <table class="table table-hover table-xs">
          ...
          <tbody>
            <?php
if($DISP_SKILL_INFO){
foreach($DISP_SKILL_INFO as $ROW_SKILL_INFO){ ?>
            <tr>
              <td>
                <?=$ROW_SKILL_INFO->col_skill_name?>
              </td>
              <td class="">
                <div class="btn-toolbar float-right" role="toolbar" aria-label="Toolbar with button groups">
                  <div class="btn-group mr-2 btn-group-sm" role="group" aria-label="First group">
                    <a class="btn btn-sm indigo lighten-2" skills_id="<?=$ROW_SKILL_INFO->id?>" title="Edit" data-toggle="modal" data-target="#modal_edit_skills" >
                      <i class="fas fa-edit">
                      </i>
                    </a>
                    <a class="btn btn-sm indigo lighten-2 text-danger SKILL_BTN_DLT" delete_key="<?=$ROW_SKILL_INFO->id?>">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header pb-0" style="border-bottom: none;">
        <h4 class="modal-title ml-1" id="exampleModalLabel">Add Skills
        </h4>
      ...
      <form action="<?php echo base_url('settings/insrt_skills'); ?>" id="SKILL_FORM_ADD" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label class="required " for="SKILL_INPF_NAME">Name
                </label>
                <input class="form-control form-control " type="text" name="SKILL_INPF_NAME" id="SKILL_INPF_NAME">
              </div>
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button class='btn btn-primary text-light' id="SKILL_BTN_SAVE">&nbsp; Save
          </button>
        </div>
      </form>
    </div>
...
<div class="modal fade" id="modal_edit_skills" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header pb-0" style="border-bottom: none;">
        <h4 class="modal-title ml-1" id="exampleModalLabel">Edit Skills
        </h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;
          </span>
      ...
      <form action="<?php echo base_url('settings/updt_skills'); ?>" id="SKILL_FORM_EDIT" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
        <div class="modal-body">
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input class="form-control form-control " type="text" name="UPDT_SKILL_INPF_NAME" id="UPDT_SKILL_INPF_NAME" required>
              </div>
            </div>
            <input type="hidden" name="UPDT_SKILL_INPF_ID" id="UPDT_SKILL_INPF_ID">
          </div>
        </div>
        <div class="modal-footer">
          <a class='btn btn-primary text-light' id="SKILL_BTN_UPDT">&nbsp; Update
          </a>
        </div>
      </form>
    </div>
...
<!-- SESSION MESSAGES -->
<?php
if($this->session->userdata('SESS_SUCC_MSG_INSRT_SKILL')){
?>
<script>
  Swal.fire(
    '<?php echo $this->session->userdata('SESS_SUCC_MSG_INSRT_SKILL'); ?>',
    '',
    'success'
  )
</script>
<?php
$this->session->unset_userdata('SESS_SUCC_MSG_INSRT_SKILL');
}
?>
<?php
if($this->session->userdata('SESS_SUCC_MSG_UPDT_SKILL')){
?>
<script>
  Swal.fire(
    '<?php echo $this->session->userdata('SESS_SUCC_MSG_UPDT_SKILL'); ?>',
    '',
    'success'
  )
</script>
<?php
$this->session->unset_userdata('SESS_SUCC_MSG_UPDT_SKILL');
}
?>
<?php
...
<script>
  $(document).ready(function(){
    ...
    var url = '<?php echo base_url(); ?>settings/getskillData';
    const openModalButton = document.querySelectorAll('[data-target]');
    openModalButton.forEach(button => {
      button.addEventListener('click', () => {
        const modal = document.querySelector(button.dataset.target);
        getskillData(url,button.getAttribute('skills_id')).then(data => {
          if(data.length > 0)
          {
            data.forEach((x) => {
              document.getElementById('UPDT_SKILL_INPF_ID').value = x.id;
              document.getElementById('UPDT_SKILL_INPF_NAME').value = x.col_skill_name;
            });
          }
        });
      });
    });
    async function getskillData(url,skills_id) {
      var formData = new FormData();
      formData.append('SKILL_GET_SKILL_DATA', skills_id);
    ...
    // Update Position
    $('#SKILL_BTN_UPDT').click(function(){
      var skills_name = $('#UPDT_SKILL_INPF_NAME').val();
      var hasErr = 0;
      if(!skills_name){
        hasErr++;
      }
      if(hasErr == 0){
        Swal.fire({
          ...
          if (result.isConfirmed) {
            $('#SKILL_FORM_EDIT').submit();
          }
        })
      }
      else {
        $('#UPDT_SKILL_INPF_NAME').addClass('is-invalid');
      }
    })
    $('#UPDT_SKILL_INPF_NAME').keyup(function(){
      $('#UPDT_SKILL_INPF_NAME').removeClass('is-invalid');
    })
    // Delete Position
    $('.SKILL_BTN_DLT').click(function(e){
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location.href = "<?= base_url(); ?>settings/dlt_skills?delete_id="+user_deleteKey;
        }
      })
    })
  })
