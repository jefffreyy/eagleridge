...
<?php 
    // Your user ID
    $this_user_id = $this->session->userdata('SESS_USER_ID');
                ...
                <div class = "col-md-9">
                    <div class = "row">
                        <div class = "mini-nav">
                            ...
                            <a href = "<?=base_url()?>profile/assets" class = "mini-links active">Assets</a>
                    ...
                    <br>
                    <div class = "card border-0">
                        <div class = "card-title mb-0">Assets (Holding)
                            <div class = "modal-btn">
                                <a type = "button" class = "btn btn-primary mr-2 text-white" style="display: none;" data-toggle="modal" data-target="#modal_add_assets"><i class="fas fa-plus mr-2"></i>Assign</a>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-xs table-hover table-nowrap mb-0" style="border:none;">
                                ...
                                <?php 
                                if($DISP_EMP_ASSETS ){
                                    foreach($DISP_EMP_ASSETS as $DISP_EMP_ASSETS_ROW){
                                    ?>
                                        <tr>
                                            <td><?= $DISP_EMP_ASSETS_ROW->col_asset_id ?></td>
                                            <td><a href="#"><?= $DISP_EMP_ASSETS_ROW->col_asset_name ?></a></td>
                                            <td><?= $DISP_EMP_ASSETS_ROW->col_asset_issued_on ?></td>
                                            <td><a href="#" style="display: none;"><i class="fas fa-edit"></i></a></td>
                                        </tr>
                                    <?php
                                    }
                                } else {
                                ?>
                                    <tr>
                                        <td colspan="4">No Assets Borrowed</td>
                                    </tr>
                                <?php
                                }
                                ?>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header pb-0" style="border-bottom: none;">
                    <h4 class="modal-title ml-1" id="exampleModalLabel">Assign Assets</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="<?php echo base_url('profile/add_employee_asset'); ?>" id="ADD_ASSET_FORM" method="post" accept-charset="utf-8" autocomplete='off' class="m-2">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label class="required" class="required" for="INSRT_ASSET_NAME">Asset</label>
                                    <select name="INSRT_ASSET_NAME" id="INSRT_ASSET_NAME" class="form-control" required>
                                        <option value="">Choose...</option>
                                        <?php 
                                            if($DISP_ASSETS_LISTS){
                                                foreach($DISP_ASSETS_LISTS as $DISP_ASSETS_LISTS_ROW){ ?>
                                                    <option value="<?= $DISP_ASSETS_LISTS_ROW->id ?>"><?= $DISP_ASSETS_LISTS_ROW->col_asset_name ?></option>
                                        <?php   }
                                            } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="required" for="INSRT_ASSET_ISSUED_ON">Received On</label>
                                    <input type="date" class="form-control" name="INSRT_ASSET_ISSUED_ON" id="INSRT_ASSET_ISSUED_ON" required>
                                </div>
                            </div>
                            <input type="hidden" name="INSRT_ASSET_USER_ID" id="INSRT_ASSET_USER_ID" value="<?= $this_user_id ?>">
                            <input type="hidden" name="INSRT_ASSET_EMPL_ID" id="INSRT_ASSET_EMPL_ID" value="<?= $user_id ?>">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button class='btn btn-primary text-light' type="submit" id="ASSET_BTN_INSRT">&nbsp; Save</button>
                    </div>
                </form>
            </div>
        </div>
