...
<div class="container-fluid">
    <div class="flex-fill p-4">
        <div class="row">
            <div class="col-md-6">
                <h1 class="page-title">Biometrics Integration</h1>
                                ...
                                if($DISP_BIOMETRICS_DATA){
                                    foreach($DISP_BIOMETRICS_DATA as $DISP_BIOMETRICS_DATA_ROW){
                                        ...
                                        if($date == $current_date){
                                            ?>
                                                <tr class="biom_row" style="background-color: <?= $row_color ?> !important; ">
                                                    <td style="display: none;"><?= $DISP_BIOMETRICS_DATA_ROW->id ?></td>
                                                    <td><?= $date ?></td>
                                                    <td><?= $time ?></td>
                                                    <td><?= $check_type ?></td>
                                                    <td empl_cmid="<?= $empl_cmid ?>"><?= $empl_cmid ?></td>
                                                    <td><?= $full_name ?></td>
                                                </tr>
                                            <?php
                                        }
        ...
        <input type="hidden" name="curr_date" id="curr_date" value="<?= date('1/3/2022'); ?>">
    </div>
</div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        var url_get_biometrics_data = '<?= base_url() ?>integbiotoattrec/get_biometrics_data';
        ...
        var url_updt_attendance_data = '<?= base_url() ?>integbiotoattrec/updt_attendance_data';
        var current_date = '';
        var check_type = '';
        var db_date = '';
        ...
        function load_biometrics_data(){
            ...
            var first_row = $('#tbl_biometrics_data').children()[0];
            console.log();

            var biom_id = $($(first_row).children()[0]).html();
            var date = $($(first_row).children()[1]).html();
            var time = $($(first_row).children()[2]).html();
            var type = $($(first_row).children()[3]).html();
            var empl_id = $($(first_row).children()[4]).html();
            var name = $($(first_row).children()[5]).html();
            
            var date = moment(date).format('YYYY-MM-DD');
            time = moment(time, "h:mm:ss A").format("HH:mm:ss");
            
            // console.log('biom_id: ' + biom_id);
            // console.log('date: ' + date);
            // console.log('time: ' + moment(time, "h:mm:ss A").format("HH:mm:ss"));
            // console.log('type: ' + type);
            // console.log('empl_id: ' + empl_id);
            // console.log('name: ' + name);
            
            updt_attendance_data(url_updt_attendance_data, date, time, empl_id, type, biom_id).then(function(data){
                console.log(data);
            });
            
            $(first_row).remove();

            if($('.biom_row').length == 0){
                get_biometrics_data(url_get_biometrics_data, 'test').then(function(data){
                    if(data.length > 0){
                        // var date_arr = [];
        ...
        var biom_interval = setInterval(load_biometrics_data, 2500);
        ...
        async function get_biometrics_data(url,sample_var){
            var formData = new FormData();
        ...
        async function updt_attendance_data(url, date, time, empl_id, type, biom_id){
            var formData = new FormData();
