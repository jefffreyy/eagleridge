  ...
  <div class="container-fluid p-4">
    <div class="row">
      ...
      <div class="col-md-6" style="text-align: right;">
        <a href="#" class="btn btn-primary shadow-none" data-toggle="modal" data-target="#modal_assign_leave">Assign Leave</a>
      </div>
    </div>
        ...
        <tbody id="tbl_application_container">
          ...
          if ($DISP_ASSIGNED_LEAVE) {
            foreach ($DISP_ASSIGNED_LEAVE as $DISP_ASSIGNED_LEAVE_ROW) {
              ...
              $assigned_by = $this->p020_emplist_mod->MOD_DISP_EMPLOYEE($DISP_ASSIGNED_LEAVE_ROW->col_assigned_by);
              $db_date_from = $DISP_ASSIGNED_LEAVE_ROW->col_leave_from;
              $date_created = $DISP_ASSIGNED_LEAVE_ROW->col_date_created;
              $db_id = $DISP_ASSIGNED_LEAVE_ROW->id;
              $application_date = date('M j, Y', strtotime($date_created));
              $date_from = date('M j, Y', strtotime($db_date_from));
              $application_id = 'LV' . str_pad($db_id, 5, "0", STR_PAD_LEFT);;
          ...
          ?>
              <tr class="view_leave_details" style="cursor: pointer;" data-toggle="modal" data-target="#modal_leave_details" employee_id="<?= $DISP_ASSIGNED_LEAVE_ROW->col_empl_id ?>" leave_id="<?= $DISP_ASSIGNED_LEAVE_ROW->id ?>">
                ...
                <td>
                  <a href="<?= base_url() ?>employees/personal?id=<?= $employee_id[0]->id ?>">
                    <img class="rounded-circle avatar " width="35" height="35" src="<?php if ($employee_id[0]->col_imag_path) {
                                                                                      ...
                                                                                      echo base_url() . 'user_images/default_profile_img3.png';
                                                                                    } ?>">&nbsp;&nbsp;<?= $employee_id[0]->col_last_name . ', ' . $employee_id[0]->col_frst_name . ' ' . $midl_ini2 ?>
                  </a>
                </td>
                <td><?= $DISP_ASSIGNED_LEAVE_ROW->col_leave_type ?></td>
                <td style="word-break: break-word;"><?php if ($DISP_ASSIGNED_LEAVE_ROW->isEdited_reason == 1) {
                                                      echo '<span class="text-success text-bold">(Edited)</span><br>';
                                                    } ?><?= $DISP_ASSIGNED_LEAVE_ROW->col_leave_comments ?></td>
                <td><?php if ($DISP_ASSIGNED_LEAVE_ROW->isEdited_duration == 1) {
                      echo '<span class="text-success text-bold">(Edited)</span><br>';
                    } ?><?= number_format($DISP_ASSIGNED_LEAVE_ROW->col_leave_duration, 2, '.', ""); ?></td>
                ...
                <td><?= $DISP_ASSIGNED_LEAVE_ROW->col_reason_rejection ?></td>
              </tr>
            <?php
            }
          } else {
...
<div class="modal fade" id="modal_leave_details" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form action="<?php echo base_url('leave/updt_leave_details'); ?>" id="form_updt_leave" method="post" accept-charset="utf-8" autocomplete='off' class="m-2" enctype="multipart/form-data">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary text-white" id="btn_save" disabled>Save</button>
        </div>
